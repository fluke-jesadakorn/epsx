...
# Load environment variables using environment manager
...
if [[ -f "$SCRIPT_DIR/env-manager.sh" ]]; then
    ...
    # Detect and load appropriate environment
    DEPLOY_ENV="${DEPLOY_ENV:-$(detect_environment)}"
    
    # For Cloud Run deployment, prefer production environment
    if [[ "$DEPLOY_ENV" == "development" ]]; then
        DEPLOY_ENV="cloud-run"
    fi
    
    echo -e "${PURPLE}Deployment Environment: ${GREEN}$DEPLOY_ENV${NC}"
    load_environment "$DEPLOY_ENV" "" true
else
    echo -e "${YELLOW}⚠️  env-manager.sh not found, using legacy environment loading${NC}"
    # Fallback to original shared loading
