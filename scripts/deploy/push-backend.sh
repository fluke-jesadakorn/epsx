...
# EPSX Backend - Manual Push Script
# Push backend image to trigger auto-revision deployment
...
echo "🚀 Pushing EPSX Backend to trigger auto-revision..."
...
if ! docker images "$LOCAL_TAG" --format "{{.Repository}}:{{.Tag}}" | grep -q "$LOCAL_TAG"; then
    ...
    echo "💡 Run './scripts/build/local-backend.sh' first to build the image"
    exit 1
fi
...
echo "   ./scripts/deploy/status.sh backend"
echo "   ./scripts/deploy/logs.sh backend"
echo ""
