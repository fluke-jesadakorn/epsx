...
echo "🎭 Deploying to Staging Environment (Unified)"
echo "=========================================="
...
# Load and validate staging environment using unified schema
load_and_validate_env "staging"

# Get environment configuration
get_env_config "staging"
...
print_info "Building and deploying to staging with unified environment..."
print_info "Staging URLs: ${BACKEND_URL}, ${FRONTEND_URL}, ${ADMIN_FRONTEND_URL}"
...
deploy_service() {
    ...
    local MEMORY=$4
    local CPU=$5
    local SERVICE_NAME="epsx-${SERVICE}-staging"
    
    print_info "Deploying $SERVICE to staging..."
    ...
    if [ "$SERVICE" = "backend" ]; then
        gcloud run deploy $SERVICE_NAME \
            ...
            --set-env-vars="$(get_backend_env_vars "staging")"
    else
        gcloud run deploy $SERVICE_NAME \
            --image=$REGISTRY/$PROJECT_ID/epsx/$SERVICE:$(git rev-parse --short HEAD) \
            --platform=managed \
            ...
            --min-instances=1 \
            --max-instances=5 \
            --set-env-vars="$(get_frontend_env_vars "staging")"
    fi
        
    print_status "$SERVICE deployed successfully to staging!"
...
# Deploy services with staging resource settings (production-like)
print_info "Starting staging deployment of all services..."

deploy_service "backend" "apps/backend" "8080" "2Gi" "2"
deploy_service "frontend" "apps/frontend" "3000" "1Gi" "1"  
deploy_service "admin" "apps/admin-frontend" "3000" "1Gi" "1"

echo "🎉 Staging deployment complete!"
echo ""
print_status "Your EPSX staging environment is ready:"
echo "🎭 Backend:  ${BACKEND_URL}"
echo "🎭 Frontend: ${FRONTEND_URL}"
echo "🎭 Admin:    ${ADMIN_FRONTEND_URL}"
echo ""
print_info "Enhanced staging features:"
echo "  ✓ Unified environment configuration"
echo "  ✓ Complete Firebase client-side integration"
echo "  ✓ Enhanced OIDC authentication"
echo "  ✓ All client-side environment variables properly configured"
echo ""
print_warning "Staging testing checklist:"
echo "  1. Test all authentication flows"
echo "  2. Verify Firebase functionality"
echo "  3. Check OIDC token exchange"
echo "  4. Test production-like performance"