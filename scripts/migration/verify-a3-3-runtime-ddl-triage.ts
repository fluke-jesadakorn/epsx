...
if (!Array.isArray(contract.findings) || contract.findings.length !== 39) {
  fail("the contract must enumerate exactly 39 scanner findings");
}
...
if (actionable.length !== 33 || reviewedExceptions.length !== 6) {
  fail(`expected 33 actionable and 6 exceptions, observed ${actionable.length}/${reviewedExceptions.length}`);
}
...
if (jsonOutput) {
  ...
  console.log(
    "a3-3-runtime-ddl-triage: OK — deterministic offline/static integrity only; all 33 actionable findings remain blocked",
  );
}
