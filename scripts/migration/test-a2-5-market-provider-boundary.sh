...
VERIFY="$SCRIPT_DIR/verify-a2-5-market-provider-boundary.sh"
CONTRACT_REL="docs/migration/contracts/a2-5-market-provider-boundary.json"
TEMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/epsx-a2-5-provider.XXXXXX")"
...
expect_failure() {
  ...
  if [[ "$status" -ne 1 ]]; then
    cat "$output" >&2
    echo "market-provider-boundary-self-test: expected $label exit 1, got $status" >&2
    exit 1
  fi
  grep -q "market-provider-boundary: ERROR:" "$output"
}

BASE="$TEMP_ROOT/base"
copy_fixture "$BASE"
verify_fixture "$BASE" integrity >"$TEMP_ROOT/integrity.out"
grep -q "14 invariants; 18 hermetic tests; 12 implementation digests; 13 residual STOPs" "$TEMP_ROOT/integrity.out"
...
if (report.productionReady !== false || report.readinessExit !== 3 || report.invariants !== 14 || report.hermeticTests !== 18 || report.implementationEvidence !== 12 || report.residualStops.length !== 13) process.exit(1);
...
if [[ "$readiness_status" -ne 3 ]]; then
  ...
  echo "market-provider-boundary-self-test: expected readiness exit 3, got $readiness_status" >&2
  exit 1
fi
grep -q "13 residual STOPs remain" "$TEMP_ROOT/readiness.out"
...
expect_failure live-env env TRADINGVIEW_URL=https://example.invalid "$VERIFY" --mode integrity --evidence-root "$BASE" --contract "$BASE/$CONTRACT_REL" --static-only
...
if grep -Fq 'git("rev-parse", `${expectedTarget.ref}^{commit}`)' "$VERIFY"; then
  echo "market-provider-boundary-self-test: target base must remain valid after the branch advances" >&2
  exit 1
fi
...
printf '\n// mutation\n' >>"$TAMPER_ROOT/apps/backend/src/web/analytics/eps/cache.rs"
...
mv "$SYMLINK_ROOT/apps/backend/src/web/analytics/eps/cache.rs" "$TEMP_ROOT/outside-cache.rs"
ln -s "$TEMP_ROOT/outside-cache.rs" "$SYMLINK_ROOT/apps/backend/src/web/analytics/eps/cache.rs"
expect_failure symlink-escape verify_fixture "$SYMLINK_ROOT" integrity

echo "market-provider-boundary-self-test: PASS 16/16 environment, static-mode, sentinel, immutable-base, pin, tuple, inventory, order, path, digest, tamper, and symlink cases"
