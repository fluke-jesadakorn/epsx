...
if (!Array.isArray(manifest.capabilities) || manifest.capabilities.length !== 9) fail("nine capability records are required");
if (!Array.isArray(manifest.cases) || manifest.cases.length !== 24) fail("twenty-four fixed cases are required");
...
console.log(`auth-session-gate: manifest 24/24 cases, ${evidenceTotal}/${evidenceTotal} anchors, ${expectedTotal} focused tests`);
...
run_test frontend-recovery-journey 1 cargo test --offline --locked -p epsx-frontend routing_tests::frontend_emits_one_private_recovery_bootstrap_only_for_refresh_eligible_html --no-fail-fast
