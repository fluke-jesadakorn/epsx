...
grep -q "PASS integrity (27 source routes; 3 redirects; 2 aligned, 4 partial, 21 blocked; 20 STOP blockers" "$temp_dir/integrity.out"
...
grep -q "STOP readiness (25 non-aligned routes: 4 partial, 21 blocked; 20 cross-cutting blockers)" "$temp_dir/readiness.out"
...
bun -e 'const report = await Bun.file(process.argv[1]).json(); if (report.routeCount !== 27 || report.redirectCount !== 3 || report.stopBlockerCount !== 20 || report.productionReady !== false || report.readinessExit !== 3 || report.statuses.aligned !== 2 || report.statuses.partial !== 4 || report.statuses.blocked !== 21) process.exit(1);' "$temp_dir/emit-one.json"
