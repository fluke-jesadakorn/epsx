...
helper="$repo_root/e2e/frontend/fixtures/about-auth-mock.mjs"
temp_dir=$(mktemp -d "${TMPDIR:-/tmp}/epsx-about-runtime.XXXXXX")
...
if curl --silent --fail --max-time 1 "$issuer/healthz" >/dev/null 2>&1; then
  echo "about runtime proof: localhost port 18080 is already in use" >&2
  exit 1
fi
if curl --silent --fail --max-time 1 http://127.0.0.1:3000/ >/dev/null 2>&1; then
  echo "about runtime proof: localhost port 3000 is already in use" >&2
  exit 1
fi
...
[ "$ready" -eq 1 ] || { cat "$temp_dir/auth.log" >&2; echo "about runtime proof: auth fixture did not start" >&2; exit 1; }
...
[ "$ready" -eq 1 ] || { cat "$temp_dir/frontend.log" >&2; echo "about runtime proof: frontend did not start" >&2; exit 1; }

A7_ABOUT_ACCESS_TOKEN=$(node "$helper" token "$temp_dir/private.pem" "$issuer") \
  bunx playwright test e2e/frontend/about-runtime.spec.ts --project=frontend --workers=1

echo "about runtime proof: PASS (ephemeral localhost RS256/JWKS fixture; cleanup armed)"
