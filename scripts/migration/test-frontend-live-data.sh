...
grep -q "PASS integrity (28 routes; 2 aligned, 8 partial, 18 blocked" "$temp_dir/integrity.out"
...
bun -e 'const report = await Bun.file(process.argv[1]).json(); if (report.routeCount !== 28 || report.productionReady !== false || report.readinessExit !== 3 || report.statuses.aligned !== 2 || report.statuses.partial !== 8 || report.statuses.blocked !== 18) process.exit(1);' "$temp_dir/emit-one.json"
