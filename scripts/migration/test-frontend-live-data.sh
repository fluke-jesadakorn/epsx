...
grep -q "PASS integrity (28 routes; 2 aligned, 9 partial, 17 blocked" "$temp_dir/integrity.out"
...
grep -q "STOP readiness (26 non-aligned routes" "$temp_dir/readiness.out"
...
bun -e 'const report = await Bun.file(process.argv[1]).json(); if (report.routeCount !== 28 || report.productionReady !== false || report.readinessExit !== 3 || report.statuses.aligned !== 2 || report.statuses.partial !== 9 || report.statuses.blocked !== 17) process.exit(1);' "$temp_dir/emit-one.json"
bun -e '
