...
grep -q "14 source pins, 40 target anchors, A2.4/A2.5/A2.6/A2.7/A2.8/A2.9/A2.10/A2.11 boundary contracts, 4 separate domains, 16 surfaces, and 24 stop blockers" "$temp_dir/integrity.out"
...
EPSX_A2_11_EVIDENCE_ROOT="$temp_dir" EPSX_A2_11_STATIC_ONLY=1 \
...
if (JSON.stringify(report.composedBoundaryEvidence) !== JSON.stringify(["A2.4", "A2.5", "A2.6", "A2.7", "A2.8", "A2.9", "A2.10", "A2.11"])) process.exit(1);
...
grep -q "B07 must retain A2.6/A2.7/A2.8/A2.9/A2.10/A2.11 evidence" "$temp_dir/a2-7-missing-link.out"
...
grep -q "B07 must retain A2.6/A2.7/A2.8/A2.9/A2.10/A2.11 evidence" "$temp_dir/a2-8-missing-b07-link.out"
...
grep -q "B21 must keep A2.7/A2.8 fixtures and A2.11 extraction distinct from reconciliation" "$temp_dir/a2-8-missing-b21-link.out"
...
grep -q "B07 must retain A2.6/A2.7/A2.8/A2.9/A2.10/A2.11 evidence" "$temp_dir/a2-10-missing-b07-link.out"

A12_CONTRACT_IN="$contract" A12_CONTRACT_OUT="$temp_dir/a2-11-drifted-link.json" bun -e '
const contract = await Bun.file(process.env.A12_CONTRACT_IN).json();
const evidence = contract.targetEvidence.find((item) => item.id === "tgt-identity-a2-11-ranking-store-extraction-contract");
evidence.anchor = "\"productionReady\": false";
await Bun.write(process.env.A12_CONTRACT_OUT, `${JSON.stringify(contract, null, 2)}\n`);
'
set +e
"$verify" --mode integrity --contract "$temp_dir/a2-11-drifted-link.json" >"$temp_dir/a2-11-drifted-link.out" 2>&1
a2_11_drifted_link_status=$?
set -e
if [ "$a2_11_drifted_link_status" -ne 1 ]; then
  cat "$temp_dir/a2-11-drifted-link.out" >&2
  echo "analytics-indexer-execution self-test: expected a2-11-drifted-link exit 1, got $a2_11_drifted_link_status" >&2
  exit 1
fi
grep -q "A2.11 ranking-store-extraction evidence is missing or drifted" "$temp_dir/a2-11-drifted-link.out"

A12_CONTRACT_IN="$contract" A12_CONTRACT_OUT="$temp_dir/a2-11-missing-b07-link.json" bun -e '
const contract = await Bun.file(process.env.A12_CONTRACT_IN).json();
const blocker = contract.blockers.find((item) => item.id === "B07");
blocker.evidenceIds = blocker.evidenceIds.filter((id) => id !== "tgt-identity-a2-11-ranking-store-extraction-contract");
await Bun.write(process.env.A12_CONTRACT_OUT, `${JSON.stringify(contract, null, 2)}\n`);
'
set +e
"$verify" --mode integrity --contract "$temp_dir/a2-11-missing-b07-link.json" >"$temp_dir/a2-11-missing-b07-link.out" 2>&1
a2_11_missing_b07_status=$?
set -e
if [ "$a2_11_missing_b07_status" -ne 1 ]; then
  cat "$temp_dir/a2-11-missing-b07-link.out" >&2
  echo "analytics-indexer-execution self-test: expected a2-11-missing-b07-link exit 1, got $a2_11_missing_b07_status" >&2
  exit 1
fi
grep -q "B07 must retain A2.6/A2.7/A2.8/A2.9/A2.10/A2.11 evidence" "$temp_dir/a2-11-missing-b07-link.out"

A12_CONTRACT_IN="$contract" A12_CONTRACT_OUT="$temp_dir/a2-11-missing-b21-link.json" bun -e '
const contract = await Bun.file(process.env.A12_CONTRACT_IN).json();
const blocker = contract.blockers.find((item) => item.id === "B21");
blocker.evidenceIds = blocker.evidenceIds.filter((id) => id !== "tgt-identity-a2-11-ranking-store-extraction-contract");
await Bun.write(process.env.A12_CONTRACT_OUT, `${JSON.stringify(contract, null, 2)}\n`);
'
set +e
"$verify" --mode integrity --contract "$temp_dir/a2-11-missing-b21-link.json" >"$temp_dir/a2-11-missing-b21-link.out" 2>&1
a2_11_missing_b21_status=$?
set -e
if [ "$a2_11_missing_b21_status" -ne 1 ]; then
  cat "$temp_dir/a2-11-missing-b21-link.out" >&2
  echo "analytics-indexer-execution self-test: expected a2-11-missing-b21-link exit 1, got $a2_11_missing_b21_status" >&2
  exit 1
fi
grep -q "B21 must keep A2.7/A2.8 fixtures and A2.11 extraction distinct from reconciliation" "$temp_dir/a2-11-missing-b21-link.out"
...
echo "analytics-indexer-execution self-test: PASS (integrity=0, readiness-stop=3, deterministic=stable, source/schema-anchor/A2.5/A2.6/A2.7/A2.8/A2.9/A2.10/A2.11-link/stale/path/domain/blocker/prod/live tamper=1)"
