...
grep -q "6 security-gate blockers" "$temp_dir/readiness.out"
...
if (scan.summary.total !== 61 || scan.summary.sourceCounts["dioxus-security-gate"] !== 27 || scan.summary.classificationCounts["legacy-2-segment"] !== 6) process.exit(1);
...
echo "permission-grammar self-test: PASS (61 records, 6 security-gate blockers, integrity=0, readiness-stop=3, deterministic=stable, tamper=1)"
