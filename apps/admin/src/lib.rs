...
mod auth_fullstack;
mod core_credits_fullstack;
mod core_wallets_fullstack;
mod escrow_fullstack;
pub mod fullstack;
mod fullstack_catalog;
mod fullstack_chat;
mod fullstack_developer;
mod fullstack_notifications;
mod fullstack_orders;
mod fullstack_payments;
mod fullstack_settings;
mod media_fullstack;
mod native_escrow;
mod news_fullstack;
mod plan_catalog;
mod wallets_fullstack;
use axum::{
    body::to_bytes,
...
pub async fn run() {
    ...
    epsx_bff::fullstack::verify_public_assets().expect("packaged Admin Fullstack assets");
    let app = fullstack::application(state);
...
pub fn state_from_env() -> Result<AppState, String> {
    ...
    validate_internal_url(&api_url, cookie_environment, "API_URL/BACKEND_URL")?;
    for (url, label) in [
        ...
        validate_internal_url(url, cookie_environment, label)?;
    }
    validate_auth_url(&issuer, cookie_environment, "OIDC_ISSUER/BACKEND_URL")?;

    let jwks_url = std::env::var("OIDC_JWKS_URL")
        .unwrap_or_else(|_| format!("{}{}", issuer.trim_end_matches('/'), JWKS_PATH));
    validate_internal_url(&jwks_url, cookie_environment, "OIDC_JWKS_URL")?;
...
fn validate_internal_url(
    ...
    if url.scheme() == "http" && !loopback {
        return Err(format!("{label} HTTP must use numeric loopback"));
    }
    if url.scheme() == "http" && loopback {
        return validate_auth_url(value, CookieEnvironment::Local, label);
    }
    validate_auth_url(value, environment, label)
}
...
fn build_app(state: AppState) -> Router {
    Router::new()
        .merge(fullstack::server_functions(state.clone()))
        .route(
            "/favicon.ico",
            get(|| async {
                (
        ...
        .route("/auth", get(fallback_handler))
        ...
        // Static assets
        .route("/plans", get(fallback_handler))
        .route(
            "/plans/{id}",
            get(fallback_handler).post(plan_catalog::save),
        )
        .route("/payments/epsx", get(fallback_handler))
        .route("/payments/epsx/{id}", get(fallback_handler))
        .route("/pay/escrows", get(fallback_handler))
        .route("/pay/escrows/{id}", get(fallback_handler))
        .route("/pay/merchant-escrows", get(fallback_handler))
        .route("/pay/merchant-escrows/{id}", get(fallback_handler))
        .route(
            "/api/v1/pay/{*path}",
            get(native_escrow::merchant_proxy).post(native_escrow::merchant_proxy),
        )
        .route("/public/dist/tailwind.css", get(serve_tailwind_css))
...
async fn serve_tailwind_css() -> impl IntoResponse {
    let path = std::path::Path::new(&public_dir()).join("dist/tailwind.css");
    match tokio::fs::read(&path).await {
        Ok(bytes) => (
            ...
            .into_response(),
        Err(_) => (StatusCode::NOT_FOUND, "tailwind.css not found").into_response(),
    }
}

fn public_dir() -> String {
    std::env::var("EPSX_PUBLIC_DIR")
...
async fn require_verified_admin_session(
    ...
    if path == "/auth"
        && (query_has_key(request.uri().query(), "clear")
            || request.uri().query().is_some_and(|q| {
                url::form_urlencoded::parse(q.as_bytes())
                    .any(|(k, v)| k == "reason" && v == "no-session")
            }))
    {
    ...
    if !is_known_protected_admin_api_path(path) {
        if request.method() == Method::GET
            && ((path.starts_with("/wallet-management/")
                && !matches!(
                    epsx_dioxus_ui::fullstack::admin_wallets::WalletPage::from_path(path),
                    epsx_dioxus_ui::fullstack::admin_wallets::WalletPage::NotFound
                ))
                || path == "/media"
                || path == "/pay/escrows"
                || path.starts_with("/pay/escrows/")
                || path == "/pay/merchant-escrows"
                || path.starts_with("/pay/merchant-escrows/")
                || path == "/auth"
                || path == "/plans"
                || path.starts_with("/plans/")
                || path == "/payments/epsx"
                || path.starts_with("/payments/epsx/"))
        {
            if path
                .strip_prefix("/plans/")
                .or_else(|| path.strip_prefix("/payments/epsx/"))
                .is_some_and(|id| uuid::Uuid::parse_str(id).is_err())
            {
                return StatusCode::NOT_FOUND.into_response();
            }
            if let Some(fullstack) = request
                .extensions()
                .get::<dioxus_server::FullstackState>()
                .cloned()
            {
                let mut response =
                    dioxus_server::FullstackState::render_handler(State(fullstack), request).await;
                response.headers_mut().insert(
                    header::CACHE_CONTROL,
                    HeaderValue::from_static("private, no-store"),
                );
    ...
    if !is_allowed_protected_admin_api_method(request.method(), path) {
        if path.starts_with("/api/v1/pay/") {
...
fn is_known_protected_admin_api_path(path: &str) -> bool {
    if merchant_pay_api_method(&Method::GET, path) || merchant_pay_api_method(&Method::POST, path) {
        return true;
    }
    let Some(segments) = api_segments(path) else {
        return false;
    ...
    matches!(
        ...
        ["api", "v1", "users"]
            ...
            | ["api", "v1", "admin", "pay", "escrows", _]
            | ["api", "v1", "admin", "pay", "operations", _]
            | ["api", "v1", "admin", "pay", "operations", _, "confirm"]
            | ["api", "v1", "admin", "pay", "contract", "pause"]
            | ["api", "v1", "admin", "pay", "contract", "operations", _]
            | [
                "api",
                "v1",
                "admin",
...
fn is_allowed_protected_admin_api_method(method: &Method, path: &str) -> bool {
    if path.starts_with("/api/v1/pay/") {
        return merchant_pay_api_method(method, path);
    }
    let Some(segments) = api_segments(path) else {
        return false;
    };
    ...
    match segments.as_slice() {
        ...
        | ["api", "v1", "admin", "pay", "contract", "pause"]
        | ["api", "v1", "admin", "pay", "contract", "operations", _]
        | ["api", "v1", "admin", "pay", "contract", "operations", _, "confirm"]
...
/// The merchant proxy remains behind Admin session verification and accepts
/// only the resource shapes used by the escrow and emergency-control screens.
fn merchant_pay_api_method(method: &Method, path: &str) -> bool {
    let Some(segments) = api_segments(path) else {
        return false;
    };
    let read = method == Method::GET || method == Method::HEAD;
    match segments.as_slice() {
        ["api", "v1", "pay", "config" | "contracts" | "escrows"]
        | ["api", "v1", "pay", "intents" | "escrows" | "operations" | "contract-controls", _] => {
            read
        }
        ["api", "v1", "pay", "intents" | "escrows", _, "resolve-release" | "resolve-refund"]
        | ["api", "v1", "pay", "contracts", "direct" | "escrow", "pause"]
        | ["api", "v1", "pay", "operations" | "contract-controls", _, "confirm"] => {
            method == Method::POST
        }
        _ => false,
    }
...
async fn fallback_handler(State(_state): State<AppState>, request: Request) -> Response {
    let path = request.uri().path();
    let canonical = path
        .strip_prefix("/admin/")
        .map(|rest| format!("/{rest}"))
    ...
    if canonical == "/notifications" {
        return Redirect::temporary("/notifications/manage").into_response();
    }
    if canonical == "/wallet-management" {
        return Redirect::permanent("/wallet-management/wallets").into_response();
    }
    if let Some(fullstack) = request
        .extensions()
        .get::<dioxus_server::FullstackState>()
        .cloned()
    {
        ...
        (
            ...
            "Admin Fullstack application is not initialized",
        )
            .into_response()
    }
}
...
mod routing_tests {
    ...
    async fn request_with_cookie(method: Method, uri: &str, cookie: Option<&str>) -> Response {
        ...
        fullstack::application(test_state())
    ...
    #[tokio::test]
    async fn admin_serializes_typed_recovery_without_legacy_runtime_or_cookie_exposure() {
        for (cookie,expected) in [
            ("epsx.admin.refresh_token=opaque-refresh",true),
            ("epsx.frontend.refresh_token=wrong-client",false),
            ("epsx.admin.access_token=malformed; epsx.admin.refresh_token=opaque-refresh",true),
            ("epsx.admin.access_token=eyJhbGciOiJSUzI1NiIsImtpZCI6Im91dGFnZSIsInR5cCI6IkpXVCJ9.e30.c2ln; epsx.admin.refresh_token=opaque-refresh",false),
            ("",false),
        ] {
            let mut headers=HeaderMap::new();headers.insert(header::COOKIE,HeaderValue::from_str(cookie).unwrap());
            let session=auth_fullstack::session(test_state(),headers).await.unwrap();
            assert_eq!(session.recover_session,expected);
            let response=request_with_cookie(Method::GET,"/analytics",Some(cookie)).await;
    ...
    #[tokio::test]
    async fn merchant_pay_routes_reach_auth_and_reject_unknown_shapes() {
        for (method, path) in [
            (Method::GET, "/api/v1/pay/contracts"),
            (Method::GET, "/api/v1/pay/escrows"),
            (Method::GET, "/api/v1/pay/intents/pi_test"),
            (Method::POST, "/api/v1/pay/intents/pi_test/resolve-release"),
            (Method::POST, "/api/v1/pay/escrows/pi_test/resolve-refund"),
            (Method::POST, "/api/v1/pay/contracts/escrow/pause"),
            (
                Method::POST,
                "/api/v1/pay/contract-controls/mctl_test/confirm",
            ),
            (Method::POST, "/api/v1/pay/operations/mop_test/confirm"),
        ] {
            assert_eq!(
                request(method, path).await.status(),
                StatusCode::UNAUTHORIZED,
                "{path}"
            );
        }
        assert_eq!(
            request(Method::DELETE, "/api/v1/pay/contracts")
                .await
                .status(),
        ...
        for path in [
            "/api/v1/pay/api-keys",
            "/api/v1/pay/escrows/pi_test/extra",
            "/api/v1/pay/contracts/unknown/pause",
        ] {
            assert_eq!(
    ...
    async fn admin_redirect_targets_are_fixed_but_transport_drift_remains_visible() {
        for uri in [
            ...
            assert_eq!(auth.status(), StatusCode::OK, "{uri}");
            assert!(auth.headers().get(header::LOCATION).is_none(), "{uri}");
            let body = to_bytes(auth.into_body(), 2_000_000).await.unwrap();
            let body = String::from_utf8_lossy(&body);
            assert!(!body.contains("epsx_browser_runtime_bootstrap"), "{uri}");
        }
        let auth_head = request(Method::HEAD, "/auth?return_url=https%3A%2F%2Fevil.example").await;
        assert_eq!(auth_head.status(), StatusCode::OK);
        assert!(auth_head.headers().get(header::LOCATION).is_none());
        assert!(to_bytes(auth_head.into_body(), 2_000_000)
            .await
            .unwrap()
            .is_empty());
...
async fn submit_commerce_mutation_form(
    ...
    if matches!(
        ...
        return (
            StatusCode::CONFLICT,
            "This form is outdated. Reload Wallets, Credits or EPSX Plans before saving.",
        )
            .into_response();
    }
    let idempotency_key = match form_value(&mut fields, "idempotency_key") {
...
async fn submit_wallet_disable_form(State(state): State<AppState>, request: Request) -> Response {
    // Old commerce version/reason commands cannot safely mutate canonical
    // wallet_users. The current page uses the core_wallets typed command.
    let headers = request.headers();
    if let Err(status) = verified_admin_form_context(&state, headers).await {
        return status.into_response();
    }
    (
        StatusCode::CONFLICT,
        "This wallet form is outdated. Reload the wallet page before changing its status.",
    )
        .into_response()
}
