...
# Build Admin Frontend from apps/admin-frontend directory
set -euo pipefail
...
echo -e "${BLUE}🔨 Building Admin Frontend from Latest Source${NC}"
...
# Image tags
IMAGE_TAG="$REGION-docker.pkg.dev/$PROJECT_ID/$REPOSITORY/admin:$TIMESTAMP"
LATEST_TAG="$REGION-docker.pkg.dev/$PROJECT_ID/$REPOSITORY/admin:latest"
...
if gcloud builds submit . \
    ...
    if gcloud container images add-tag "$IMAGE_TAG" "$LATEST_TAG" --quiet; then
        ...
        echo "$IMAGE_TAG" > .admin-image
        echo "$LATEST_TAG" > .admin-latest
        
        # Cleanup
        rm -f cloudbuild.yaml
        exit 0
    else
        echo -e "${RED}❌ Failed to tag admin frontend as latest${NC}"
        rm -f cloudbuild.yaml
        exit 1
    fi
else
