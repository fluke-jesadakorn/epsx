...
import { ApiResponse, isApiSuccess, PaginatedResponse, UserSubscription } from '../types/api';
import { UnifiedApiClient } from '../utils/api-client';

export type SubscriptionResponse = UserSubscription;
...
/**
 * Plan Response (Legacy/Unified compatibility)
 * Includes pricing and subscription data
 */
...
export class PlansApi {
   ...
   * GET /api/admin/plans
   */
  async listPlans(filters?: PlanFilters): Promise<ApiResponse<PaginatedResponse<PlanResponse>>> {
    return this.client.get<PaginatedResponse<PlanResponse>>('/api/admin/plans', filters);
  }

  /** Alias for listPlans */
  async getPlans(filters?: PlanFilters): Promise<ApiResponse<PaginatedResponse<PlanResponse>>> {
    return this.listPlans(filters);
  }
   ...
   */
  async getPlan(plan_id: string): Promise<ApiResponse<PlanResponse>> {
    return this.client.get<PlanResponse>(`/api/admin/plans/${plan_id}`);
  }
   ...
   */
  async getSubscriptions(filters?: any): Promise<ApiResponse<{ subscriptions: SubscriptionResponse[]; total: number }>> {
  ...
  async getSubscription(id: string): Promise<ApiResponse<SubscriptionResponse>> {
    return this.client.get<SubscriptionResponse>(`/api/admin/subscriptions/${id}`);
  }

  /**
   * Update subscription
   */
  async updateSubscription(id: string, data: UpdateSubscriptionRequest): Promise<ApiResponse<SubscriptionResponse>> {
    return this.client.put<SubscriptionResponse>(`/api/admin/subscriptions/${id}`, data);
  }
  ...
  async cancelSubscription(id: string): Promise<ApiResponse<{ cancelled: boolean }>> {
    return this.client.post<{ cancelled: boolean }>(`/api/admin/subscriptions/${id}/cancel`, {});
  }
}
