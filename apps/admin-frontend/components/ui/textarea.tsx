/**
 * Enhanced Textarea Component - Windows Phone + PancakeSwap Design System
 * Features: adminTextareaVariants with Windows Phone styling and PancakeSwap accents
 */
  ...
  "flex w-full min-h-[80px] resize-y transition-all duration-300 font-light ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 relative",
  {
    variants: {
      ...
      size: {
        ...
        default: "min-h-[80px] px-4 py-3 text-sm rounded-lg",
  ...
  ({ className, variant, size, state, ...props }, ref) => {
    return (
      <div className="relative">
        <textarea
          className={cn(adminTextareaVariants({ variant, size, state, className }))}
          ref={ref}
          {...props}
        />
...
export { Textarea, adminTextareaVariants }