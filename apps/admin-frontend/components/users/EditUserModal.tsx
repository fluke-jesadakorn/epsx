...
import { X, User, Shield, Mail, Check, AlertCircle, Edit } from 'lucide-react'
import { ClientUserAPI } from '@/lib/api/client-admin-api'

/**
 * PancakeSwap x Windows Phone Edit User Modal
 * Modal for editing existing users with backend integration
 */
...
export default function EditUserModal({ isOpen, onClose, onUserUpdated, user }: EditUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id) {
      setErrorMessage('User ID is required for update')
      return
    }
    ...
    try {
      const result = await ClientUserAPI.updateUser(user.id, {
      ...
      setTimeout(() => {
        handleClose()
        onUserUpdated() // Refresh the parent component
      }, 1500)
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-700 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-extralight tracking-wide text-white">edit user</h2>
                <p className="text-white/70 text-xs font-light">update user details</p>
              </div>
            </div>
            <button 
              onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 font-light focus:border-blue-400 focus:outline-none transition-all"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto">
              {PERMISSION_OPTIONS.map(permission => (
                <button
                  ...
                  disabled={isSubmitting}
                  className={`p-3 text-left transition-all border border-transparent hover:border-blue-400 ${
          ...
          {submitStatus === 'success' && (
            <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-600 dark:text-green-400">
              <Check size={16} />
              <span className="text-sm font-light">User updated successfully!</span>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-500 to-blue-700 text-white font-medium hover:from-blue-600 hover:to-blue-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
