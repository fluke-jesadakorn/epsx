...
function getStatusColor(status: string): string {
    switch (status) {
        ...
        case 'expiring_soon': return 'bg-warning/10 text-warning border border-warning/20';
...
interface UserAccessTableProps {
    userAccess: PlanAccessData[];
}

function UserAccessMobileCards({ userAccess }: UserAccessTableProps) {
    return (
        <div className="block sm:hidden space-y-4">
            {userAccess.map((user) => (
                <div key={user.wallet_address} className="p-4 bg-muted/30 border border-border/50 rounded-2xl">
                    ...
                    <div className="mt-3 text-xs text-muted-foreground">
                        Expires: {user.plan_expires_at !== null ? formatDate(user.plan_expires_at) : 'Never'}
                    </div>
                </div>
            ))}
        </div>
...
function UserAccessDesktopTable({ userAccess }: UserAccessTableProps) {
    return (
                    ...
                    {userAccess.map((user) => (
                        <tr key={user.wallet_address} className="hover:bg-muted/30 transition-colors">
                            ...
                            <td className="px-4 py-4 whitespace-nowrap text-sm text-muted-foreground">
                                {user.plan_expires_at !== null ? formatDate(user.plan_expires_at) : 'Never'}
                            </td>
                            <td className="px-4 py-4 whitespace-nowrap">
...
export function UserAccessManagement() {
    ...
    const loadUserAccess = useCallback(async () => {
        try {
            ...
            const response = await fetchUserAccessAction(params);

            if (response.success === true && response.data !== null && response.data !== undefined) {
                const data = response.data as { users?: PlanAccessData[] };
                setUserAccess(data.users ?? []);
            } else {
                const errMsg = response.error?.message ?? 'Failed to load user access';
                        ...
                        <div className="p-4 sm:p-6 lg:p-8">
                            ...
                            {userAccess.length === 0 ? (
                                <div className="text-center py-12 sm:py-16">
                                    ...
                                    <h3 className="text-xl font-semibold text-foreground mb-2">No users with plan access found</h3>
                                    <p className="text-muted-foreground">Users with active subscriptions will appear here</p>
                                </div>
                            ) : (
                                <>
                                    <UserAccessMobileCards userAccess={userAccess} />
                                    <UserAccessDesktopTable userAccess={userAccess} />

                                    <div className="mt-6 flex items-center justify-between">
                                        <button
                                            onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                                            disabled={currentPage === 1}
                                            className="px-4 py-2 text-sm font-semibold rounded-xl border border-border/40 text-muted-foreground hover:bg-muted/50 disabled:opacity-40 transition-all"
                                        >
                                            Previous
                                        </button>
                                        ...
                                        <button
                                            ...
                                            disabled={userAccess.length < itemsPerPage}
                                            className="px-4 py-2 text-sm font-semibold rounded-xl border border-border/40 text-muted-foreground hover:bg-muted/50 disabled:opacity-40 transition-all"
                                        >
                                            Next
                                        </button>
