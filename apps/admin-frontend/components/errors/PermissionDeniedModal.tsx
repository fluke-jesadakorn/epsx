      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-xl">
            {error.error_type === 'InsufficientGroup' ? 'Admin Upgrade Required' : 'Admin Access Restricted'}
          </DialogTitle>
          ...
          {error.current_group && error.required_group && (
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600 dark:text-slate-400">Current Admin Group:</span>
                <Badge variant="outline">{error.current_group}</Badge>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600 dark:text-slate-400">Required Admin Group:</span>
          ...
          {error.current_usage !== undefined && error.limit !== undefined && (
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm text-slate-600 dark:text-slate-400">Admin Usage:</span>
          ...
          {error.error_type === 'SecurityRestriction' && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4 rounded-lg">
              ...
              <p className="text-sm text-red-800 dark:text-red-200 mt-2">
                {error.reason || 'This admin action has been restricted for security reasons.'}
              </p>
          ...
          {error.error_type === 'PermissionExpired' && error.expired_at && (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 p-4 rounded-lg">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                Admin permission expired: {new Date(error.expired_at).toLocaleString()}
              </p>
            </div>
          )}
        </div>
        ...
        <DialogFooter className="flex gap-2">
          ...
          {(error.upgrade_url || error.renewal_url) && (
            ...
            >
              {error.renewal_url ? 'Renew Admin Access' : 'Upgrade Admin Access'}
            </Button>
          )}
          {error.contact_support && (
