                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {users.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getTierColor(user.packageTier)}`}>
                          {user.packageTier}
                        </span>
                      </td>
                      
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(user.subscriptionStatus)}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-y-1">
                        ...
                        <div className="relative">
                          <select
                            onChange={(e) => {
                              if (e.target.value) {
                                if (confirm(`Upgrade ${user.email} to ${e.target.value}?`)) {
                                  handlePackageUpgrade(user.id, e.target.value as PackageTier);
                                }
                          ...
                          >
                            ...
                            {Object.values(PackageTier)
                              ...
                              .map((tier) => (
                                <option key={tier} value={tier}>
                                  To {tier.charAt(0).toUpperCase() + tier.slice(1)}
                                </option>
                              ))}
                          </select>
                        </div>
        ...
        {activeTab === 'users' ? (
          ...
          <div className="p-6">
            <div className="text-center text-gray-500">
              <h3 className="text-lg font-semibold mb-4">Firebase IAM Debug Panel</h3>
              <p>Debug panel temporarily disabled for testing.</p>
              <div className="mt-4">
                <p className="text-sm">
                  The users you're seeing are: <strong>{users.length > 0 ? 'Firebase/Mock Data' : 'No users found'}</strong>
                </p>
                {users.length > 0 && (
                  <div className="mt-2 p-4 bg-gray-100 rounded">
                    <p className="text-sm font-medium">Sample User Info:</p>
                    <p className="text-xs">Email: {users[0]?.email}</p>
                    <p className="text-xs">Name: {users[0]?.name || 'Not set'}</p>
                    <p className="text-xs">Display Name: {users[0]?.displayName || 'Not set'}</p>
