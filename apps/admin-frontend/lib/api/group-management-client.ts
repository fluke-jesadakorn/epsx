...
export const groupMgmt = {
  async getPermissionGroups(): Promise<PermissionGroup[]> {
    const res = await apiGet<any>('/api/admin/permissions/groups')
  ...
  async createPermissionGroup(req: {
    ...
    return apiPost('/api/admin/permissions/groups', req)
  },

  async updatePermissionGroup(groupId: string, req: {
    name?: string
    permissions?: string[]
    description?: string
    default_expiry_days?: number
  ...
  async assignUserToGroup(req: {
    ...
    return apiPost('/api/admin/wallet-assignments', {
  ...
  async verifyWalletAssets(walletAddress: string): Promise<any> {
    return apiPost('/api/auth/web3/assignment/verify-assets', { wallet_address: walletAddress })
  },
  ...
  async getGroupAssignmentHistory(filters?: {
    ...
    if (filters?.operation_source) params.set('operation_source', filters.operation_source)
  ...
  async updateUser(walletAddress: string, updates: {
    tier_level?: string
    is_active?: boolean
    metadata?: any
  }): Promise<any> {
  ...
  async getPlatformOverview(period?: string): Promise<any> {
    return apiGet(`/api/admin/analytics/overview${period ? `?period=${period}` : ''}`)
  },

  async getUserAnalytics(period?: string): Promise<any> {
    return apiGet(`/api/admin/analytics/users${period ? `?period=${period}` : ''}`)
  },
  ...
  async getRevenueAnalytics(period?: string): Promise<any> {
    return apiGet(`/api/admin/analytics/revenue${period ? `?period=${period}` : ''}`)
  },
  ...
  async getGroupAnalytics(): Promise<GroupAnalytics> {
    ...
    return {
      ...
      most_popular_groups: data.group_membership?.slice(0, 3).map((g: any) => ({
        group_name: g.group_name,
        member_count: g.member_count
      })) || [],
      permission_distribution: data.permission_usage?.reduce((acc: any, p: any) => {
  ...
  async checkUserPermission(userId: string, permission: string): Promise<boolean> {
    const res = await apiGet<{ has_permission: boolean }>(`/api/admin/users/${userId}/check-permission?permission=${permission}`)
