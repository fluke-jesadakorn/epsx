/**
 * Consolidated FCM Client
 * Refactored to extend UnifiedAdminClient for consistency
 * Integrates Firebase Cloud Messaging with admin API patterns
 */
...
export class ConsolidatedFCMClient extends UnifiedAdminClient {
  ...
  // ============================================================================
  // BACKEND INTEGRATION (using inherited UnifiedAdminClient methods)
  // ============================================================================
  ...
  private async registerAdminTokenWithBackend(token: string): Promise<any> {
    ...
    const response = await this.post('/admin/notifications/fcm/subscribe', subscriptionData);
    
    if (!response.success) {
      throw new Error(response.error || 'Failed to register admin FCM token with backend');
    }
  ...
  async unsubscribe(): Promise<{ success: boolean; backendSuccess: boolean }> {
    ...
    if (this.currentToken) {
      try {
        const response = await this.post('/admin/notifications/fcm/unsubscribe', {
  ...
  async sendTestNotification(notification: AdminFCMTestNotificationRequest): Promise<{ success: boolean; message: string }> {
    ...
    const response = await this.post('/admin/notifications/fcm/test', {
      ...notification,
      fcmToken: this.currentToken,
      isAdminTest: true
    });
    ...
    if (!response.success) {
      throw new Error(response.error || 'Failed to send admin test notification');
    }
  ...
  async getUserTokens(): Promise<AdminFCMTokenInfo[]> {
    const response = await this.get<{ tokens: AdminFCMTokenInfo[] }>('/admin/notifications/fcm/tokens/my');

    if (!response.success || !response.data) {
      throw new Error(response.error || 'Failed to get admin user tokens');
    }
...
export function createFCMClient(baseURL?: string, token?: string): ConsolidatedFCMClient {
  return new ConsolidatedFCMClient(baseURL, token, false);
}

export function createServerFCMClient(baseURL?: string, token?: string): ConsolidatedFCMClient {
  return new ConsolidatedFCMClient(baseURL, token, true);
}

// Export singleton instance
export const fcmClient = createFCMClient();

// Legacy compatibility exports
export const adminFCMClient = fcmClient;

// Export convenience functions
export const requestAdminFCMPermission = () => fcmClient.requestPermission();
export const subscribeToAdminFCM = () => fcmClient.subscribe();
export const unsubscribeFromAdminFCM = () => fcmClient.unsubscribe();
export const getAdminFCMSubscriptionStatus = () => fcmClient.getSubscriptionStatus();
