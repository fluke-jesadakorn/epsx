                ...
                <div className="bg-card rounded-3xl p-8 border border-border/50 shadow-sm relative overflow-hidden transition-all">
                    ...
                    <form onSubmit={handleSubmit} className="relative space-y-6">
                        <UserAndPlanFields
                            formData={formData}
                            plans={plans}
                            onUserChange={(v) => setFormData({ ...formData, user_id: v })}
                            onPlanChange={(v) => setFormData({ ...formData, plan_id: v })}
                        />

                        <AccessContextSelector
                            context={formData.access_context}
                            onChange={handleAccessContextChange}
                        />
                        ...
                        <SubscriptionSettings
                            ...
                            onExpiryChange={(v) => setFormData({ ...formData, expires_at: v })}
                            onAutoRenewChange={(v) => setFormData({ ...formData, auto_renew: v })}
                        />

                        <AdvancedSettings
                            metadata={formData.metadata}
                            onMetadataChange={(v) => setFormData({ ...formData, metadata: v })}
                        />
