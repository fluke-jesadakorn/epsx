...
export default function AuthPageClient({ serverHasSession }: AuthPageClientProps) {
    ...
    const { isAuthenticated, user, isLoading } = useSharedAuth();
    ...
    useEffect(() => {
        ...
        const checkAndRedirect = async () => {
            ...
            if (serverHasSession && isAuthenticated && user) {
                ...
                console.log('[AUTH] Server+Client authenticated, redirecting to', returnUrl);
    ...
    const handleAuthSuccess = async () => {
        try {
            ...
            const returnUrl = await getAndClearReturnUrlAction();
            console.log('[AUTH] handleAuthSuccess: redirecting to', returnUrl);
            router.replace(returnUrl);
        } catch (error) {
            console.error('[AUTH] handleAuthSuccess error:', error);
