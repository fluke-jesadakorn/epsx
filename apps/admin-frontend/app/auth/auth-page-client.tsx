        ...
        <div className="relative z-10 hidden w-full flex-col justify-center overflow-hidden p-8 text-foreground dark:text-foreground lg:flex lg:w-3/5 xl:p-20">
            <div className="mb-12 animate-auth-fade-in">
                ...
                <h1 className="text-5xl font-bold leading-tight tracking-tight xl:text-7xl">
                    Admin{' '}
                    <span className="animate-auth-gradient bg-gradient-to-r from-purple-400 via-orange-400 to-purple-500 bg-clip-text text-transparent">Control Center</span>
                </h1>
            ...
            <div className="grid max-w-2xl gap-8 sm:grid-cols-2">
                {features.map((f, i) => (
                    <div key={f.title} className="group flex gap-4 animate-auth-slide-up" style={{ animationDelay: `${i * 100}ms` }}>
...
function AuthRightPanel({ reason, isWagmiReady, showModal, setShowModal, handleAuthSuccess }: AuthRightPanelProps) {
    return (
        <div className="relative z-10 flex w-full items-center justify-center p-4 sm:p-6 lg:w-2/5 lg:border-l lg:border-border/20 lg:bg-white/[0.02] lg:backdrop-blur-3xl">
            <div className="w-full max-w-md">
                ...
                <div className="group relative animate-auth-slide-up overflow-hidden rounded-2xl border border-border/20 bg-white/90 dark:bg-background/60 p-6 shadow-[0_24px_48px_-12px_rgba(0,0,0,0.5)] sm:rounded-3xl sm:p-8 lg:p-10">
                    ...
                    <div className="relative">
                        ...
                        <div className="space-y-5 sm:space-y-6">
                            ...
                            <button type="button" onClick={() => { setShowModal(true); }} disabled={!isWagmiReady} className="flex w-full items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-purple-500 to-orange-500 py-5 text-lg font-bold text-white shadow-2xl shadow-purple-500/10 transition-all hover:from-purple-600 hover:to-orange-600 hover:shadow-purple-500/20 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed sm:rounded-2xl sm:py-6 sm:text-xl">
                                {!isWagmiReady ? (<><div className="h-5 w-5 animate-spin rounded-full border-2 border-white/30 border-t-white" />Loading...</>) : (<><Lock className="h-5 w-5" />Connect Wallet</>)}
                            </button>
                            <div className="border-t border-border/20 pt-5 sm:pt-6">
                                <div className="grid grid-cols-1 gap-3 sm:gap-4">
                                    {benefits.map((text) => (
                                        <div key={text} className="flex items-center gap-3 text-xs font-medium text-slate-400 sm:text-sm">
                                            <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-purple-500/10 ring-1 ring-purple-500/20"><CheckCircle className="h-3 w-3 text-purple-400" /></div>
                                            {text}
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="border-t border-border/20 pt-5 sm:pt-6 lg:hidden">
                                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                                    {features.map((f) => (
                                        <div key={f.title} className="flex flex-col items-center rounded-xl border border-border/20 bg-white/[0.02] p-3 text-center transition-colors hover:bg-white/[0.04] sm:p-4">
                                            <div className="mb-2 flex h-10 w-10 items-center justify-center rounded-lg bg-purple-500/10 ring-1 ring-purple-500/20"><f.icon className="h-5 w-5 text-purple-400" /></div>
                ...
                <AuthModal isOpen={showModal} onClose={() => { setShowModal(false); }} variant="admin" onSuccess={() => { void handleAuthSuccess(); }} />
                ...
                <div className="mt-4 text-center">
                    <Link href="/" className="text-xs text-slate-500 hover:text-purple-500 transition-colors underline underline-offset-2">Go to Dashboard</Link>
                </div>
            </div>
        </div>
    );
...
export default function AuthPageClient() {
    ...
    useEffect(() => {
        if (isWagmiReady && isConnected) { setShowModal(true); }
    }, [isWagmiReady, isConnected]);
    ...
    const handleAuthSuccess = async () => {
        try {
            ...
            logger.auth.success('handleAuthSuccess: redirecting', { returnUrl });
            router.replace(returnUrl);
        } catch (error) {
            logger.auth.error('handleAuthSuccess error', { error: error instanceof Error ? error.message : String(error) });
    ...
    return (
        <div className="fixed inset-0 z-50 flex w-full flex-col overflow-y-auto bg-background lg:flex-row">
            ...
            <AuthLeftPanel />
            <AuthRightPanel reason={reason} isWagmiReady={isWagmiReady} showModal={showModal} setShowModal={setShowModal} handleAuthSuccess={handleAuthSuccess} />
        </div>
    );
}
