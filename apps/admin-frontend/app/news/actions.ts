...
export async function deleteNewsAction(id: string): Promise<ApiResponse> {
  const res = await client().delete(`/api/admin/news/${id}`);
  if (res.success) { revalidatePath('/news'); }
  return res;
}

export async function publishNewsAction(id: string): Promise<ApiResponse> {
  const res = await client().put(`/api/admin/news/${id}/publish`, {});
  if (res.success) { revalidatePath('/news'); }
  return res;
}

export async function unpublishNewsAction(id: string): Promise<ApiResponse> {
...
export async function pinNewsAction(id: string): Promise<ApiResponse> {
  const res = await client().put(`/api/admin/news/${id}/pin`, {});
  if (res.success) { revalidatePath('/news'); }
  return res;
}

export async function unpinNewsAction(id: string): Promise<ApiResponse> {
