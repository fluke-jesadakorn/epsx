/**
 * Complete E2E Admin Authentication Flow Tests with Analytics Command Center Theme
...
test.describe('🎯 Analytics Command Center - Admin Authentication Complete Flow', () => {
  ...
  test('🎯 Complete Admin OIDC Authorization Flow', async () => {
    ...
    // Step 4: Verify Analytics Command Center theme
    const isAnalyticsPortal = await page.locator('h2:has-text("Analytics Command Center"), h1:has-text("Analytics Command Center")').first().isVisible({ timeout: 5000 }).catch(() => false);
    const isAnalyticsAdmin = await page.locator('h2:has-text("Data Insights Portal"), text=admin').first().isVisible({ timeout: 5000 }).catch(() => false);
    const isAnalyticsTheme = await page.locator('text=📊, text=🎯, .insight-logo, [class*="insight"]').first().isVisible({ timeout: 5000 }).catch(() => false);
    
    if (isAnalyticsPortal) {
      console.log('✅ Analytics Command Center theme detected');
      await expect(page.locator('text=🎯, text=📊')).toBeVisible();
    } else if (isAnalyticsAdmin || isAnalyticsTheme) {
      console.log('✅ Analytics Portal admin theme detected');
    ...
    expect(scopeValue).toContain('permissions');
...
console.log('🎯 Analytics Command Center Admin E2E Test Suite Loaded');