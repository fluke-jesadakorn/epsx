...
use crate::auth::unified_web3_auth_service::{
    UnifiedWeb3AuthService, Web3VerificationRequest, Web3AuthChallenge,
};
...
mod tests {
    ...
    impl Web3AuthTestSuite {
        ...
        /// Create test Web3 auth context with permissions
        fn create_test_context(&self, permissions: Vec<String>) -> Web3AuthContext {
            Web3AuthContext {
                wallet_address: self.wallet_address.clone(),
                permissions,
                tier_level: "Gold".to_string(),
                is_active: true,
                verified_at: Utc::now(),
    ...
    async fn test_valid_wallet_authentication() {
        ...
        // Create valid Web3 auth context with admin permissions
        let context = suite.create_test_context(vec![
    ...
    async fn test_expired_signature_rejection() {
        ...
        let timestamp_str = chrono::DateTime::from_timestamp(expires_at, 0)
            .unwrap().format("%Y-%m-%dT%H:%M:%S%.3fZ").to_string();
    ...
    async fn test_insufficient_web3_permissions() {
        ...
        let context = suite.create_test_context(vec!["epsx:analytics:read".to_string()]);
    ...
    async fn test_web3_temporal_permission_expiry() {
        ...
        assert!(temporal_permission.contains(&(Utc::now() - Duration::minutes(1)).timestamp().to_string()));
    }
    ...
    async fn test_web3_signature_performance() {
        ...
        let context = suite.create_test_context(vec!["admin:users:read".to_string()]);
        ...
        assert!(elapsed.as_millis() < 100, "Web3 validation too slow: {:?}", elapsed);
