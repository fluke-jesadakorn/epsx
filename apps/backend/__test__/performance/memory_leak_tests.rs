...
async fn test_memory_leak_detection() {
    ...
    while start_time.elapsed() < duration {
        ...
        let _ = cache.set(&cache_key, &cache_value, Some(Duration::from_secs(10))).await;
        ...
        if iteration % 1000 == 0 {
            ...
            assert!(memory_growth < 3.0, 
                ...
                memory_growth, initial_memory, current_memory);
        }
...
async fn test_connection_pool_resource_cleanup() {
    ...
    for round in 0..iterations {
        ...
        for i in 0..concurrent_operations {
            ...
            handles.push(tokio::spawn(async move {
                ...
                let _ = user_repo.find_by_id(&user_id).await;
                let _ = permission_service.get_user_permissions(&user_id).await;
                let _ = permission_service.check_permission(&user_id, "READ_PROFILE").await;
            }));
        }
        ...
        if round % 20 == 0 {
            let current_memory = get_memory_usage();
            println!("Memory after {} rounds of connection pool tests: {} MB", round, current_memory);
            
            // Connection pool should not cause excessive memory growth
            let memory_growth = current_memory as f64 / initial_memory as f64;
            assert!(memory_growth < 2.0, 
                "Connection pool caused excessive memory growth: {}x", memory_growth);
        }
    ...
    let memory_growth = final_memory as f64 / initial_memory as f64;
    assert!(memory_growth < 1.8, 
        "Connection pool leaked memory: {}x growth", memory_growth);
}
...
async fn test_cache_memory_management() {
    ...
    for i in 0..cache_entries {
        ...
        cache.set(&key, &large_value, Some(Duration::from_minutes(5))).await.unwrap();
        cache_keys.push(key);
    ...
    println!("Peak memory with {} cache entries: {} MB", cache_entries, peak_memory);
    ...
    assert!(memory_retention < 1.5, 
        "Cache cleanup didn't free enough memory: {}x retention", memory_retention);
}
...
async fn test_concurrent_memory_allocation() {
    ...
    for task_id in 0..concurrent_tasks {
        ...
        handles.push(tokio::spawn(async move {
            for allocation_id in 0..allocations_per_task {
                let user_id = UserId::new(format!("concurrent-{}-{}", task_id, allocation_id));
                ...
                let large_data = (0..500).map(|i| serde_json::json!({
                    "id": i,
                    "task": task_id,
                ...
                let _ = cache.set(&cache_key, &serde_json::Value::Array(large_data), 
    ...
    let memory_growth = final_memory as f64 / initial_memory as f64;
    assert!(memory_growth < 2.0, 
        "Concurrent allocations caused excessive memory growth: {}x", memory_growth);
    
    assert_eq!(total_allocations, concurrent_tasks * allocations_per_task,
...
async fn test_session_memory_lifecycle() {
    ...
    for i in 0..session_count {
        ...
        cache.set(&session_key, &session_value, Some(Duration::from_minutes(30))).await.unwrap();
        session_data.push((session_id, session_key));
    ...
    for (_, session_key) in &session_data {
        let session: Option<serde_json::Value> = cache.get(session_key).await.unwrap();
        if session.is_some() {
            valid_sessions += 1;
        }
    }
    ...
    assert!(valid_sessions > session_count * 95 / 100, 
        "Too many sessions were lost: {}/{}", valid_sessions, session_count);
    ...
    let memory_retention = final_memory as f64 / initial_memory as f64;
    assert!(memory_retention < 1.6, 
        "Session cleanup didn't free enough memory: {}x retention", memory_retention);
}
...
async fn test_resource_handle_cleanup() {
    ...
    for iteration in 0..iterations {
        ...
        for i in 0..resources_per_iteration {
            ...
            handles.push(tokio::spawn(async move {
                ...
                let user_id = UserId::new(format!("resource-user-{}-{}", iteration, i));
                ...
                let _ = cache.set(&cache_key, &cache_data, Some(Duration::from_secs(5))).await;
        ...
        // Verify shared data is cleaned up
        {
            ...
            assert!(guard.is_empty(), "Shared data not cleaned up properly in iteration {}", iteration);
        }
        
        // Check memory periodically
        if iteration % 50 == 0 {
            let current_memory = get_memory_usage();
            println!("Memory after {} resource iterations: {} MB", iteration, current_memory);
            
            let memory_growth = current_memory as f64 / initial_memory as f64;
            assert!(memory_growth < 2.5, 
                "Resource handles caused excessive memory growth: {}x", memory_growth);
        }
    ...
    let memory_growth = final_memory as f64 / initial_memory as f64;
    assert!(memory_growth < 2.0, 
        "Resource handle cleanup failed: {}x memory growth", memory_growth);
}
...
async fn test_sustained_load_memory_stability() {
    ...
    while start_time.elapsed() < duration {
        ...
        match operation_type {
            0 => {
                // User operations
                let user_id = UserId::new(format!("sustained-user-{}", operation_count));
                let user_repo = container.user_repo();
                let _ = user_repo.find_by_id(&user_id).await;
            }
            1 => {
                // Permission operations
                let user_id = UserId::new(format!("sustained-perm-{}", operation_count));
                let permission_service = container.permission_service();
                let _ = permission_service.get_user_permissions(&user_id).await;
            }
            2 => {
            ...
            3 => {
                // Mixed operations
                let user_id = UserId::new(format!("sustained-mixed-{}", operation_count));
                let cache = container.cache();
                let permission_service = container.permission_service();
                
                let cache_key = format!("mixed:{}", operation_count);
                let _ = cache.set(&cache_key, &operation_count, Some(Duration::from_secs(10))).await;
                let _ = permission_service.check_permission(&user_id, "READ_PROFILE").await;
                let _ = cache.delete(&cache_key).await;
            }
            _ => unreachable!()
