...
pub trait StockDataSvc: Send + Sync {
  async fn get_real_time_price(
    &self,
    symbol: &Symbol
  ) -> Result<StockPrice, StockServiceError>;
  async fn get_historical_data(
    &self,
    symbol: &Symbol,
    period: &str
  ) -> Result<Vec<StockPrice>, StockServiceError>;
  async fn get_market_status(
    &self,
    market: &str
  ) -> Result<MarketStatus, StockServiceError>;
  async fn search_symbols(
    &self,
    query: &str
  ) -> Result<Vec<SymbolInfo>, StockServiceError>;
}
...
pub trait WebSocketSvc: Send + Sync {
  ...
  async fn broadcast_to_role(
    &self,
    role: &str,
    message: &str
  ) -> Result<(), WebSocketError>;
  async fn broadcast_stock_update(
    &self,
    symbol: &Symbol,
    price: Decimal
  ) -> Result<(), WebSocketError>;
...
pub enum AuthServiceError {
  ...
  #[error("External service error: {0}")] ExternalError(String),
...
pub enum EmailServiceError {
  #[error("Invalid email address: {0}")] InvalidEmail(String),

  #[error("Email delivery failed: {0}")] DeliveryFailed(String),

  #[error("Email template error: {0}")] EmailTemplateError(String),
...
pub enum PaymentServiceError {
  #[error("Invalid currency: {0}")] InvalidCurrency(String),

  #[error("Address generation failed: {0}")] AddressGenerationFailed(String),
...
pub enum WebSocketError {
  #[error("User not connected: {0}")] UserNotConnected(String),
  ...
  #[error("Broadcast failed: {0}")] BroadcastFailed(String),
}
...
pub enum NotificationServiceError {
  ...
  #[error("Invalid notification data: {0}")] InvalidData(String),
  ...
  #[error("Update error: {0}")] UpdateError(String),

  #[error("Delete error: {0}")] DeleteError(String),
  ...
  #[error("Feature not implemented: {0}")] NotImplemented(String),
...
mod tests {
  ...
  fn should_create_stock_price() {
    let price = StockPrice {
      ...
      price: rust_decimal_macros::dec!(150.5),
      volume: 1000000,
      timestamp: Utc::now(),
      change: Some(rust_decimal_macros::dec!(2.5)),
      change_percent: Some(rust_decimal_macros::dec!(1.69)),
    };

    assert_eq!(price.price, rust_decimal_macros::dec!(150.5));
