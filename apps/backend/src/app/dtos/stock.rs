...
impl GetStockReq {
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        Symbol::new(&self.sym)
            .map_err(|_| ValidationError::InvalidSymbol(self.sym.clone()))?;
...
impl GetStocksReq {
    pub fn validate(&self) -> Result<(), ValidationError> {
        ...
        for sym in &self.syms {
            Symbol::new(sym)
                .map_err(|_| ValidationError::InvalidSymbol(sym.clone()))?;
        }
...
pub enum ValidationError {
    ...
    #[error("Too many symbols: {0} (max 100 for batch, 50 for subscription)")]
    ...
    #[error("Invalid mover type: {0} (valid: gainers, losers, volume)")]
    InvalidMoverType(String),
    
    #[error("Invalid period: {0} (valid: 1d, 1w, 1m, 3m, 6m, 1y, 2y, 5y)")]
...
mod tests {
    ...
    #[test]
    fn should_create_stock_dto_from_entity() {
        let stock = Stock::new(
        ...
        let dto = StockDto::from_entity(&stock);
        
        assert_eq!(dto.sym, "AAPL");
        assert_eq!(dto.px, dec!(150.50));
        assert_eq!(dto.vol, 1000000);
        assert_eq!(dto.market, "NASDAQ");
    }
    
    #[test]
    fn should_calculate_price_change() {
        let stock = Stock::new(
            Symbol::new("AAPL").unwrap(),
            dec!(150.50),
            1000000,
        ...
        let dto = StockDto::from_entity(&stock)
            .with_change(Some(dec!(148.00)));
        
        assert_eq!(dto.change, Some(dec!(2.50)));
        assert!(dto.change_pct.unwrap() > dec!(1.6) && dto.change_pct.unwrap() < dec!(1.7));
    }
    
    #[test]
    fn should_validate_get_stock_request() {
        let valid_req = GetStockReq {
            sym: "AAPL".to_string(),
        };
        
        assert!(valid_req.validate().is_ok());
        
        let invalid_req = GetStockReq {
            sym: "".to_string(),
        };
    ...
    #[test]
    fn should_validate_search_request() {
        let valid_req = SearchStocksReq {
        ...
        let invalid_req = SearchStocksReq {
            query: "".to_string(),
            limit: Some(10),
        };
