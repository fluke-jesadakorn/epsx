...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UpdatePackageTierRes {
  pub usr: UserDto,
  pub event_id: String,
}
...
pub struct ListUsersReq {
  pub offset: u32,
  pub limit: u32,
  pub package_tier_filter: Option<String>,
...
pub struct UserStatsReq {
  pub include_package_tiers: bool,
...
pub enum ValidationError {
  #[error("Field cannot be empty: {0}")] EmptyField(String),
  ...
  #[error("Invalid subscription tier: {0}")] InvalidSubscriptionTier(String),

  #[error("Bulk update too large: {0} updates (max 100)")] BulkUpdateTooLarge(
    usize,
  ),
  ...
  #[error("Reason too long: {0} characters (max 500)")] ReasonTooLong(usize),
}
