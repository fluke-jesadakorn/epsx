...
use crate::application::payment::commands::handlers::ActivateSubscriptionHandler;
...
pub async fn confirm_payment(
    ...
    info!(
        ...
        user_id, request.plan_id, request.transaction_hash
    );

    // Validate transaction hash format
    if request.transaction_hash.len() != 66 || !request.transaction_hash.starts_with("0x") {
        warn!("Invalid transaction hash format: {}", request.transaction_hash);
        return Ok(ResponseJson(ConfirmPaymentResponse {
            success: false,
    ...
    let transaction_hash = match TransactionHash::new(
    ...
    let command = ActivateSubscriptionCommand::new(
        payment_id.clone(),
        user_id.clone(),
        request.plan_id,
        transaction_hash,
    ...
    Ok(ResponseJson(ConfirmPaymentResponse {
        success: true,
...
pub async fn get_subscription_status(
    State(container): State<Arc<AppContainer>>,
) -> Result<ResponseJson<SubscriptionStatusResponse>, StatusCode> {
    // TODO: Extract user_id from auth middleware in the future
    let user_id = UserId::new(); // This should come from auth middleware
    ...
    let user_result = sqlx::query!(
        "SELECT package_tier FROM users WHERE id = $1",
        user_uuid
    )
    ...
    match user_result {
        Ok(Some(user)) => {
            let package_tier = user.package_tier;
            ...
            let permissions = permissions_result
                .unwrap_or_default()
                .into_iter()
                .map(|row| row.permission)
                .collect();

            Ok(ResponseJson(SubscriptionStatusResponse {
                has_subscription,
