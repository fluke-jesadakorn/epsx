...
use crate::auth::{
    UnifiedAuthService, AuthMethod, UnifiedVerifyRequest, VerificationData,
    MigrationRequest, UserProfile, UnifiedAuthContext
};
...
pub struct ChallengeRequest {
    pub method: String, // "web3" or "firebase"
    pub wallet_address: Option<String>, // Required for web3
    pub redirect_uri: Option<String>, // Required for firebase
}
...
/// Challenge data that varies by auth method
#[derive(Debug, Serialize)]
#[serde(untagged)]
...
pub async fn generate_challenge(
    ...
    info!("Generating unified authentication challenge for method: {}", request.method);
    ...
    let challenge = match request.method.as_str() {
        "web3" => {
            let wallet_address = request.wallet_address
                .ok_or_else(|| AppError::bad_request("wallet_address is required for Web3 authentication"))?;
            
            unified_auth.generate_web3_challenge(&wallet_address).await
                .map_err(|e| {
                    error!("Failed to generate Web3 challenge: {}", e);
                    match e.to_string().as_str() {
                        s if s.contains("Invalid wallet address") => AppError::bad_request("Invalid wallet address format"),
        ...
        "firebase" => {
            let redirect_uri = request.redirect_uri
                .ok_or_else(|| AppError::bad_request("redirect_uri is required for Firebase authentication"))?;
            
            unified_auth.generate_firebase_challenge(&redirect_uri).await
                .map_err(|e| {
                    error!("Failed to generate Firebase challenge: {}", e);
                    AppError::internal_server_error("Failed to generate challenge")
                })?
        }
        _ => {
            return Err(AppError::bad_request("Invalid authentication method. Use 'web3' or 'firebase'"));
        }
    };
    ...
    let response_data = match &challenge.data {
        ...
        crate::auth::AuthChallengeData::Firebase { redirect_uri, state } => {
            // Generate OAuth authorize URL for Firebase
            let authorize_url = format!(
                "https://accounts.google.com/oauth2/v2/auth?client_id={}&redirect_uri={}&response_type=code&scope=openid%20profile%20email&state={}",
                "your-client-id", // TODO: Get from environment
                redirect_uri,
                state
            );
            
            ChallengeData::Firebase {
...
pub async fn verify_authentication(
    ...
    let verify_data = match request.data {
        ...
        VerifyData::Firebase { id_token, access_token } => {
            VerificationData::Firebase { id_token, access_token }
        }
    };
    ...
    let unified_request = UnifiedVerifyRequest {
        ...
        method: match request.method.as_str() {
            "web3" => AuthMethod::Web3Wallet { wallet_address: "pending".to_string() },
            "firebase" => AuthMethod::Firebase { firebase_uid: "pending".to_string() },
            _ => return Err(AppError::bad_request("Invalid authentication method")),
        },
        data: verify_data,
    };

    let auth_result = unified_auth.verify_authentication(unified_request).await
        .map_err(|e| {
            error!("Authentication verification failed: {}", e);
            match e.to_string().as_str() {
                s if s.contains("Invalid signature") => AppError::unauthorized("Invalid signature"),
                s if s.contains("Invalid token") => AppError::unauthorized("Invalid token"),
    ...
    let response = AuthResponse {
        ...
        firebase_uid: auth_result.firebase_uid,
    };
...
pub async fn get_profile(
    ...
    let profile = unified_auth.get_user_profile(token).await
        .map_err(|e| {
            error!("Failed to get user profile: {}", e);
            match e.to_string().as_str() {
                s if s.contains("Invalid token") => AppError::unauthorized("Invalid or expired token"),
    ...
    let auth_method_strings: Vec<String> = profile.auth_methods
        ...
        .map(|method| match method {
            ...
            AuthMethod::Firebase { .. } => "firebase".to_string(),
            AuthMethod::Hybrid { .. } => "hybrid".to_string(),
        })
        .collect();
...
/// Migrate Firebase user to Web3
/// POST /api/auth/unified/migrate
pub async fn migrate_user(
    State(container): State<AppContainer>,
    Json(request): Json<MigrationRequest>,
) -> ApiResult<Json<MigrationResponse>> {
    info!("Migrating Firebase user {} to Web3 wallet {}", 
          request.firebase_uid, request.wallet_address);

    let unified_auth = container.unified_auth_service();

    let auth_result = unified_auth.migrate_firebase_to_web3(request.clone()).await
        .map_err(|e| {
            error!("Migration failed: {}", e);
            match e.to_string().as_str() {
                s if s.contains("Invalid wallet signature") => AppError::unauthorized("Invalid wallet signature"),
                s if s.contains("User not found") => AppError::not_found("Firebase user not found"),
                _ => AppError::internal_server_error("Migration failed"),
            }
        })?;

    let response = MigrationResponse {
        success: true,
        message: "Successfully migrated Firebase user to Web3 wallet".to_string(),
        user_id: auth_result.user_id.to_string(),
    ...
    info!("Migration successful for user: {}", response.user_id);
    Ok(Json(response))
}
...
pub async fn logout(
    ...
    let auth_context = unified_auth.validate_bearer_token(token).await
        .map_err(|e| {
    ...
    unified_auth.revoke_authentication(&auth_context.user_id).await
        .map_err(|e| {
...
mod tests {
    ...
    fn test_verify_data_deserialization() {
        let json = r#"{"message": "test", "signature": "sig", "wallet_address": "0x123"}"#;
