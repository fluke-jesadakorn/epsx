...
pub async fn generate_challenge(
  ...
  let challenge = web3_auth
    .generate_challenge(&request.wallet_address).await
    .map_err(|e| {
      error!("Failed to generate challenge: {}", e);
      match e.to_string().as_str() {
        s if s.contains("Invalid wallet address") =>
          ApiError::bad_request("Invalid wallet address format"),
...
pub async fn verify_signature(
  ...
  let permissions = web3_permissions
    .get_wallet_permissions(&request.wallet_address).await
    .map_err(|e| {
  ...
  let _automatic_grants = web3_permissions
    .process_automatic_permissions(&request.wallet_address).await
    .unwrap_or_default();
  ...
  let access_token = jwt_service.create(user_data).map_err(|e| {
  ...
  web3_auth
    ...
    .map_err(|e| {
      ...
      match e.to_string().as_str() {
        s if s.contains("Invalid wallet address") =>
          ApiError::bad_request("Invalid wallet address format"),
        s if s.contains("already linked") =>
          ApiError::conflict("Wallet already linked"),
        _ => ApiError::internal_server_error("Failed to link wallet"),
      }
    })?;

  info!(
    "Successfully linked wallet {} to user {}",
    request.wallet_address,
    request.user_id
  );
...
pub async fn process_automatic_permissions(
  ...
  let automatic_grants = web3_permissions
    .process_automatic_permissions(wallet_address).await
    .map_err(|e| {
