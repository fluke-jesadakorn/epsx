...
pub fn transform_ranking_to_unified_format(
    ...
    UnifiedRankingItem {
        ...
        next_earnings_date: ranking.next_earnings_date.as_ref().and_then(|date_str| {
            chrono::NaiveDate::parse_from_str(date_str, "%Y-%m-%d")
                .ok()
                .and_then(|date| date.and_hms_opt(0, 0, 0))
                .map(|dt| dt.and_utc().timestamp())
        }),
        last_earnings_date: ranking.last_earnings_date.as_ref().and_then(|date_str| {
            chrono::NaiveDate::parse_from_str(date_str, "%Y-%m-%d")
                .ok()
                .and_then(|date| date.and_hms_opt(0, 0, 0))
                .map(|dt| dt.and_utc().timestamp())
...
pub fn transform_unified_to_card_format(unified_item: &UnifiedRankingItem) -> SymbolCardData {
    ...
    let quarterly_performance: Vec<QuarterlyPerformanceData> = unified_item
        .quarterly_data
        .iter()
        .map(|q| {
