...
pub async fn enhance_with_websocket_data(
    ...
    info!(
        "Starting optimized TradingView WebSocket enhancement for {} symbols",
        symbols.len()
    );
    ...
    if symbols.len() > 15 {
        warn!(
            "Too many symbols for WebSocket enhancement: {}, limiting to 15",
            symbols.len()
        );
...
async fn enhance_symbols_batch(
    ...
    let config = crate::infrastructure::adapters::tradingview_types::TradingViewConfig::from(
        &get_fallback_config(),
    );
    ...
    info!(
        "Fetched WebSocket data for {} symbols",
        websocket_data.len()
    );
    ...
    for ranking in rankings.iter_mut() {
        if let Some(ws_data) = websocket_map.get(&ranking.symbol) {
            ...
            if ws_data.current_eps > 0.0
                && is_valid_eps_for_ranking(ws_data.current_eps)
                && (ranking.current_eps.is_none()
                    || ranking.current_eps.unwrap() != ws_data.current_eps)
            {
                debug!(
                    "Updating {} EPS: {:?} → {}",
            ...
            if ws_data.qoq_growth.abs() > 0.1
                && (ranking.growth_factor.is_none()
                    || (ranking.growth_factor.unwrap() - ws_data.qoq_growth).abs() > 1.0)
            {
                debug!(
                    "Updating {} growth: {:?} → {}%",
            ...
            if ws_data.price_current > 0.0
                && (ranking.price_current.is_none()
                    || (ranking.price_current.unwrap() - ws_data.price_current).abs() > 0.01)
            {
                debug!(
                    "Updating {} price: {:?} → ${:.2}",
            ...
            if ws_data.next_earnings_date.is_some()
                && ws_data.next_earnings_date != ranking.next_earnings_date
            {
                info!(
                    "Updating {} next_earnings_date: {:?} → {:?}",
            ...
            if ws_data.last_earnings_date.is_some()
                && ws_data.last_earnings_date != ranking.last_earnings_date
            {
                info!(
                    "Updating {} last_earnings_date: {:?} → {:?}",
    ...
    info!(
        "Enhanced {} ranking fields with real-time WebSocket data",
        enhanced_count
    );
    Ok(enhanced_count)
}
