...
async fn fetch_realtime_estimate_async(unified_item: &UnifiedRankingItem) -> Option<EstimateData> {
    let fallback_config = super::system::get_simple_fallback_config();
    let tradingview_config =
        crate::infrastructure::adapters::tradingview_types::TradingViewConfig::from(
            &fallback_config,
        );
    let websocket_handler =
        crate::infrastructure::adapters::services::tradingview::TradingViewWebSocketHandler::new(
            tradingview_config,
        );

    let enhanced_data_list = websocket_handler
        .fetch_enhanced_eps_data(vec![unified_item.symbol.clone()])
        .await
        .ok()?;
...
pub(super) fn generate_next_quarter_estimate(
    ...
    if let Some(estimate) =
        extract_tradingview_estimate(unified_item, quarterly_performance, current_date)
    {
    ...
    info!(
        "[{}] No TradingView timestamp - trying WebSocket",
        unified_item.symbol
    );
    ...
    let realtime_estimate = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current()
            .block_on(async { fetch_realtime_estimate_async(unified_item).await })
    });
