...
pub(super) fn calculate_system_mode(quarterly_performance: &[QuarterlyPerformanceData]) -> String {
    if quarterly_performance.is_empty() {
        return "STOP".to_string();
    }

    let recent_quarters: Vec<&QuarterlyPerformanceData> =
        quarterly_performance.iter().take(3).collect();
    ...
    match recent_quarters.len() {
        ...
        2 => {
            if positive_count >= 2
                || eps_positive_count >= 2
                || (eps_positive_count == 1 && recent_quarters[0].eps_growth > 0.0)
            {
                "TRACK".to_string()
            } else {
                "STOP".to_string()
