...
use crate::infrastructure::adapters::services::tradingview_websocket::{TradingViewWebSocketService, QuarterlyEPSData, EPSWebSocketData};
...
impl WebSocketEarningsService {
    ...
    pub async fn fetch_earnings_dates(symbols: Vec<String>) -> Result<HashMap<String, (i64, i32)>, AppError> {
        ...
        // Fetch with timeout
        let websocket_data = Self::fetch_with_timeout(symbols_to_fetch.clone()).await?;
        ...
        for ws_data in websocket_data {
            if let Some(next_earnings) = Self::find_next_earnings_announcement(&ws_data.quarterly_data, current_timestamp) {
                ...
                tracing::info!("✅ Real QoQ pattern earnings for {}: {} days", ws_data.symbol, days_until);
            } else {
                Self::handle_missing_quarterly_data(&ws_data.symbol, &ws_data.quarterly_data, current_timestamp, &mut result_map);
            }
        }
    ...
    fn handle_missing_quarterly_data(
        ...
        if let Some(last_quarter) = quarterly_data.first() {
            ...
            if estimated_next > current_timestamp {
                ...
                tracing::info!("✅ Estimated from last quarter for {}: {} days", symbol, days_until);
                return;
            }
        }
    ...
    fn add_cached_results(symbols: &[String], result_map: &mut HashMap<String, (i64, i32)>) {
        for symbol in symbols {
    ...
    fn find_next_earnings_announcement(quarterly_data: &[QuarterlyEPSData], current_timestamp: i64) -> Option<i64> {
        ...
        for quarter in quarterly_data {
            if let Some(announcement_date) = quarter.estimated_earnings_date {
                if announcement_date > current_timestamp {
                    tracing::info!("🎯 Found future announcement: {} days from now",
        ...
        if let Some(estimated) = Self::calculate_from_intervals(quarterly_data, current_timestamp) {
    ...
    fn calculate_from_intervals(quarterly_data: &[QuarterlyEPSData], current_timestamp: i64) -> Option<i64> {
        ...
        for i in 0..max_lookback {
            let interval = quarterly_data[i].timestamp - quarterly_data[i + 1].timestamp;
        ...
        if estimated_next > current_timestamp {
            ...
            tracing::info!("✅ QoQ pattern: {} days median interval → {} days until next",
                          median_interval / 86400, days_until);
            Some(estimated_next)
        } else {
            None
        }
    ...
    pub async fn fetch_qoq_data(symbols: Vec<String>) -> Result<HashMap<String, f64>, AppError> {
        let current_time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        let mut result_map = HashMap::new();
        let mut symbols_to_fetch = Vec::new();
        ...
        tracing::info!("🔌 Fetching {} QoQ symbols via WebSocket", symbols_to_fetch.len());
