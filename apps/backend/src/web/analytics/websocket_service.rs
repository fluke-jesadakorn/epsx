...
use crate::infrastructure::adapters::services::tradingview_websocket::{
    EPSWebSocketData, QuarterlyEPSData, TradingViewWebSocketService,
};
...
impl WebSocketEarningsService {
    ...
    pub async fn fetch_earnings_dates(
        ...
        for ws_data in websocket_data {
            if let Some(next_earnings) =
                Self::find_next_earnings_announcement(&ws_data.quarterly_data, current_timestamp)
            {
                ...
                // Update cache
                {
                    ...
                    cache.data.insert(
                        ws_data.symbol.clone(),
                        (next_earnings, days_until, current_time),
                    );
                }
    ...
    async fn fetch_with_timeout(symbols: Vec<String>) -> Result<Vec<EPSWebSocketData>, AppError> {
        let fetch_future = async {
            ...
            websocket_service
                .connect_and_fetch_eps_data(symbols.clone())
                .await
        };
    ...
    pub async fn fetch_qoq_data(symbols: Vec<String>) -> Result<HashMap<String, f64>, AppError> {
        ...
        for ws_data in websocket_data {
            if let Some(qoq_growth) = Self::calculate_real_qoq_growth(&ws_data.quarterly_data) {
                ...
                cache
                    .qoq_data
                    .insert(ws_data.symbol.clone(), (qoq_growth, current_time));
