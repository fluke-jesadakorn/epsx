...
pub async fn send_notification(
  ...
  ): Extension<Arc<NotificationRepositoryAdapter>>,
  Extension(auth_user): Extension<AuthenticatedUser>,
  ...
  info!(
    "Admin {} sending notification via DDD: {}",
    auth_user.user_id,
    request.title
  );
  ...
  let ddd_notification =
    NotificationMapper::create_ddd_notification_from_legacy(
  ...
  if let Some(topic_name) = &request.fcm_topic_id {
    ...
    match delivery_result {
      Ok(delivery_result) => {
        match delivery_result {
          ...
          crate::domain::notification::aggregates::notification::DeliveryResult::Failed {
            error_message,
            ..
          } => {
    ...
    let delivery_results = notification_adapter
      .deliver_notification_to_user(
        &ddd_notification,
        user_id,
        fcm_token,
        email
    ...
    if successful_deliveries > 0 {
      info!(
        "DDD user notification sent successfully to {} via {} channels",
        user_id,
...
pub async fn broadcast_to_topic(
  ...
  info!(
    "Admin {} broadcasting to topic via DDD: {}",
    auth_user.user_id,
    request.topic
  );
      ...
      match delivery_result {
        crate::domain::notification::aggregates::notification::DeliveryResult::Success {
          ...
          let response = BroadcastResponse {
            message_id: message_id.unwrap_or_else(||
        ...
        crate::domain::notification::aggregates::notification::DeliveryResult::Failed {
          ...
          warn!(
            "DDD topic broadcast failed for {}: {}",
            request.topic,
            error_message
          );
          Err(AppError {
...
pub async fn track_notification(Json(
  request,
): Json<TrackNotificationRequest>) -> Result<impl IntoResponse, AppError> {
  info!(
...
pub async fn get_unread_notifications(
  ...
  match get_result {
    ...
    Err(e) => {
      warn!(
        "Failed to fetch unread notifications for user {}: {}",
        auth_user.user_id,
        e
      );
      Err(AppError {
...
pub async fn get_preferences(Extension(
  auth_user,
): Extension<AuthenticatedUser>) -> Result<impl IntoResponse, AppError> {
