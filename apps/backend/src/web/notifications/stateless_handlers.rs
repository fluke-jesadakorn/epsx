...
pub async fn list_notifications(
  ...
  let user_id = Uuid::parse_str(user_id).map_err(|_|
    ApiError::bad_request("Invalid user_id format")
  )?;
  ...
  if let Some(_notification_type) = &query.notification_type {
    param_count += 1;
    where_conditions.push(format!("n.notification_type = ${}", param_count));
  }

  if let Some(_priority) = &query.priority {
    param_count += 1;
...
pub async fn get_notification(
  ...
  let notification = sqlx
    ...
    .bind(notification_id)
    .fetch_optional(db_pool.as_ref()).await
    .map_err(|e| {
...
pub async fn update_notification(
  ...
  if let Some(is_read) = request.is_read {
    ...
    if is_read {
      updates.push(
        format!("is_read = ${}, read_at = CURRENT_TIMESTAMP", param_count)
      );
    } else {
  ...
  if let Some(is_archived) = request.is_archived {
    ...
    if is_archived {
      updates.push(
        format!("is_archived = ${}, archived_at = CURRENT_TIMESTAMP", param_count)
      );
    } else {
      updates.push(
        format!("is_archived = ${}, archived_at = NULL", param_count)
      );
    }
  }
...
pub async fn get_notification_count(
  ...
  let response = NotificationCountResponse {
    total: counts.try_get::<i64, _>("total").unwrap_or(0),
    unread: counts.try_get::<i64, _>("unread").unwrap_or(0),
    urgent: counts.try_get::<i64, _>("urgent").unwrap_or(0),
    by_category,
    by_priority,
  };
...
pub async fn get_preferences(
  ...
  let preferences = sqlx
    ::query_as::<_, NotificationPreferences>(
...
pub async fn cleanup_notifications(State(
  container,
): State<AppContainer>) -> ApiResult<Json<serde_json::Value>> {
  let db_pool = container.db_pool();

  let result: Option<i32> = sqlx
    ::query_scalar("SELECT cleanup_notifications()")
    .fetch_one(db_pool.as_ref()).await
    .map_err(|e| {
