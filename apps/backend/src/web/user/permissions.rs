...
pub async fn get_user_permissions(
    ...
    info!(
        "Getting permission status for user {}",
    ...
    for (permission, claim) in &permissions_map {
        let is_expired = claim
            .expires_at
            .is_some_and(|exp| DateTime::from_timestamp(exp, 0).is_some_and(|exp_dt| exp_dt < now));
        ...
        permission_infos.push(UserPermissionInfo {
            ...
            granted_at: DateTime::from_timestamp(claim.granted_at, 0).unwrap_or_else(Utc::now),
            is_active,
    ...
    let has_admin_access =
        epsx_contracts::permissions::has_admin_platform_permission(&user_context.permissions);
