...
impl PermissionError {
    ...
    pub fn to_error_response(&self) -> PermissionErrorResponse {
        ...
        match self {
            PermissionError::AuthenticationRequired { message, code } => PermissionErrorResponse {
            ...
            PermissionError::PermissionExpired {
                ...
                user_message:
                    "Your permission has expired. Please renew your subscription to continue."
                        .to_string(),
                timestamp,
                error_id,
                context: HashMap::new(),
            ...
            PermissionError::UsageLimitExceeded {
                permission,
                current_usage,
                limit,
                period,
            ...
            PermissionError::InsufficientPlan {
                current_plan,
                required_plan,
                upgrade_url,
                benefits,
                ...
                status_code: 403,
                message: format!(
                    "Insufficient permission plan: {} required, you have {}",
                ...
                user_message: format!(
                    "This feature requires {} permission plan. Upgrade your plan to access it.",
                    required_plan
                ),
                timestamp,
                error_id,
            ...
            PermissionError::SecurityRestriction {
                reason,
                risk_level,
                contact_support,
            } => PermissionErrorResponse {
                ...
                details: PermissionErrorDetails {
                    ...
                    security_info: Some(SecurityInfo {
                        ...
                        contact_support_url: if *contact_support {
                ...
                context: HashMap::from([(
                    ...
                    serde_json::Value::String(format!("{:?}", risk_level)),
                )]),
            },

            PermissionError::SystemError {
                error_id: sys_error_id,
                retry_after,
            } => PermissionErrorResponse {
                ...
                context: HashMap::from([(
                    ...
                    serde_json::Value::Number(serde_json::Number::from(retry_after.unwrap_or(30))),
                )]),
            },

            // Handle other error types similarly...
            _ => PermissionErrorResponse {
    ...
    pub fn to_error_response_with_context(
        &self,
        ctx: &PermissionErrorContext,
    ) -> PermissionErrorResponse {
...
impl IntoResponse for PermissionError {
    fn into_response(self) -> Response {
        ...
        let status_code = StatusCode::from_u16(error_response.status_code)
