...
use crate::auth::{jwt::Service as JwtService, web3_permission_service::Web3PermissionService};
...
pub async fn web3_auth_middleware(
    ...
    let web3_context = Web3AuthContext {
        ...
        token_claims: {
            ...
            map.insert("sub".to_string(), serde_json::Value::String(claims.sub.clone()));
            map.insert("iss".to_string(), serde_json::Value::String(claims.iss.clone()));
            map.insert("aud".to_string(), serde_json::Value::String(claims.aud.clone()));
            map.insert("exp".to_string(), serde_json::Value::Number(serde_json::Number::from(claims.exp)));
            map.insert("iat".to_string(), serde_json::Value::Number(serde_json::Number::from(claims.iat)));
            map
        },
    };
...
pub fn require_web3_permission(required_permission: &'static str) -> impl Fn(Request, Next) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Response, StatusCode>> + Send>> + Clone {
    move |request: Request, next: Next| {
        ...
        Box::pin(async move {
            ...
            let web3_context = request
                ...
                .get::<Web3AuthContext>()
                .ok_or_else(|| {
                    warn!("Web3 context not found - ensure web3_auth_middleware runs first");
                    StatusCode::UNAUTHORIZED
                })?;
            ...
            if !has_permission {
                warn!(
                    ...
                    web3_context.user_id, web3_context.wallet_address, required_perm
                );
...
impl Web3AuthContext {
    ...
    pub fn wallet_address_typed(&self) -> Result<ethers::types::Address, Box<dyn std::error::Error>> {
        self.wallet_address.parse().map_err(|e| Box::new(e) as Box<dyn std::error::Error>)
    }
}
...
pub fn require_permission(permission: &'static str) -> impl Clone + Fn(Request, Next) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Response, StatusCode>> + Send>> {
...
mod tests {
    ...
    fn test_wallet_address_parsing() {
        ...
        assert_eq!(addr.to_string().to_lowercase(), "0x742d35cc6634c0532925a3b8d369d7763f3c45c6");
    }
}