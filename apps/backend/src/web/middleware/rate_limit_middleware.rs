...
impl RateLimitBucket {
    ...
    fn refill(&mut self) {
        ...
        if elapsed_minutes > 0.0 {
            let tokens_to_add = ((self.tier.requests_per_minute() as f64) * elapsed_minutes / 60.0) as u32;
            self.tokens = (self.tokens + tokens_to_add).min(self.tier.burst_capacity());
...
impl Web3RateLimitService {
    ...
    pub async fn is_allowed(&self, wallet_address: &str) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    async fn get_wallet_tier(&self, wallet_address: &str) -> Result<RateLimitTier, Box<dyn std::error::Error>> {
        // Check cache first
        {
            let cache = self.holdings_cache.read().await;
            if let Some(cached) = cache.get(wallet_address) {
                if Utc::now() < cached.expires_at {
                    debug!("Using cached tier {:?} for wallet {}", cached.tier, wallet_address);
        ...
        // Cache the result
        {
            ...
            cache.insert(wallet_address.to_string(), TokenHoldingsCache {
                ...
                tier: tier.clone(),
                cached_at: Utc::now(),
                expires_at: Utc::now() + Duration::minutes(self.config.cache_duration_minutes),
            });
        }
        
        debug!("Calculated tier {:?} for wallet {} (${:.2} total value)", tier, wallet_address, total_value);
        Ok(tier)
    }
    
    /// Calculate total USD value across all supported chains
    async fn calculate_cross_chain_value(&self, wallet_address: &str) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        for (network, tokens) in networks {
            for (contract_address, _symbol, price_usd) in tokens {
                match self.get_token_balance_value(wallet_address, contract_address, network, price_usd).await {
                    Ok(value) => {
                        total_value += value;
                        debug!("Found ${:.2} worth of tokens on {} for {}", value, network, wallet_address);
                    }
                    Err(e) => {
        ...
        if let Ok(nft_value) = self.estimate_nft_portfolio_value(wallet_address).await {
    ...
    async fn estimate_nft_portfolio_value(&self, wallet_address: &str) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    pub async fn get_tier_info(&self, wallet_address: &str) -> (RateLimitTier, u32, u32) {
        let tier = self.get_wallet_tier(wallet_address).await
...
pub async fn web3_rate_limit_middleware(
    ...
    if let Some(wallet_address) = wallet_address {
        ...
        let allowed = if is_premium { true } else {
