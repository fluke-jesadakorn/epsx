...
pub async fn get_wallet_permissions(
    ...
    if let Some(wallet_address) = &params.wallet_address {
        match app_state.web3_permission_service.get_wallet_permissions(wallet_address).await {
            Ok(permissions) => {
                let response_permissions: Vec<WalletPermissionResponse> = permissions.into_iter().map(|p| {
                ...
                info!("✅ Admin: Retrieved {} permissions for wallet {}", response.total_count, wallet_address);
                Ok(Json(response))
            }
            Err(e) => {
                error!("❌ Admin: Failed to get permissions for wallet {}: {}", wallet_address, e);
...
pub async fn grant_manual_permission(
    ...
    info!("🎯 Admin: Granting manual permissions to wallet: {}", request.wallet_address);
    ...
    for permission in &request.permissions {
        match app_state.web3_permission_service.grant_manual_permission(
            ...
            Ok(permission_id) => {
                info!("✅ Admin: Granted permission '{}' to wallet {}", permission, request.wallet_address);
                granted_permissions.push(permission.clone());
            }
            Err(e) => {
                error!("❌ Admin: Failed to grant permission '{}' to wallet {}: {}", 
                    permission, request.wallet_address, e);
    ...
    info!("🎉 Admin: Successfully granted {} permission(s) to wallet {}", 
        granted_permissions.len(), request.wallet_address);
    Ok(Json(response))
}
...
pub async fn create_nft_gate(
    ...
    info!("🎨 Admin: Creating NFT gate for contract: {}", request.contract_address);
    ...
    for permission in &request.permissions {
        ...
        match app_state.web3_permission_service.configure_nft_permission(
            nft_config,
            Uuid::new_v4(), // TODO: Use actual admin user ID
        ).await {
            Ok(gate_id) => {
                info!("✅ Admin: Created NFT gate for permission '{}' on {}", permission, network);
                created_gates.push(permission.clone());
            }
            Err(e) => {
                error!("❌ Admin: Failed to create NFT gate for permission '{}': {}", permission, e);
            }
        }
    }
    ...
    info!("🎉 Admin: Successfully created {} NFT gate(s) for contract {}", 
        created_gates.len(), request.contract_address);
    Ok(Json(response))
}
...
pub async fn create_token_gate(
    ...
    info!("🪙 Admin: Creating token gate for contract: {}", request.contract_address);
    ...
    for permission in &request.permissions {
        ...
        match app_state.web3_permission_service.configure_token_permission(
            token_config,
            Uuid::new_v4(), // TODO: Use actual admin user ID
        ).await {
            Ok(gate_id) => {
                info!("✅ Admin: Created token gate for permission '{}' on {}", permission, network);
                created_gates.push(permission.clone());
            }
            Err(e) => {
                error!("❌ Admin: Failed to create token gate for permission '{}': {}", permission, e);
            }
        }
    }
    ...
    info!("🎉 Admin: Successfully created {} token gate(s) for contract {}", 
        created_gates.len(), request.contract_address);
    Ok(Json(response))
}
...
pub async fn create_dao_proposal(
    ...
    let expires_at = match chrono::DateTime::parse_from_rfc3339(&request.expires_at) {
    ...
    for permission in &request.permissions {
        ...
        match app_state.web3_permission_service.create_dao_proposal(dao_proposal).await {
            Ok(proposal_id) => {
                info!("✅ Admin: Created DAO proposal for permission '{}' on {}", permission, network);
                created_proposals.push(permission.clone());
            }
            Err(e) => {
                error!("❌ Admin: Failed to create DAO proposal for permission '{}': {}", permission, e);
            }
        }
    }
    ...
    info!("🎉 Admin: Successfully created {} DAO proposal(s): {}", 
        created_proposals.len(), request.title);
    Ok(Json(response))
}
