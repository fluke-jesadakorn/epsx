...
pub async fn get_revenue_analytics_handler(
    ...
    let total_revenue = match sqlx::query_as::<_, RevenueResult>(
    ...
    let monthly_recurring_revenue = match sqlx::query_as::<_, RevenueResult>(
        "SELECT COALESCE(SUM(
            CASE
    ...
    let revenue_by_tier = match sqlx::query_as::<_, TierRevenueRow>(
    ...
    let active_subscriptions = match sqlx::query_as::<_, CountResult>(
    ...
    let new_subscriptions = match sqlx::query_as::<_, CountResult>(
        "SELECT COUNT(*)::bigint as count FROM wallet_plan_assignments wga
         INNER JOIN plans pg ON wga.plan_id = pg.id
         WHERE wga.is_active = true AND pg.plan_type = 'subscription'
    ...
    let cancelled_subscriptions = match sqlx::query_as::<_, CountResult>(
        "SELECT COUNT(*)::bigint as count FROM wallet_plan_assignments wga
         INNER JOIN plans pg ON wga.plan_id = pg.id
    ...
    let revenue_trends = match sqlx::query_as::<_, RevenueTrendRow>(
        r#"
        SELECT
