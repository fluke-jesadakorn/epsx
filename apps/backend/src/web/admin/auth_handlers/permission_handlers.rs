...
pub async fn get_user_permissions(
    ...
    info!(
        "Admin: Successfully fetched {} wallet permissions",
        response.permissions.len()
    );
    Ok(Json(AdminApiResponse::success_with_meta(
        response,
        "Wallet permissions retrieved successfully",
        metadata,
    )))
}
...
pub async fn grant_manual_permission(
    ...
    for permission_str in &request.permissions {
        ...
        let command =
            crate::application::wallet_management::commands::models::GrantPermissionCommand::new(
                wallet_address.clone(),
                permission_str.clone(),
            )
            .with_expiration(
                expires_at.unwrap_or_else(|| chrono::Utc::now() + chrono::Duration::days(365)),
            )
            ...
            .with_reason(
                request
                    .grant_reason
                    .clone()
                    .unwrap_or_else(|| "Manual admin grant".to_string()),
            );
        ...
        match handler.handle(command).await {
            Ok(response) => {
                info!(
                    "Admin: Successfully granted permission {} to wallet {}",
                    permission_str, wallet_address
                );
            ...
            Err(e) => {
                error!(
                    ...
                    permission_str, wallet_address, e
                );
    ...
    let metadata =
        AdminMetadata::crud_operation("grant_manual_permission", Some("admin".to_string()));
