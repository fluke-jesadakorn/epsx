...
pub async fn grant_permission_handler(
    ...
    let admin_id = extractuser_id_from_context().await?;
    verify_admin_permissions(&admin_id, "/api/v1/admin/permissions", "POST").await?;
    
    info!("Admin {} granting permission to user: {} - {:?}", admin_id, user_id, permission_data);
...
pub async fn revoke_permission_handler(
    ...
    let admin_id = extractuser_id_from_context().await?;
    verify_admin_permissions(&admin_id, "/api/v1/admin/permissions", "DELETE").await?;
    
    info!("Admin {} revoking permission {} from user: {}", admin_id, permission, user_id);
...
pub async fn list_permissions_handler(
    ...
    let admin_id = extractuser_id_from_context().await?;
    verify_admin_permissions(&admin_id, "/api/v1/admin/permissions", "GET").await?;
...
pub async fn get_wallet_permissions_handler(
    ...
    info!("Admin {} getting permissions for wallet: {}", admin_id, wallet_address);
