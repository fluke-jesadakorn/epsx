// Direct Permission Management
// Consolidates direct permission operations from normalized_permission_handlers.rs and granular_permissions.rs

use axum::{
    extract::{Path, State},
    response::IntoResponse,
    Json,
};
use serde::{Deserialize, Serialize};
use chrono::{DateTime, Utc};
use uuid::Uuid;
use sqlx::Row;

use crate::web::auth::AppState;
use crate::web::responses::AdminResponse;

// ============================================================================
// REQUEST/RESPONSE TYPES
// ============================================================================

#[derive(Debug, Deserialize)]
pub struct GrantDirectPermissionRequest {
    pub wallet_address: String,
    pub permission_string: String,
    pub expires_at: Option<DateTime<Utc>>,
    pub reason: Option<String>,
}

#[derive(Debug, Deserialize)]
pub struct RevokeDirectPermissionRequest {
    pub wallet_address: String,
    pub permission_string: String,
}

#[derive(Debug, Serialize)]
pub struct DirectPermissionResponse {
    pub id: String,
    pub wallet_address: String,
    pub permission_id: String,
    pub permission_string: String,
    pub platform: String,
    pub resource: String,
    pub action: String,
    pub granted_at: DateTime<Utc>,
    pub expires_at: Option<DateTime<Utc>>,
    pub is_active: bool,
}

#[derive(Debug, Deserialize)]
pub struct AddPermissionToGroupRequest {
    pub permission_string: String,
}

// ============================================================================
// HANDLERS
// ============================================================================

/// Grant a direct permission to a wallet
/// POST /admin/permissions/direct
pub async fn grant_permission(
    State(app_state): State<AppState>,
    Json(req): Json<GrantDirectPermissionRequest>,
) -> impl IntoResponse {
    let wallet = req.wallet_address.to_lowercase();

    // Validate wallet address format
    if !wallet.starts_with("0x") || wallet.len() != 42 {
        return AdminResponse::bad_request("Invalid wallet address format").into_response();
    }

    // Begin transaction
    let mut tx = match app_state.db_pool.begin().await {
        Ok(tx) => tx,
        Err(e) => {
            tracing::error!("Failed to begin transaction: {}", e);
            return AdminResponse::server_error("Database transaction failed").into_response();
        }
    };

    // Parse permission string
    let parts: Vec<&str> = req.permission_string.split(':').collect();
    if parts.len() < 3 {
        return AdminResponse::bad_request("Invalid permission string format (expected platform:resource:action)").into_response();
    }

    // Get or create permission
    let perm_id = match sqlx::query_scalar::<_, Uuid>(
        r#"
        INSERT INTO permissions (permission_string, platform, resource, action, permission_type)
        VALUES ($1, $2, $3, $4, 'manual')
        ON CONFLICT (permission_string) DO UPDATE SET permission_string = EXCLUDED.permission_string
        RETURNING id
        "#
    )
    .bind(&req.permission_string)
    .bind(parts[0])
    .bind(parts[1])
    .bind(parts[2])
    .fetch_one(&mut *tx)
    .await
    {
        Ok(id) => id,
        Err(e) => {
            tracing::error!("Failed to create/get permission: {}", e);
            return AdminResponse::server_error("Failed to create permission").into_response();
        }
    };

    // Grant direct permission to wallet
    let grant_id = match sqlx::query_scalar::<_, Uuid>(
        r#"
        INSERT INTO wallet_direct_permissions (wallet_address, permission_id, expires_at)
        VALUES ($1, $2, $3)
        ON CONFLICT (wallet_address, permission_id) DO UPDATE
        SET expires_at = EXCLUDED.expires_at, is_active = true, updated_at = NOW()
        RETURNING id
        "#
    )
    .bind(&wallet)
    .bind(perm_id)
    .bind(req.expires_at)
    .fetch_one(&mut *tx)
    .await
    {
        Ok(id) => id,
        Err(e) => {
            tracing::error!("Failed to grant direct permission: {}", e);
            return AdminResponse::server_error("Failed to grant permission").into_response();
        }
    };

    // Commit transaction
    if let Err(e) = tx.commit().await {
        tracing::error!("Failed to commit transaction: {}", e);
        return AdminResponse::server_error("Failed to save permission").into_response();
    }

    tracing::info!(
        "Granted direct permission '{}' to wallet {}",
        req.permission_string,
        wallet
    );

    let response = DirectPermissionResponse {
        id: grant_id.to_string(),
        wallet_address: wallet,
        permission_id: perm_id.to_string(),
        permission_string: req.permission_string.clone(),
        platform: parts[0].to_string(),
        resource: parts[1].to_string(),
        action: parts[2].to_string(),
        granted_at: Utc::now(),
        expires_at: req.expires_at,
        is_active: true,
    };

    AdminResponse::created(response, "Direct permission granted successfully").into_response()
}

/// Revoke a direct permission from a wallet
/// DELETE /admin/permissions/direct
pub async fn revoke_permission(
    State(app_state): State<AppState>,
    Json(req): Json<RevokeDirectPermissionRequest>,
) -> impl IntoResponse {
    let wallet = req.wallet_address.to_lowercase();

    // Get permission ID
    let perm_id = match sqlx::query_scalar::<_, Uuid>(
        "SELECT id FROM permissions WHERE permission_string = $1"
    )
    .bind(&req.permission_string)
    .fetch_optional(&*app_state.db_pool)
    .await
    {
        Ok(Some(id)) => id,
        Ok(None) => return AdminResponse::not_found("Permission").into_response(),
        Err(e) => {
            tracing::error!("Failed to fetch permission: {}", e);
            return AdminResponse::server_error("Database query failed").into_response();
        }
    };

    // Revoke direct permission
    match sqlx::query(
        "DELETE FROM wallet_direct_permissions WHERE wallet_address = $1 AND permission_id = $2"
    )
    .bind(&wallet)
    .bind(perm_id)
    .execute(&*app_state.db_pool)
    .await
    {
        Ok(result) if result.rows_affected() > 0 => {
            tracing::info!(
                "Revoked direct permission '{}' from wallet {}",
                req.permission_string,
                wallet
            );
            AdminResponse::success_with_message(
                serde_json::json!({"deleted": true}),
                "Direct permission revoked successfully"
            ).into_response()
        },
        Ok(_) => AdminResponse::not_found("Direct permission grant").into_response(),
        Err(e) => {
            tracing::error!("Failed to revoke permission: {}", e);
            AdminResponse::server_error("Failed to revoke permission").into_response()
        }
    }
}

/// List direct permissions for a wallet
/// GET /admin/permissions/direct/:wallet
pub async fn list_wallet_permissions(
    State(app_state): State<AppState>,
    Path(wallet): Path<String>,
) -> impl IntoResponse {
    let wallet = wallet.to_lowercase();

    let rows = match sqlx::query(
        r#"
        SELECT
            wdp.id, wdp.permission_id, wdp.granted_at, wdp.expires_at, wdp.is_active,
            p.permission_string, p.platform, p.resource, p.action
        FROM wallet_direct_permissions wdp
        JOIN permissions p ON wdp.permission_id = p.id
        WHERE wdp.wallet_address = $1 AND wdp.is_active = true
        ORDER BY wdp.granted_at DESC
        "#
    )
    .bind(&wallet)
    .fetch_all(&*app_state.db_pool)
    .await
    {
        Ok(rows) => rows,
        Err(e) => {
            tracing::error!("Failed to list direct permissions: {}", e);
            return AdminResponse::server_error("Database query failed").into_response();
        }
    };

    let permissions: Vec<DirectPermissionResponse> = rows.iter().map(|row| {
        DirectPermissionResponse {
            id: row.get::<Uuid, _>("id").to_string(),
            wallet_address: wallet.clone(),
            permission_id: row.get::<Uuid, _>("permission_id").to_string(),
            permission_string: row.get("permission_string"),
            platform: row.get("platform"),
            resource: row.get("resource"),
            action: row.get("action"),
            granted_at: row.get("granted_at"),
            expires_at: row.get("expires_at"),
            is_active: row.get("is_active"),
        }
    }).collect();

    AdminResponse::success(serde_json::json!({
        "wallet_address": wallet,
        "permissions": permissions,
        "count": permissions.len()
    })).into_response()
}

/// Add a permission to a group
/// POST /admin/permissions/groups/:group_id/permissions
pub async fn add_permission_to_group(
    State(app_state): State<AppState>,
    Path(group_id): Path<String>,
    Json(req): Json<AddPermissionToGroupRequest>,
) -> impl IntoResponse {
    let group_uuid = match Uuid::parse_str(&group_id) {
        Ok(id) => id,
        Err(_) => return AdminResponse::bad_request("Invalid group ID format").into_response(),
    };

    // Begin transaction
    let mut tx = match app_state.db_pool.begin().await {
        Ok(tx) => tx,
        Err(e) => {
            tracing::error!("Failed to begin transaction: {}", e);
            return AdminResponse::server_error("Database transaction failed").into_response();
        }
    };

    // Parse permission string
    let parts: Vec<&str> = req.permission_string.split(':').collect();
    if parts.len() < 3 {
        return AdminResponse::bad_request("Invalid permission string format (expected platform:resource:action)").into_response();
    }

    // Get or create permission
    let perm_id = match sqlx::query_scalar::<_, Uuid>(
        r#"
        INSERT INTO permissions (permission_string, platform, resource, action, permission_type)
        VALUES ($1, $2, $3, $4, 'manual')
        ON CONFLICT (permission_string) DO UPDATE SET permission_string = EXCLUDED.permission_string
        RETURNING id
        "#
    )
    .bind(&req.permission_string)
    .bind(parts[0])
    .bind(parts[1])
    .bind(parts[2])
    .fetch_one(&mut *tx)
    .await
    {
        Ok(id) => id,
        Err(e) => {
            tracing::error!("Failed to create/get permission: {}", e);
            return AdminResponse::server_error("Failed to create permission").into_response();
        }
    };

    // Add permission to group
    let membership = match sqlx::query(
        r#"
        INSERT INTO permission_group_memberships (group_id, permission_id)
        VALUES ($1, $2)
        ON CONFLICT (group_id, permission_id) DO NOTHING
        RETURNING id, group_id, permission_id, granted_at
        "#
    )
    .bind(group_uuid)
    .bind(perm_id)
    .fetch_optional(&mut *tx)
    .await
    {
        Ok(result) => result,
        Err(e) => {
            tracing::error!("Failed to add permission to group: {}", e);
            return AdminResponse::server_error("Failed to add permission to group").into_response();
        }
    };

    // Commit transaction
    if let Err(e) = tx.commit().await {
        tracing::error!("Failed to commit transaction: {}", e);
        return AdminResponse::server_error("Failed to save changes").into_response();
    }

    if let Some(m) = membership {
        tracing::info!(
            "Added permission '{}' to group {}",
            req.permission_string,
            group_id
        );
        AdminResponse::created(
            serde_json::json!({
                "id": m.get::<Uuid, _>("id").to_string(),
                "group_id": m.get::<Uuid, _>("group_id").to_string(),
                "permission_id": m.get::<Uuid, _>("permission_id").to_string(),
                "granted_at": m.get::<DateTime<Utc>, _>("granted_at"),
            }),
            "Permission added to group successfully"
        ).into_response()
    } else {
        AdminResponse::success_with_message(
            serde_json::json!({"exists": true}),
            "Permission already exists in group"
        ).into_response()
    }
}

/// Remove a permission from a group
/// DELETE /admin/permissions/groups/:group_id/permissions/:permission_id
pub async fn remove_permission_from_group(
    State(app_state): State<AppState>,
    Path((group_id, permission_id)): Path<(String, String)>,
) -> impl IntoResponse {
    let group_uuid = match Uuid::parse_str(&group_id) {
        Ok(id) => id,
        Err(_) => return AdminResponse::bad_request("Invalid group ID format").into_response(),
    };

    let perm_uuid = match Uuid::parse_str(&permission_id) {
        Ok(id) => id,
        Err(_) => return AdminResponse::bad_request("Invalid permission ID format").into_response(),
    };

    match sqlx::query(
        "DELETE FROM permission_group_memberships WHERE group_id = $1 AND permission_id = $2"
    )
    .bind(group_uuid)
    .bind(perm_uuid)
    .execute(&*app_state.db_pool)
    .await
    {
        Ok(result) if result.rows_affected() > 0 => {
            tracing::info!(
                "Removed permission {} from group {}",
                permission_id,
                group_id
            );
            AdminResponse::success_with_message(
                serde_json::json!({"deleted": true}),
                "Permission removed from group successfully"
            ).into_response()
        },
        Ok(_) => AdminResponse::not_found("Permission membership").into_response(),
        Err(e) => {
            tracing::error!("Failed to remove permission from group: {}", e);
            AdminResponse::server_error("Failed to remove permission from group").into_response()
        }
    }
}
