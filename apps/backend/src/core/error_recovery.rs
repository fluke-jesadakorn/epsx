...
use crate::core::errors::{AppError, ErrorKind, ErrorLogger, ErrorContextBuilder};
...
pub trait RecoveryStrategy<T>: Send + Sync {
    ...
    async fn recover(&self, error: &AppError, attempt: u32) -> Result<Option<T>, AppError>;
...
#[async_trait]
impl<F, Fut, T> RecoveryStrategy<T> for RetryRecoveryStrategy<F, Fut, T>
where
    F: Fn() -> Fut + Send + Sync,
    Fut: std::future::Future<Output = Result<T, AppError>> + Send,
    T: Send + 'static,
{
    ...
    async fn recover(&self, error: &AppError, attempt: u32) -> Result<Option<T>, AppError> {
        ...
        ErrorLogger::log_recovery_attempt(error, &format!("retry_attempt_{}", attempt + 1));
...
impl CircuitBreakerRecovery {
    pub fn new(operation_name: String, failure_threshold: u32, recovery_timeout: Duration) -> Self {
        Self {
            failure_threshold,
            recovery_timeout,
    ...
    pub fn is_circuit_open(&self) -> bool {
        let failures = self.current_failures.load(std::sync::atomic::Ordering::Acquire);
    ...
    pub fn record_failure(&self) {
        let failures = self.current_failures.fetch_add(1, std::sync::atomic::Ordering::AcqRel) + 1;
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
...
#[async_trait]
impl<T> RecoveryStrategy<T> for FallbackRecovery<T>
where
    T: Clone + Send + Sync + 'static,
{
    ...
    async fn recover(&self, error: &AppError, _attempt: u32) -> Result<Option<T>, AppError> {
        tracing::warn!(
            operation = %self.operation_name,
            error_kind = %error.kind,
    ...
    T: Send + 'static,
{
    ...
    pub fn add_strategy(mut self, strategy: Box<dyn RecoveryStrategy<T>>) -> Self {
        self.strategies.push(strategy);
        self
    }
    
    pub async fn recover_from_error(&self, error: AppError) -> Result<T, AppError> {
        ...
        for strategy in &self.strategies {
            ...
            while attempt < config.max_retries {
                match strategy.recover(&current_error, attempt).await {
                    ...
                    Ok(None) => break, // Strategy gave up
                    Err(retry_error) => {
        ...
        let final_error = AppError::new(
            ...
            format!("All recovery strategies failed for error: {}", current_error.message)
        ).with_context(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_retry_recovery_success_after_failures() {
        ...
        let strategy = RetryRecoveryStrategy::new(operation, "test_operation".to_string())
            .with_config(config);
        ...
        let error = AppError::new(ErrorKind::NetworkError, "Initial failure");
        let result = orchestrator.recover_from_error(error).await;
        
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), "Success");
        assert_eq!(counter.load(Ordering::Acquire), 3); // First attempt + 2 retries
    }
    
    #[tokio::test]
    async fn test_fallback_recovery() {
        let fallback_strategy = FallbackRecovery::new("fallback_value".to_string(), "test".to_string());
        
        let mut orchestrator = RecoveryOrchestrator::new()
            .add_strategy(Box::new(fallback_strategy));
        
        let error = AppError::new(ErrorKind::InternalError, "Unrecoverable error");
        let result = orchestrator.recover_from_error(error).await;
    ...
    fn test_circuit_breaker() {
        ...
        circuit_breaker.record_failure();
        assert!(!circuit_breaker.is_circuit_open()); // Still under threshold
        
        circuit_breaker.record_failure();
        assert!(circuit_breaker.is_circuit_open()); // Should be open now
    ...
    fn test_backoff_delay_calculation() {
        ...
        let delay_0 = calculate_backoff_delay(&config, 0);
        let delay_1 = calculate_backoff_delay(&config, 1);
        let delay_2 = calculate_backoff_delay(&config, 2);
