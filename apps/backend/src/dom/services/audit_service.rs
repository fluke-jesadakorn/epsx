...
impl<'a> AuditServiceBuilder<'a> {
    pub fn action(self, action: AuditAction, resource_type: ResourceType, resource_id: String) -> AuditLogBuilder {
        AuditLogBuilder {
...
impl AuditLogBuilder {
    ...
    pub fn with_previous_values(mut self, previous: HashMap<String, String>) -> Self {
    ...
    pub async fn failure(mut self, service: &AuditService, error_message: String) -> Result<(), AuditError> {
        let duration_ms = self.start_time.elapsed().as_millis() as u64;
    ...
    pub async fn denied(self, service: &AuditService, reason: Option<String>) -> Result<(), AuditError> {
        let duration_ms = self.start_time.elapsed().as_millis() as u64;
        let mut metadata_builder = self.metadata_builder;
        
        if let Some(reason) = reason {
            metadata_builder.additional_data.insert("denial_reason".to_string(), reason);
        }
...
mod tests {
    use super::*;
    use std::sync::Arc;
    use crate::infra::repos::audit_repo::AuditRepoImpl;
    
    #[tokio::test]
    async fn should_log_simple_success() {
        let audit_repo = Arc::new(AuditRepoImpl::new());
        let service = AuditService::new(audit_repo.clone());
        
        let context = AuditContext::new(UserId::new("user123".to_string()));
        ...
        let query = crate::dom::entities::audit::AuditQuery::new();
        let entries = audit_repo.search(&query).await.unwrap();
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].action(), &AuditAction::UserCreated);
    }
    
    #[tokio::test]
    async fn should_log_authentication() {
        let audit_repo = Arc::new(AuditRepoImpl::new());
        let service = AuditService::new(audit_repo.clone());
    ...
    #[tokio::test]
    async fn should_log_permission_check() {
        let audit_repo = Arc::new(AuditRepoImpl::new());
        let service = AuditService::new(audit_repo.clone());
        
        let context = AuditContext::new(UserId::new("user123".to_string()));
        
        let result = service.log_permission_check(
            context,
            "users:read",
            "users/*",
            false,
        ...
        let entries = audit_repo.search(&query).await.unwrap();
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].action(), &AuditAction::PermissionEvaluated);
        assert_eq!(entries[0].result(), &AuditResult::Denied);
    }
    
    #[tokio::test]
    async fn should_log_bulk_operation() {
        let audit_repo = Arc::new(AuditRepoImpl::new());
        let service = AuditService::new(audit_repo.clone());
        
        let context = AuditContext::new(UserId::new("admin123".to_string()));
        
        let result = service.log_bulk_operation(
            context,
            AuditAction::BulkUserUpdate,
            ResourceType::User,
            25,
            true,
        ).await;
        ...
        let entries = audit_repo.search(&query).await.unwrap();
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].action(), &AuditAction::BulkUserUpdate);
        assert_eq!(entries[0].metadata().affected_count, Some(25));
    }
}