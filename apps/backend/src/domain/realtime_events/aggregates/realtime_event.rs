...
impl RealtimeEvent {
    ...
    pub fn create(
        ...
        if channel.is_empty() {
            return Err(RealtimeEventError::InvalidChannel(
        ...
        event
            .base
            .add_event(Box::new(super::super::events::EventCreated {
    ...
    pub fn start_delivery(&mut self) -> Result<(), RealtimeEventError> {
        if !matches!(
            self.status,
            EventStatus::Pending | EventStatus::Scheduled | EventStatus::Retrying
        ) {
        ...
        self.delivery_metadata
            .insert("delivery_started_at".to_string(), Utc::now().to_rfc3339());
        self.delivery_metadata
            .insert("attempt".to_string(), self.delivery_attempts.to_string());
    ...
    pub fn mark_delivered(&mut self) -> Result<(), RealtimeEventError> {
        ...
        self.base
            .add_event(Box::new(super::super::events::EventDelivered {
    ...
    pub fn mark_failed(&mut self, reason: String) -> Result<(), RealtimeEventError> {
        ...
        if self.delivery_attempts < self.max_retry_attempts {
            ...
            // Publish retry event
            self.base
                .add_event(Box::new(super::super::events::EventRetryScheduled {
                    event_id: self.id.clone(),
            ...
            self.base
                .add_event(Box::new(super::super::events::EventFailed {
                    event_id: self.id.clone(),
...
mod tests {
    ...
    fn test_event_delivery_lifecycle() {
        ...
        let mut event =
            RealtimeEvent::create_broadcast(payload, "notifications".to_string()).unwrap();
