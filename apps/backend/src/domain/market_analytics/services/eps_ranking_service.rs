...
pub trait EPSRepository: Send + Sync {
    ...
    async fn batch_store_eps_data(
        &self,
        eps_data_list: Vec<EPSGrowthData>,
...
impl PermissionParser {
    ...
    pub fn extract_ranking_config(permissions: &[String]) -> (i32, i32) {
        debug!(
            "Extracting ranking config from {} permissions",
            permissions.len()
        );
        ...
        for perm in permissions {
            ...
            if let Some(val_str) = perm
                .strip_prefix("epsx:rankings:limit:")
        ...
        info!(
            "Calculated ranking config: offset={}, limit={}",
...
impl EPSRankingService {
    ...
    pub async fn get_eps_rankings(
        ...
        let validated_country = if let Some(ref country) = params.country {
            Some(CountryValidator::validate_country(country).map_err(|e| {
                AppError::new(epsx_contracts::errors::ErrorKind::ValidationError, e)
            })?)
        } else {
            None
        };
        ...
        debug!(
            "Found {} rankings from repository (offset: {})",
            rankings.len(),
            params.rank_offset
        );
        ...
        let total_count = self
            ...
            .await?;
        debug!(
            "Total count for pagination: {} (from rank {} onwards)",
            total_count, params.rank_offset
        );
        ...
        info!(
            "Returning {} EPS rankings for page {} (total: {})",
            response.rankings.len(),
            page,
            total_count
        );
    ...
    pub async fn store_eps_data(&self, mut eps_data: EPSGrowthData) -> Result<(), AppError> {
        ...
        eps_data.validate().map_err(|e| {
            AppError::new(
                epsx_contracts::errors::ErrorKind::ValidationError,
                format!("EPS data validation failed: {}", e),
            )
        })?;
        ...
        debug!(
            "Validated EPS data for {} with ranking score: {:?}",
    ...
    pub async fn batch_store_eps_data(
        &self,
        eps_data_list: Vec<EPSGrowthData>,
    ) -> Result<usize, AppError> {
        ...
        if validation_errors > 0 {
            warn!(
                "Validation errors occurred for {} entries",
                validation_errors
            );
        }

        debug!(
            "Validated {} out of {} EPS entries",
            validated_data.len(),
            validated_data.len() + validation_errors
        );
    ...
    pub async fn get_sectors_by_country(
        ...
        let sectors = self
            .eps_repo
            .get_sectors_by_country(validated_country)
            .await?;
    ...
    pub async fn get_total_count_for_params(
        ...
        let count = self
            .eps_repo
            .get_total_count(params.rank_offset, validated_country, params.sector.clone())
            .await?;
        debug!(
            "Total count for parameters: {} (from rank {} onwards)",
            count, params.rank_offset
        );
    ...
    pub fn validate_ranking_params(&self, params: &EPSRankingParams) -> Result<(), AppError> {
        ...
        if let Some(ref country) = params.country {
            CountryValidator::validate_country(country).map_err(|e| {
                AppError::new(epsx_contracts::errors::ErrorKind::ValidationError, e)
            })?;
        }
        ...
        if let Some(min_eps) = params.min_eps {
            if !(-1000.0..=1000.0).contains(&min_eps) {
                return Err(AppError::new(
                    epsx_contracts::errors::ErrorKind::ValidationError,
                    "min_eps must be between -1000 and 1000",
                ));
            }
        }
        ...
        if let Some(min_growth) = params.min_growth {
            if !(-500.0..=1000.0).contains(&min_growth) {
                return Err(AppError::new(
                    epsx_contracts::errors::ErrorKind::ValidationError,
                    "min_growth must be between -500 and 1000",
                ));
            }
        }
...
mod tests {
    ...
    fn test_starter_plan_permissions() {
        ...
        assert_eq!(
            offset, 100,
            "Starter plan should have default offset 100 without explicit offset permission"
        );
