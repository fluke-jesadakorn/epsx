...
impl GrowthFactor {
    ...
    pub fn new(percentage: f64) -> Result<Self, String> {
        ...
        if !(-1000.0..=10000.0).contains(&percentage) {
            return Err(
                "Growth factor is outside reasonable range (-1000% to +10000%)".to_string(),
            );
        }
...
mod tests {
    ...
    fn test_invalid_growth_factors() {
        ...
        for value in &invalid_values {
            ...
            assert!(
                growth.is_err(),
                "Should have failed for growth factor: {}",
                value
            );
        }
    }
    ...
    fn test_growth_classification() {
        assert_eq!(
            GrowthFactor::new(150.0).unwrap().classify(),
            GrowthClassification::Explosive
        );
        assert_eq!(
            GrowthFactor::new(75.0).unwrap().classify(),
            GrowthClassification::Strong
        );
        assert_eq!(
            GrowthFactor::new(25.0).unwrap().classify(),
            GrowthClassification::Moderate
        );
        assert_eq!(
            GrowthFactor::new(5.0).unwrap().classify(),
            GrowthClassification::Weak
        );
        assert_eq!(
            GrowthFactor::new(0.05).unwrap().classify(),
            GrowthClassification::Flat
        );
        assert_eq!(
            GrowthFactor::new(-5.0).unwrap().classify(),
            GrowthClassification::SlightDecline
        );
        assert_eq!(
            GrowthFactor::new(-25.0).unwrap().classify(),
            GrowthClassification::Decline
        );
        assert_eq!(
            GrowthFactor::new(-75.0).unwrap().classify(),
            GrowthClassification::Collapse
        );
    }
