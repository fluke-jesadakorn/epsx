...
impl RetryConfiguration {
    ...
    match channel {
      DeliveryChannelType::WebPush =>
        Self {
          ...
          retry_on_errors: ["timeout", "server_error", "rate_limited"]
            .iter()
            .map(|s| s.to_string())
            .collect(),
        },
  ...
  pub fn delay_for_attempt(&self, attempt: u8) -> u32 {
    ...
    let delay =
      (self.initial_delay_seconds as f32) *
  ...
  pub fn should_retry_error(&self, error: &str) -> bool {
    self.retry_on_errors
      .iter()
      .any(|pattern| error.to_lowercase().contains(&pattern.to_lowercase()))
  }
}
...
impl MultiChannelConfig {
  ...
  pub fn channels_by_cost(
    &self,
    max_cost: DeliveryCost
  ) -> Vec<&DeliveryChannel> {
    self.channels
      .iter()
      .filter(
        |c|
          c.is_enabled() && Self::cost_is_within_limit(c.cost_tier(), max_cost)
      )
      .collect()
  }
  ...
  fn cost_is_within_limit(
    channel_cost: DeliveryCost,
    max_cost: DeliveryCost
  ) -> bool {
...
mod tests {
  ...
  fn test_retry_configuration() {
    let email_retry = RetryConfiguration::default_for_channel(
      &DeliveryChannelType::Email
    );
