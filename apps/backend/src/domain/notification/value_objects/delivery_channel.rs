...
impl DeliveryChannel {
    ...
    pub fn max_content_length(&self) -> Option<ContentLimits> {
        match self.channel_type {
            DeliveryChannelType::WebPush => Some(ContentLimits {
                title_max: 50,
                body_max: 150,
...
impl RetryConfiguration {
    ...
    pub fn default_for_channel(channel: &DeliveryChannelType) -> Self {
        match channel {
            DeliveryChannelType::WebPush => Self {
                max_attempts: 3,
            ...
            DeliveryChannelType::Email => Self {
                ...
                backoff_multiplier: 2.0,
                retry_on_errors: ["timeout", "server_error", "rate_limited", "temp_failure"]
                    .iter()
                    .map(|s| s.to_string())
                    .collect(),
            },
    ...
    pub fn delay_for_attempt(&self, attempt: u8) -> u32 {
        ...
        let delay = (self.initial_delay_seconds as f32)
