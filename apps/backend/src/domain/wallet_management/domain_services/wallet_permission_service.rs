...
impl WalletPermissionService {
    pub fn new() -> AppResult<Self> {
        Ok(Self {
            blockchain_client: Arc::new(BlockchainValidationClient::new().map_err(|e| {
                epsx_contracts::errors::AppError::blockchain_rpc_error(e.to_string())
            })?),
        })
    }
    ...
    pub async fn calculate_effective_permissions(
        ...
        effective_permissions.extend(self.get_web3_validated_permissions(wallet, context).await?);
    ...
    pub async fn can_wallet_access(
        ...
        let effective_permissions = self
            .calculate_effective_permissions(wallet, context)
            .await?;
    ...
    pub async fn validate_web3_permissions(
        ...
        for permission in permissions {
            if permission.requires_web3_validation() {
                let result = self
                    .validate_single_web3_permission(wallet.wallet_address(), permission, context)
                    .await?;
                results.push(result);
            } else {
    ...
    pub async fn validate_single_web3_permission(
        ...
        match permission.permission_type() {
            PermissionType::Manual => Ok(Web3ValidationResult {
                permission: permission.clone(),
                is_valid: true,
                validation_type: Web3ValidationType::Manual,
                blockchain_data: None,
    ...
    pub async fn sync_permissions_with_blockchain(
        ...
        let web3_permissions: Vec<Permission> = wallet
            .permissions()
            .iter()
        ...
        let validation_results = self
            .validate_web3_permissions(wallet, &web3_permissions, context)
            .await?;
...
mod tests {
    ...
    async fn generate_default_permissions_for_basic_wallet() {
        ...
        let permissions = service
            .generate_default_permissions(&wallet, &context)
            .await
            .unwrap();
        ...
        assert!(permissions
            .iter()
            .any(|p| p.as_str() == "epsx:analytics:view"));
        assert!(permissions
            .iter()
            .any(|p| p.as_str() == "epsx:basic:access"));
    }
    ...
    fn temporary_web3_permissions_creation() {
        ...
        let temp_permissions = service
            .create_temporary_web3_permissions(&base_permissions, Duration::hours(1))
            .unwrap();
