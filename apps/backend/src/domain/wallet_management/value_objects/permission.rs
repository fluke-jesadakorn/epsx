...
impl Permission {
    ...
    pub fn from_embedded_timestamp(
        permission: impl Into<String>,
    ) -> Result<Self, ValueObjectError> {
        let permission = permission.into();
        ...
        if parts.len() == 4 {
            // Has timestamp
            let timestamp = parts[3].parse::<i64>().map_err(|_| {
                ValueObjectError::InvalidFormat("Invalid timestamp format".to_string())
            })?;

            let expires_at = DateTime::from_timestamp(timestamp, 0).ok_or_else(|| {
                ValueObjectError::InvalidFormat("Invalid timestamp value".to_string())
            })?;
    ...
    pub fn contract_address(&self) -> Option<&String> {
        match &self.permission_type {
            PermissionType::Manual => None,
            PermissionType::NftGated {
                contract_address, ..
            } => Some(contract_address),
            PermissionType::TokenGated {
                contract_address, ..
            } => Some(contract_address),
...
impl ValueObject for Permission {
    ...
    fn validate(&self) -> Result<(), Self::Error> {
        ...
        if !valid_chars(&self.platform)
            || !valid_chars(&self.resource)
            || !valid_chars(&self.action)
        {
            return Err(ValueObjectError::InvalidFormat(
        ...
        match &self.permission_type {
            ...
            PermissionType::NftGated {
                ...
                if contract_address.is_empty() {
                    return Err(ValueObjectError::Required(
                        "NFT contract address cannot be empty".to_string(),
                    ));
                }
                if !Self::is_valid_ethereum_address(contract_address) {
                    return Err(ValueObjectError::InvalidFormat(
                        "Invalid NFT contract address format".to_string(),
                    ));
                }
                if *chain_id == 0 {
                    return Err(ValueObjectError::InvalidFormat(
            ...
            PermissionType::TokenGated {
                ...
                if min_balance.is_empty() || min_balance.parse::<u128>().is_err() {
                    return Err(ValueObjectError::InvalidFormat(
                        "Minimum balance must be a valid number".to_string(),
                    ));
                }
                if *chain_id == 0 {
                    return Err(ValueObjectError::InvalidFormat(
            ...
            PermissionType::DaoGovernance {
                ...
                if dao_contract.is_empty() {
                    return Err(ValueObjectError::Required(
                        "DAO contract address cannot be empty".to_string(),
                    ));
                }
                if !Self::is_valid_ethereum_address(dao_contract) {
                    return Err(ValueObjectError::InvalidFormat(
                        "Invalid DAO contract address format".to_string(),
                    ));
                }
                if min_voting_power.is_empty() || min_voting_power.parse::<u128>().is_err() {
                    return Err(ValueObjectError::InvalidFormat(
                        "Minimum voting power must be a valid number".to_string(),
                    ));
                }
                if *chain_id == 0 {
                    return Err(ValueObjectError::InvalidFormat(
        ...
        match &self.permission_type {
            ...
            PermissionType::NftGated {
                ...
                write!(
                    f,
                    "{}[nft:{}:{}]",
                    self.permission, contract_address, chain_id
                )
            }
            PermissionType::TokenGated {
                contract_address,
                ...
                write!(
                    f,
                    "{}[token:{}:{}:{}]",
                    self.permission, contract_address, min_balance, chain_id
                )
            }
            PermissionType::DaoGovernance {
                dao_contract,
                ...
                write!(
                    f,
                    "{}[dao:{}:{}:{}]",
                    self.permission, dao_contract, min_voting_power, chain_id
                )
            }
        }
    }
...
mod tests {
    ...
    fn permission_with_metadata_and_expiration() {
        ...
        assert!(!perm.is_expired());
        assert_eq!(
            perm.metadata().get("granted_by"),
            Some(&"admin123".to_string())
        );
        assert_eq!(
            perm.metadata().get("reason"),
    ...
    fn ethereum_address_validation() {
        assert!(Permission::is_valid_ethereum_address(
            "0x742d35Cc6634C0532925a3b8D369D7763F3c45c6"
        ));
        assert!(!Permission::is_valid_ethereum_address(
            "742d35Cc6634C0532925a3b8D369D7763F3c45c6"
        )); // No 0x prefix
        assert!(!Permission::is_valid_ethereum_address("0x742d35Cc")); // Too short
        assert!(!Permission::is_valid_ethereum_address(
            "0xGGGd35Cc6634C0532925a3b8D369D7763F3c45c6"
        )); // Invalid hex
    }
}
