...
use super::resource_type::{ResourceCategory, ResourceType};
...
pub struct RealTimeUsageTracker {
    pub identifier: String, // user_id, api_key, or sid
...
impl UsageMetrics {
    ...
    pub fn add_resource_usage(
        ...
        resource_stats.average_cost_per_unit =
            resource_stats.total_cost / Decimal::from(resource_stats.count);
        resource_stats.last_used_at = now;

        if let Some(bytes) = bytes {
            resource_stats.total_bytes = Some(resource_stats.total_bytes.unwrap_or(0) + bytes);
            self.total_data_transfer_bytes += bytes;
        }

        if let Some(duration) = duration_ms {
            resource_stats.total_duration_ms =
                Some(resource_stats.total_duration_ms.unwrap_or(0) + duration);
        }
    ...
    pub fn calculate_efficiency_score(&self) -> f64 {
        ...
        let cost_per_request =
            self.total_cost.to_f64().unwrap_or(0.0) / (self.total_requests as f64);
    ...
    fn get_top_resource_types(&self, limit: usize) -> Vec<(ResourceType, u64)> {
        let mut resource_counts: Vec<(ResourceType, u64)> = self
            .resource_usage
            .iter()
...
impl RealTimeUsageTracker {
    pub fn new(identifier: String) -> Self {
        ...
        Self {
            identifier,
            current_minute: UsageWindow::new(
                now.with_second(0).unwrap().with_nanosecond(0).unwrap(),
            ),
            current_hour: UsageWindow::new(
                now.with_minute(0)
                    .unwrap()
                    .with_second(0)
                    .unwrap()
