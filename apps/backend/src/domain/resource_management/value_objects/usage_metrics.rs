...
impl UsageMetrics {
  ...
  pub fn get_usage_summary(&self) -> UsageSummary {
    UsageSummary {
      ...
      total_data_transfer_gb: (self.total_data_transfer_bytes as f64) /
      1_000_000_000.0,
  ...
  fn recalculate_cost_percentages(&mut self) {
    ...
    if total_cost_f64 > 0.0 {
      for category_stats in self.category_totals.values_mut() {
        let category_cost_f64 = category_stats.total_cost
          .to_f64()
          .unwrap_or(0.0);
        category_stats.cost_percentage =
          (category_cost_f64 / total_cost_f64) * 100.0;
      }
    }
  }
