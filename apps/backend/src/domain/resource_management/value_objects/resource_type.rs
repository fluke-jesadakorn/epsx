    ...
    pub fn base_cost_factor(&self) -> f64 {
        match self {
            ...
            ResourceType::ApiBulkOperation {
                records_processed, ..
            } => (*records_processed as f64) * 0.001,
            ...
            ResourceType::DatabaseQuery {
                execution_time_ms, ..
            } => (*execution_time_ms as f64) * 0.00001,
            ResourceType::CacheOperation {
                data_size_bytes, ..
            } => (*data_size_bytes as f64) * 0.000001,
            ResourceType::FileStorage {
                file_size_bytes, ..
            } => (*file_size_bytes as f64) * 0.0000001,
            ResourceType::ComputeTime {
                duration_ms,
                cpu_cores,
                ..
...
impl fmt::Display for ResourceType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ResourceType::WebAnalyticsView { dataset } => {
                write!(f, "WebAnalyticsView({})", dataset)
            }
            ResourceType::WebDashboardInteraction { component } => {
                write!(f, "WebDashboardInteraction({})", component)
            }
            ResourceType::WebSessionActivity { duration_seconds } => {
                write!(f, "WebSessionActivity({}s)", duration_seconds)
            }

            ResourceType::ApiCall {
                endpoint, method, ..
            } => write!(f, "ApiCall({} {})", method, endpoint),
            ResourceType::ApiDataTransfer { bytes, direction } => {
                write!(f, "ApiDataTransfer({:?} {}B)", direction, bytes)
            }
            ResourceType::ApiWebhookDelivery { destination, .. } => {
                write!(f, "ApiWebhookDelivery({})", destination)
            }
            ResourceType::ApiBulkOperation {
                operation_type,
                records_processed,
                ...
                write!(
                    f,
                    "ApiBulkOperation({} {})",
                    operation_type, records_processed
                )
            }

            ResourceType::AdminWalletManagement { operation } => {
                write!(f, "AdminWalletManagement({})", operation)
            }
            ResourceType::AdminPlanManagement { operation } => {
                write!(f, "AdminPlanManagement({})", operation)
            }
            ResourceType::AdminSystemOperation { operation } => {
                write!(f, "AdminSystemOperation({})", operation)
            }
            ResourceType::AdminDataExport { export_type, .. } => {
                write!(f, "AdminDataExport({})", export_type)
            }

            ResourceType::DatabaseQuery { query_type, .. } => {
                write!(f, "DatabaseQuery({:?})", query_type)
            }
            ResourceType::CacheOperation { operation, .. } => {
                write!(f, "CacheOperation({:?})", operation)
            }
            ResourceType::FileStorage { operation, .. } => {
                write!(f, "FileStorage({:?})", operation)
            }
            ResourceType::ComputeTime {
                duration_ms,
                cpu_cores,
            } => write!(f, "ComputeTime({}ms, {}cores)", duration_ms, cpu_cores),
        }
    }
}
