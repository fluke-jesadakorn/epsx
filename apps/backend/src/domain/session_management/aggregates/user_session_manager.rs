...
impl UserSessionManager {
  ...
  pub fn add_session(
    ...
    self.session_collection
      .add_session(session_metadata.clone())
      ...
      self.global_activity.record_activity(
        ...
        Some(
          format!(
            ...
            session_metadata.provider_type
          )
        )
      );
    }
  ...
  pub fn record_session_activity(
    ...
    if self.enable_security_monitoring {
      ...
      if !patterns.is_empty() {
        ...
        let converted_patterns: Vec<SuspiciousPattern> = patterns
          .into_iter()
          .filter_map(|_p| {
          ...
          .collect();
        self.handle_suspicious_patterns(
          converted_patterns.as_slice(),
          session_id
        )?;
      }
    }
  ...
  pub fn terminate_session(
    ...
    let session = self.session_collection
  ...
  pub fn terminate_all_sessions(
    ...
    let terminated_count = self.session_collection.terminate_all_sessions(
      reason.clone()
    );
  ...
  pub fn update_session_device(
    ...
    let device_changed = session.device_info
      .as_ref()
      .map(
        |existing|
          existing.device_fingerprint.hash !=
          device_info.device_fingerprint.hash
      )
      .unwrap_or(false);
  ...
  pub fn perform_security_assessment(&self) -> SessionSecurityAssessment {
    ...
    let converted_patterns: Vec<SuspiciousPattern> = suspicious_patterns
      .into_iter()
      .filter_map(|_p| {
        SuspiciousPattern::new(
          crate::domain::session_management::value_objects::PatternType::VelocityAnomaly,
  ...
  fn validate_session_security(
    ...
    let suspicious_count = self.session_collection
  ...
  fn handle_suspicious_patterns(
    &mut self,
    _patterns: &[super::super::value_objects::SuspiciousPattern],
    session_id: &SessionId
  ) -> Result<(), SessionManagerError> {
  ...
  fn calculate_risk_score(
    &self,
    suspicious_sessions: &[&SessionMetadata],
    patterns: &[super::super::value_objects::SuspiciousPattern]
  ) -> f64 {
    let mut score = 0.0;
  ...
  fn generate_security_recommendations(
    ...
    match risk_level {
      RiskLevel::Critical | RiskLevel::High => {
        recommendations.push(
          "Consider terminating all sessions and requiring re-authentication".to_string()
        );
        recommendations.push(
          "Enable additional security monitoring".to_string()
        );
        recommendations.push(
          "Require multi-factor authentication for new sessions".to_string()
        );
      }
      RiskLevel::Medium => {
        ...
        if !suspicious_sessions.is_empty() {
          recommendations.push(
            "Consider terminating suspicious sessions".to_string()
          );
        }
      }
      RiskLevel::Low => {
    ...
    if self.active_session_count() > 5 {
      recommendations.push(
        "User has many active sessions - consider session cleanup".to_string()
      );
    }
...
impl AggregateRoot for UserSessionManager {
  ...
  fn uncommitted_events(
    &self
  ) -> &[Box<dyn crate::domain::shared_kernel::DomainEvent>] {
...
pub enum SessionManagerError {
  ...
  #[error("Session collection error: {0}")] CollectionError(
    #[from] super::super::value_objects::SessionCollectionError,
  ),
...
mod tests {
  ...
  fn add_session_to_manager() {
    ...
    let session = create_test_session(user_id, ProviderType::Web3);
  ...
  fn terminate_session() {
    ...
    let result = manager.terminate_session(
      &session_id,
