...
impl CommandHandler<UpdateWalletCommand> for UpdateWalletCommandHandler {
    async fn handle(
        ...
        tracing::info!(
            "Processing UpdateWalletCommand for wallet_address: {}",
            command.wallet_address.to_string()
        );
        ...
        let mut user = self
            ...
            .ok_or_else(|| {
                ApplicationError::not_found("User", command.wallet_address.to_string())
            })?;

        // Web3-first: Email operations not supported
        if command.email.is_some() {
            return Err(ApplicationError::not_implemented(
                "Email updates not supported in Web3-first architecture",
            ));
        }
        ...
        if let Some(permissions) = &command.permissions {
            ...
            for permission_str in permissions {
                let permission = crate::domain::wallet_management::value_objects::Permission::new(
        ...
        if let Some(is_active) = command.is_active {
            if is_active {
                user.activate().map_err(ApplicationError::from)?;
            } else {
        ...
        if command.email_verified.is_some() {
            return Err(ApplicationError::not_implemented(
                "Email verification not supported in Web3-first architecture",
            ));
        }
