...
impl CommandHandler<DeleteWalletCommand> for DeleteWalletCommandHandler {
    async fn handle(
    ...
    ) -> ApplicationResult<DeleteWalletResponse> {
        tracing::info!(
            "Processing DeleteWalletCommand for wallet_address: {}",
        ...
        let mut wallet = self
            ...
            .ok_or_else(|| {
                ApplicationError::not_found("Wallet", command.wallet_address.to_string())
            })?;
        ...
        // Create response
        Ok(DeleteWalletResponse::new(format!(
