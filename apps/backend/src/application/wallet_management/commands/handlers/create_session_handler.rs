...
impl CommandHandler<CreateSessionCommand> for CreateSessionCommandHandler {
  async fn handle(
    ...
    let wallet = self.user_repository
      .find_by_wallet(&wallet_addr).await
      .map_err(|e| ApplicationError::infrastructure(e.to_string()))?
      .ok_or_else(||
        ApplicationError::not_found("Wallet", wallet_addr.to_string())
      )?;

    if !wallet.is_active() {
      return Err(
        ApplicationError::authorization(
          "Cannot create session for inactive wallet"
        )
      );
    }

    // 2. Generate session ID
    let sid = self.session_repository
      .next_identity().await
      .map_err(|e| ApplicationError::infrastructure(e.to_string()))?;
