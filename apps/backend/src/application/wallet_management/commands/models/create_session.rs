...
use epsx_contracts::value_objects::{SessionId, UserId};

use crate::application::shared::{ApplicationResult, Command, ValidationUtils};
...
impl Command for CreateSessionCommand {
    ...
    fn validate(&self) -> ApplicationResult<()> {
        ...
        if let Some(error) = ValidationUtils::required("access_token", &self.access_token) {
            return Err(crate::application::ApplicationError::validation(
                &error.field,
                &error.message,
            ));
...
impl CreateSessionCommand {
    ...
    pub fn new(wallet_address: String, access_token: String, expires_at: DateTime<Utc>) -> Self {
        Self {
            wallet_address,
            access_token,
            refresh_token: None,
...
mod tests {
    ...
    fn create_session_command_validation_emptyuser_id() {
        let future_time = Utc::now() + Duration::hours(1);
        let command =
            CreateSessionCommand::new("".to_string(), "access_token_123".to_string(), future_time);
