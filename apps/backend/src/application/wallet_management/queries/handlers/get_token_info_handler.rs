...
use crate::domain::wallet_management::{SessionRepositoryPort, WalletUserRepositoryPort};
...
impl QueryHandler<GetTokenInfoQuery> for GetTokenInfoQueryHandler {
    async fn handle(&self, query: GetTokenInfoQuery) -> ApplicationResult<GetTokenInfoResponse> {
        // 1. Validate query
        query.validate()?;

        // 2. Find session by access token
        let session = self.session_repository
            .find_by_access_token(&query.token)
            .await
            .map_err(|e| ApplicationError::infrastructure(e.to_string()))?
            .ok_or_else(|| ApplicationError::not_found("Token", "Invalid or expired token".to_string()))?;
        ...
        let wallet = self.wallet_repository
            .find_by_wallet(session.user_id())
            .await
            .map_err(|e| ApplicationError::infrastructure(e.to_string()))?
            .ok_or_else(|| ApplicationError::not_found("Wallet", session.user_id().to_string()))?;

        let permissions: Vec<String> = wallet.permissions()
            .iter()
