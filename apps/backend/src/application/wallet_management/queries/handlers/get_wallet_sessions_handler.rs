...
use crate::application::wallet_management::queries::models::{
    GetWalletSessionsQuery, GetWalletSessionsResponse
};
...
impl QueryHandler<GetWalletSessionsQuery> for GetWalletSessionsQueryHandler {
    async fn handle(&self, query: GetWalletSessionsQuery) -> ApplicationResult<GetWalletSessionsResponse> {
        // 1. Validate query
        query.validate()?;
        ...
        let sessions = self.session_repository
            .find_by_wallet_id(&wallet_addr.to_user_id())
            .await
        ...
        let active_sessions: Vec<crate::application::wallet_management::queries::models::get_wallet_sessions::SessionSummary> = sessions
            .iter()
            .filter(|s| s.is_valid())
            .map(|s| crate::application::wallet_management::queries::models::get_wallet_sessions::SessionSummary {
        ...
        let expired_sessions: Vec<crate::application::wallet_management::queries::models::get_wallet_sessions::SessionSummary> = if query.include_expired {
            sessions
                .iter()
                .filter(|s| !s.is_valid())
                .map(|s| crate::application::wallet_management::queries::models::get_wallet_sessions::SessionSummary {
