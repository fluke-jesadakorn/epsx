...
impl UserApplicationService {
  ...
  pub fn new(
    ...
    let create_user_handler = Arc::new(
      CreateUserCommandHandler::new(user_repository.clone(), event_bus.clone())
    );

    let update_user_handler = Arc::new(
      UpdateUserCommandHandler::new(user_repository.clone(), event_bus.clone())
    );

    let delete_user_handler = Arc::new(
      DeleteUserCommandHandler::new(user_repository.clone(), event_bus.clone())
    );

    let grant_permission_handler = Arc::new(
      GrantPermissionCommandHandler::new(
  ...
  pub async fn register_user_with_session(
    ...
    let create_user_cmd = CreateUserCommand::new_web3(email)
      .with_permissions(
        vec!["epsx:analytics:view".to_string(), "epsx:user:read".to_string()]
      )
