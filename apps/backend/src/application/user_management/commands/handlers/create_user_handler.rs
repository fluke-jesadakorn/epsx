...
use crate::application::user_management::commands::models::{
  CreateUserCommand,
  CreateUserResponse,
};
...
impl CommandHandler<CreateUserCommand> for CreateUserCommandHandler {
  async fn handle(
    ...
    let email = Email::new(&command.email).map_err(|e|
      ApplicationError::validation("email", e.to_string())
    )?;
    ...
    if let Ok(Some(_)) = self.user_repository.find_by_email(&email).await {
      return Err(
        ApplicationError::conflict("User with this email already exists")
      );
    }
    ...
    let mut user = User::create_web3_user(user_id, email.clone()).map_err(
    ...
    if !command.initial_permissions.is_empty() {
      for permission_str in &command.initial_permissions {
        let permission = Permission::new(permission_str).map_err(|e|
          ApplicationError::validation("initial_permissions", e.to_string())
        )?;

        // Convert command.initiated_by to UserId if present
        let granted_by = if
          let Some(ref initiated_by_str) = command.initiated_by
        {
          Some(
            UserId::from_string(initiated_by_str.clone()).map_err(|e|
              ApplicationError::validation("initiated_by", e.to_string())
            )?
          )
        } else {
          None
        ...
        user
          .grant_permission(permission, granted_by)
    ...
    if command.email_verified == Some(true) {
      user.verify_email().map_err(ApplicationError::from)?;
    }
...
mod tests {
  ...
  // FirebaseUid import removed - migrated to Web3
  ...
  mock! {
    ...
    impl UserRepositoryPort for UserRepo {
      async fn find_by_id(
        &self,
        id: &UserId
      ) -> Result<Option<User>, crate::domain::DomainError>;
      async fn find_by_email(
        &self,
        email: &Email
      ) -> Result<Option<User>, crate::domain::DomainError>;
      // find_by_firebase_uid removed - migrated to Web3
      async fn save(
        &self,
        user: &User
      ...
      ) -> Result<Vec<User>, crate::domain::DomainError>;
      async fn find_by_criteria(
        &self,
        criteria: &crate::domain::user_management::UserSearchCriteria,
        limit: u32,
        offset: u32
      ) -> Result<
        crate::domain::user_management::UserSearchResult,
        crate::domain::DomainError
      >;
      async fn count_by_criteria(
        &self,
        criteria: &crate::domain::user_management::UserSearchCriteria
      ) -> Result<u64, crate::domain::DomainError>;
      async fn find_eligible_for_auto_assignment(
        &self
      ) -> Result<Vec<User>, crate::domain::DomainError>;
      async fn save_batch(
        &self,
      ...
      async fn next_identity(
        &self
      ) -> Result<UserId, crate::domain::DomainError>;
      async fn health_check(&self) -> Result<(), crate::domain::DomainError>;
      async fn cleanup_expired_permissions(
        &self
      ) -> Result<u32, crate::domain::DomainError>;
    }
  }
  ...
  async fn create_user_handler_success() {
    ...
    // Firebase UID check removed in Web3 migration

    mock_repo.expect_next_identity().returning(move || Ok(newuser_id.clone()));

    mock_repo.expect_save().returning(|_| Ok(()));

    let handler = CreateUserCommandHandler::new(Arc::new(mock_repo), event_bus);

    let command = CreateUserCommand::new("test@example.com".to_string());
  ...
  async fn create_user_handler_email_already_exists() {
    ...
    mock_repo.expect_find_by_email().returning(|_| {
      let existing_user = User::create_web3_user(
        UserId::new(),
