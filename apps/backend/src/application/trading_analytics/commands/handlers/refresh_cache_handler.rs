...
use crate::application::trading_analytics::commands::{
...
impl CommandHandler<RefreshCacheCommand> for RefreshCacheCommandHandler {
    async fn handle(
        ...
        let stats_before = self.tradingview_service.get_cache_stats();
        let entries_before = stats_before.total_count;

        if command.force_full_refresh {
            // Clear entire cache
            self.tradingview_service.clear_cache();
        ...
        let stats_after = self.tradingview_service.get_cache_stats();
