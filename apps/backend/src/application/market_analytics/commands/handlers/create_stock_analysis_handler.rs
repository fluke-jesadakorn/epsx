...
use crate::domain::market_analytics::{
    Country, EPSValue, MarketSector, StockAnalysis, StockAnalysisRepositoryPort, StockSymbol,
};
use crate::prelude::*;
...
impl CommandHandler<CreateStockAnalysisCommand> for CreateStockAnalysisCommandHandler {
    async fn handle(
        &self,
        command: CreateStockAnalysisCommand,
    ) -> ApplicationResult<CreateStockAnalysisResponse> {
            ...
            .map_err(|e| ApplicationError::infrastructure(e.to_string()))?
        {
            return Err(ApplicationError::conflict(
                "Stock analysis already exists for this symbol",
            ));
        }
