...
impl std::fmt::Display for SecurityError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            SecurityError::MonitoringFailed(msg) => {
                write!(f, "Security monitoring failed: {}", msg)
            }
    ...
    async fn handle_threat_response(
        ...
        match threat_level {
            ...
            ThreatLevel::High => {
                ...
                self.apply_temporary_restrictions(&context.wallet_address, Duration::minutes(15))
                    .await?;
            }
            ThreatLevel::Critical => {
                ...
                self.block_user(&context.wallet_address, Duration::hours(1))
                    .await?;
...
mod tests {
    ...
    async fn test_threat_escalation() {
        ...
        let threat1 = service
            .analyze_security_event(SecurityEvent::InvalidJwtAttempt, context.clone())
            .await
            .unwrap();
    ...
    async fn test_user_blocking() {
        ...
        service
            .analyze_security_event(SecurityEvent::PermissionIntegrityViolation, context.clone())
            .await
            .unwrap();
