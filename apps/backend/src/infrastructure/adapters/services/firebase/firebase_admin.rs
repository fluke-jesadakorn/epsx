...
impl FirebaseAdmin {
  ...
  pub async fn authenticate_user(
    ...
    // For development, check environment variables for test credentials
    if
      let (Ok(test_email), Ok(test_password)) = (
        std::env::var("TEST_ADMIN_EMAIL"),
  ...
  pub async fn update_user(
    ...
    if let Some(disabled) = disabled {
      tracing::info!(
        "Updated disabled status for user {}: {}",
        firebase_uid,
        disabled
      );
    }
  ...
  pub async fn set_user_role(
    ...
    tracing::info!(
      "Setting role '{}' for Firebase user: {}",
      role,
      firebase_uid
    );
  ...
  ) -> Result<(Vec<FirebaseUser>, Option<String>), FirebaseError> {
    tracing::info!(
      "Listing Firebase users - max_results: {:?}, page_token: {:?}",
      max_results,
      page_token
    );
...
pub enum FirebaseError {
  #[error("Token verification failed: {0}")] TokenVerificationFailed(String),
