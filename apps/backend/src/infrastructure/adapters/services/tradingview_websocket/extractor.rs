...
use super::types::{EPSWebSocketData, PriceData, QuarterlyEPSData};
...
pub fn extract_eps_from_st4(
    ...
    for (index, entry) in st4_array.iter().enumerate() {
        if let Some(v) = entry["v"].as_array() {
            ...
            let earnings_timestamp = v
                .first()
                .and_then(|v| v.as_f64())
                .map(|f| f as i64)
                .unwrap_or(0);
            let estimated_eps = v.get(1).and_then(|v| v.as_f64());
            let quarter_end_timestamp = v.get(3).and_then(|v| v.as_f64()).map(|f| f as i64);
            let earnings_announcement_timestamp_ms =
                v.get(4).and_then(|v| v.as_f64()).map(|f| f as i64);
            let actual_eps = v.get(5).and_then(|v| v.as_f64());

            let is_future_earnings = i_value > 100;
            if is_future_earnings {
                info!(
                    "FOUND FUTURE EARNINGS: i={}, announcement timestamp: {:?}",
            ...
            if let Some(eps_value) = actual_eps_value {
                ...
                let quarter_end_date = quarter_end_timestamp.map(|ts| {
                    chrono::Utc
                        .timestamp_opt(ts, 0)
                        .single()
                        .unwrap_or_default()
                ...
                let stored_estimated_eps =
                    estimated_eps.filter(|&est| (est - eps_value).abs() > 0.01);
                ...
                quarterly_data.push(QuarterlyEPSData {
                    ...
                    beat_estimate: None,
                    eps_type: if is_future_earnings {
                        "st4_future_estimate".to_string()
                    } else {
                        "st4_earnings_study".to_string()
                    },
                    source: if is_future_earnings {
                        "st4_future_calendar".to_string()
                    } else {
                        "st4_earnings_data".to_string()
                    },
