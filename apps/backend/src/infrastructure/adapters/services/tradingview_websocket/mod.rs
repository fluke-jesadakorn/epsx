...
impl TradingViewWebSocketService {
    ...
    pub async fn connect_and_fetch_eps_data(
        ...
        let ws_stream =
            connection::connect_websocket(&self.websocket_url, &self.origin_url).await?;
        ...
        for symbol in &symbols {
            ...
            match self
                .extract_symbol_eps_data(&mut write, &mut read, symbol)
                .await
            {
                Ok(eps_data) => {
    ...
    async fn extract_symbol_eps_data(
        ...
        self.process_messages_until_complete(read, symbol, write)
            .await
    }
        ...
        match extraction_timeout {
            ...
            Err(_) => {
                ...
                Err(AppError::internal_server_error(format!(
                    "WebSocket extraction timeout for {}",
                    symbol
                )))
            }
        }
        ...
        if let Some(params) = parsed["p"].as_array() {
            if params.len() >= 2 {
                if let Some(st4_array) = params[1]["st4"].as_array() {
                    ...
                    let quarterly_data =
                        extractor::extract_eps_from_st4(st4_array, symbol, self.debug, |data| {
                            self.correlate_price_with_earnings(data)
                        });
            ...
            if params.len() >= 2 {
                if let Some(st4_data) = params[1]["st4"].as_object() {
                    if let Some(st_array) = st4_data["st"].as_array() {
                        ...
                        let quarterly_data =
                            extractor::extract_eps_from_st4(st_array, symbol, self.debug, |data| {
                                self.correlate_price_with_earnings(data)
                            });
