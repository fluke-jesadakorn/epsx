...
use crate::application::ports::outbound::service_ports::{
    SecurityEvent, SecurityMonitoringServicePort, ThreatLevel,
};
...
impl SecurityMonitoringServiceAdapter {
    ...
    async fn update_user_security_metrics(
        ...
        self.cache.set(
            &metrics_key,
            serialized,
            Some(Duration::days(30).num_seconds() as u64),
        );
...
impl SecurityMonitoringServicePort for SecurityMonitoringServiceAdapter {
    ...
    async fn log_security_event(&self, event: SecurityEvent) -> Result<(), Self::Error> {
        ...
        if let Some(wallet_address) = &event.wallet_address {
            self.update_user_security_metrics(wallet_address, "security_event", 1)
                .await
    ...
    async fn check_threat_level(&self, ip: &str) -> Result<ThreatLevel, Self::Error> {
        ...
        match self.cache.get(&ip_rep_key) {
            ...
            None => {
                debug!(
                    ip = ip,
                    "No reputation data found for IP, assuming low threat"
                );
                Ok(ThreatLevel::Low)
            }
        }
