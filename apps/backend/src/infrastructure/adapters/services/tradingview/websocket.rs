...
use super::types::{FrontendEPSData, MarketDataError, TradingViewConfig};
...
impl TradingViewWebSocketHandler {
    ...
    pub async fn create_session_with_symbols(
        ...
        debug!(
            "Creating WebSocket session: {} for {} symbols",
            sid,
            symbols.len()
        );
    ...
    pub async fn subscribe_to_symbols(
        ...
        debug!(
            "Subscribing to {} symbols for session: {}",
            symbols.len(),
            sid
        );
        ...
        info!(
            "Subscribed to {} symbols for session: {}",
            symbols.len(),
            sid
        );
        Ok(())
    ...
    pub async fn start_message_loop<F>(&self, message_handler: F) -> Result<(), MarketDataError>
    where
