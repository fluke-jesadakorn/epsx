...
use super::types::{StockDataField, TradingViewStock};
use super::utils::{extract_symbol, get_number, get_number_opt, get_string};
...
impl TradingViewMapper {
    ...
    pub fn convert_to_eps_growth_data(stock: TradingViewStock) -> StockDataResult<EPSGrowthData> {
        ...
        let eps_data = EPSGrowthData::new(
            crate::domain::shared_kernel::entities::eps_growth::EPSGrowthParams {
                symbol: symbol.clone(),
                name,
        ...
        debug!(
            "Successfully converted TradingView data to EPS data for: {}",
            symbol
        );
        Ok(eps_data)
    }
    ...
    fn select_best_quarterly_eps_field(
        candidates: Vec<(usize, &str, &str, f64)>,
        symbol: &str,
    ) -> Option<f64> {
        ...
        for (idx, name, _, value) in &valid_candidates {
            if *idx == 14 {
                info!(
                    "Selected primary quarterly EPS field {} for {}: {}",
                    name, symbol, value
                );
                return Some(*value);
            }
        ...
        info!(
            "Selected fallback quarterly EPS field {} for {}: {}",
            name, symbol, value
        );
        Some(*value)
    }
    ...
    pub fn merge_scanner_and_websocket_data(
        ...
        for scanner_item in scanner_data {
            ...
            if let Some(websocket_item) = websocket_data
                .iter()
                .find(|ws| ws.symbol == scanner_item.symbol)
            {
                ...
                let merged_item = FrontendEPSData {
                    ...
                    current_eps: if websocket_item.current_eps > 0.0 {
                        websocket_item.current_eps
                    } else {
                        scanner_item.current_eps
                    },
                    qoq_growth: if websocket_item.qoq_growth != 0.0 {
                        websocket_item.qoq_growth
                    } else {
                        scanner_item.qoq_growth
                    },
                    market_cap: if websocket_item.market_cap > 0 {
                        websocket_item.market_cap
                    } else {
                        scanner_item.market_cap
                    },
                    price_current: if websocket_item.price_current > 0.0 {
                        websocket_item.price_current
                    } else {
                        scanner_item.price_current
                    },
                    volume: if websocket_item.volume > 0 {
                        websocket_item.volume
                    } else {
                        scanner_item.volume
                    },
                    country: scanner_item.country, // Keep scanner country
                    sector: scanner_item.sector,   // Keep scanner sector
                    currency: if !websocket_item.currency.is_empty()
                        && websocket_item.currency != "USD"
                    {
                    ...
                    ranking_score: Self::calculate_ranking_score(
                        ...
                        if websocket_item.qoq_growth != 0.0 {
                            websocket_item.qoq_growth
                        } else {
                            scanner_item.qoq_growth
                        },
                        if websocket_item.market_cap > 0 {
                            websocket_item.market_cap as f64
                        } else {
                            scanner_item.market_cap as f64
                        },
                        if websocket_item.price_current > 0.0 {
                            websocket_item.price_current
                        } else {
                            scanner_item.price_current
                        },
                    ),
                    next_earnings_date: websocket_item
                        .next_earnings_date
                        .clone()
                        .or(scanner_item.next_earnings_date.clone()),
                    last_earnings_date: websocket_item
                        .last_earnings_date
                        .clone()
                        .or(scanner_item.last_earnings_date.clone()),
                };
        ...
        debug!(
            "Merged {} scanner items with {} WebSocket items",
            merged_data.len(),
            websocket_data.len()
        );
        merged_data
...
mod tests {
    ...
    fn test_eps_value_validation() {
        ...
        assert!(!TradingViewMapper::is_valid_quarterly_eps_value(
            f64::INFINITY
        ));
    }
    ...
    fn test_quarterly_eps_selection() {
        let candidates = vec![
            ...
            (22, "earnings_per_share_forecast_fq", "quarterly", 3.5),
        ];
    ...
    fn test_data_merging() {
        let scanner_data = vec![FrontendEPSData {
            id: "1".to_string(),
            symbol: "AAPL".to_string(),
            company_name: "Apple Inc".to_string(),
            current_eps: 3.25,
        ...
        let websocket_data = vec![FrontendEPSData {
            ...
            symbol: "AAPL".to_string(),
            company_name: "Apple Inc".to_string(),
            current_eps: 3.3,     // Updated EPS from WebSocket
        ...
        let merged =
            TradingViewMapper::merge_scanner_and_websocket_data(scanner_data, websocket_data);
        assert_eq!(merged.len(), 1);

        let merged_item = &merged[0];
        assert_eq!(merged_item.current_eps, 3.3); // Should use WebSocket EPS
