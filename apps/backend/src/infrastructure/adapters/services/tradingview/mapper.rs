...
impl TradingViewMapper {
  ...
  pub fn convert_to_eps_growth_data(
    stock: TradingViewStock
  ...
  fn detect_quarterly_eps_dynamically(
    data: &[StockDataField],
    symbol: &str
  ) -> Option<f64> {
    let get_number = |data: &[StockDataField], idx: usize| -> Option<f64> {
  ...
  pub fn merge_scanner_and_websocket_data(
    ...
    for scanner_item in scanner_data {
      ...
      if
        let Some(websocket_item) = websocket_data
          .iter()
  ...
  pub fn batch_convert_to_frontend(
    stocks: Vec<TradingViewStock>
  ) -> Vec<FrontendEPSData> {
    stocks.into_iter().map(Self::map_to_frontend_eps_data).collect()
  }
...
mod tests {
  ...
  fn test_ranking_score_calculation() {
    let score = TradingViewMapper::calculate_ranking_score(
      3.25,
      15.5,
      2_000_000_000.0,
      150.0
    ...
    let zero_score = TradingViewMapper::calculate_ranking_score(
      0.0,
      0.0,
      0.0,
      0.0
  ...
  fn test_quarterly_eps_selection() {
    ...
    let result = TradingViewMapper::select_best_quarterly_eps_field(
      candidates,
      "AAPL"
    );
