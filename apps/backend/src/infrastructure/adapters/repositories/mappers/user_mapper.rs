...
use crate::domain::user_management::{ User, Email, Permission };
...
impl UserMapper {
  ...
  /// Convert domain aggregate to new database model (Web3-first)
  pub fn to_new_db(user: &User) -> DomainResult<NewDbUser> {
    Ok(NewDbUser {
      email: user.email().to_string(),
      // Web3-first user creation without Firebase dependencies
    })
  }

  /// Convert domain aggregate to update database model
  pub fn to_update_db(user: &User) -> UpdateDbUser {
    UpdateDbUser {
