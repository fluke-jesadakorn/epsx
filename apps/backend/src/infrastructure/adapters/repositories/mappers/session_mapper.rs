...
use crate::domain::wallet_management::{ Session };
use crate::infrastructure::adapters::repositories::database_types::{
  Session as DbSession,
  NewSession as NewDbSession,
...
impl SessionMapper {
  ...
  fn sid_to_uuid(sid: &str) -> Uuid {
    // First try to parse as regular UUID
    if let Ok(uuid) = Uuid::from_str(sid) {
      return uuid;
    }
  ...
  pub fn to_domain(db_session: DbSession) -> AppResult<Session> {
    let sid = SessionId::from_string(db_session.id.to_string());
    let wallet_address = WalletAddress::new(
      db_session.wallet_address.to_string()
    )?;
  ...
  pub fn to_new_db(session: &Session) -> AppResult<NewDbSession> {
    let session_uuid = Self::sid_to_uuid(&session.id().to_string());
    let user_uuid = Self::sid_to_uuid(&session.user_id().to_string());
    ...
    let ip_address = match session.ip_address() {
      Some(ip_str) if !ip_str.is_empty() => {
        ...
        let _: std::net::IpAddr = ip_str
          .parse()
          .map_err(|e|
            crate::core::errors::AppError::validation_error(
