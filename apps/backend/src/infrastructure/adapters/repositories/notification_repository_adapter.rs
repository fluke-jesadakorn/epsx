...
use crate::infrastructure::adapters::services::fcm_service::{
  FcmService,
  FcmNotification,
  FcmResponse,
  DeliveryStats,
...
impl NotificationRepositoryAdapter {
            ...
            match self.deliver_via_fcm(notification, token).await {
              Ok(response) => {
                delivery_results.push(DeliveryResult::Success {
                  ...
                  message_id: Some(
                    Self::extract_fcm_message_id(&response.name)
                  ),
                });
                info!(
                  "FCM delivery successful for notification {}",
                  notification.id().as_str()
                );
              }
              Err(e) => {
                warn!(
                  "FCM delivery failed for notification {}: {}",
                  notification.id().as_str(),
                  e
                );
          ...
          if let Some(user_email) = &email {
            match self.deliver_via_email(notification, user_email).await {
              Ok(()) => {
                ...
                info!(
                  "Email delivery successful for notification {}",
                  notification.id().as_str()
                );
              }
              Err(e) => {
  ...
  pub async fn deliver_notification_to_topic(
    ...
    debug!(
      "Delivering notification {} to topic {}",
      notification.id().as_str(),
      topic_name
    );
    ...
    match self.deliver_topic_via_fcm(notification, topic_name).await {
      Ok(response) => {
        info!(
          "FCM topic delivery successful for notification {}",
          notification.id().as_str()
        );
      ...
      Err(e) => {
        warn!(
          "FCM topic delivery failed for notification {}: {}",
          notification.id().as_str(),
          e
        );
  ...
  pub async fn deliver_notification_multicast(
    ...
    debug!(
      "Delivering notification {} to {} tokens",
      notification.id().as_str(),
      fcm_tokens.len()
    );
    ...
    match
      self.fcm_service.send_multicast(fcm_tokens, fcm_notification, data).await
    {
      Ok(stats) => {
        info!(
          "Multicast delivery completed: {} sent, {} failed",
          stats.sent,
          stats.failed
        );
        Ok(stats)
  ...
  fn convert_to_fcm_notification(
    &self,
    notification: &Notification
  ) -> FcmNotification {
    FcmNotification {
  ...
  fn build_fcm_data(
    ...
    data.insert(
      ...
      serde_json::Value::String(notification.id().as_str().to_string())
    );
    data.insert(
      "notification_type".to_string(),
      serde_json::Value::String(notification.notification_type().to_string())
    );
    data.insert(
      "priority".to_string(),
      serde_json::Value::String(notification.priority().to_string())
    );
    ...
    if let Some(action_url) = notification.metadata().action_url() {
      data.insert(
        ...
        serde_json::Value::String(action_url.to_string())
      );
    }
  ...
  async fn deliver_via_fcm(
    ...
    let fcm_message =
      crate::infrastructure::adapters::services::fcm_service::FcmMessage {
        ...
        android: Some(
          crate::infrastructure::adapters::services::fcm_service::FcmAndroidConfig {
        ...
        webpush: Some(
          crate::infrastructure::adapters::services::fcm_service::FcmWebpushConfig {
            headers: Some(
              serde_json::json!({"Urgency": self.map_priority_to_urgency(notification.priority())})
            ),
            data: None,
            notification: None,
            fcm_options: notification
              .metadata()
              .action_url()
              .map(|url| {
                crate::infrastructure::adapters::services::fcm_service::FcmWebpushFcmOptions {
                  link: Some(url.to_string()),
                  analytics_label: Some(
                    format!("notification_{}", notification.notification_type())
                  ),
                }
              }),
          }
  ...
  async fn deliver_via_email(
    ...
    match notification.notification_type() {
      ...
      NotificationType::Security => {
        ...
        let reset_link = notification
          .metadata()
          .action_url()
          .unwrap_or("https://epsx.io/reset");
      ...
      _ => {
        ...
        warn!(
          "Email delivery not implemented for notification type: {}",
          notification.notification_type()
        );
        Err(
          EmailServiceError::DeliveryFailed(
            "Notification type not supported for email".to_string()
          )
        )
      }
    }
...
mod tests {
  ...
  fn test_fcm_notification_conversion() {
    ...
    let fcm_service = Arc::new(
      FcmService::new(
        Arc::new(
          crate::infrastructure::firebase_admin::FirebaseAdmin::new_mock()
        )
      )
    );
    let email_service = Arc::new(
      SendGridEmailService::new(Arc::new(crate::config::Config::default()))
    );
    ...
    assert_eq!(
      ...
      Some("This is a test notification body".to_string())
    );
  ...
  fn test_priority_mapping() {
    ...
    assert_eq!(
      adapter.map_priority_to_fcm(&NotificationPriority::Urgent),
      "high"
    );
    assert_eq!(
      adapter.map_priority_to_fcm(&NotificationPriority::High),
      "high"
    );
    assert_eq!(
      adapter.map_priority_to_fcm(&NotificationPriority::Normal),
      "normal"
    );
    assert_eq!(
      adapter.map_priority_to_fcm(&NotificationPriority::Low),
      "normal"
    );

    assert_eq!(
      adapter.map_priority_to_urgency(&NotificationPriority::Urgent),
      "high"
    );
    assert_eq!(
      adapter.map_priority_to_urgency(&NotificationPriority::High),
      "normal"
    );
    assert_eq!(
      adapter.map_priority_to_urgency(&NotificationPriority::Normal),
      "low"
    );
    assert_eq!(
      adapter.map_priority_to_urgency(&NotificationPriority::Low),
      "very-low"
    );
  }
}
