  ...
  fn map_to_legacy(
    ...
    LegacySessionMapping {
      ...
      refresh_token: session
        ...
        .map(|t| t.token().to_string()),
      id_token: session.current_id_token().map(|t| t.token().to_string()),
    }
  }

  /// Convert legacy session data to DDD AuthenticationSession
  async fn map_from_legacy(
    &self,
    legacy_data: LegacySessionData
    ...
    let session_id = SessionId::from_string(legacy_data.session_id).map_err(|e|
...
#[async_trait]
impl AuthenticationSessionRepositoryPort
for AuthenticationSessionRepositoryAdapter {
  async fn save(&self, session: &AuthenticationSession) -> Result<(), String> {
    ...
    let session_data =
      crate::domain::shared_kernel::entities::session::Session {
        id: Some(legacy_mapping.session_id.parse().unwrap_or_default()),
        user_id: legacy_mapping.user_id.parse().unwrap_or_default(),
        // Legacy identifier replaced with user_id for Web3-first architecture
    ...
    self.legacy_session_repo
      .create_session(session_data).await
      .map_err(|e|
        format!("Failed to save session to legacy repository: {}", e)
      )?;
    ...
    match self.legacy_session_repo.find_session_by_id(session_id_i32).await {
      Ok(Some(legacy_session)) => {
        ...
        let legacy_data = LegacySessionData {
          ...
          last_activity: legacy_session.last_accessed.unwrap_or(
  ...
  async fn find_by_user(
    ...
    match
      self.legacy_session_repo.find_sessions_byuser_id(numericuser_id).await
    {
      Ok(legacy_sessions) => {
  ...
  async fn delete(&self, session_id: &SessionId) -> Result<(), String> {
    ...
    self.legacy_session_repo
      .delete_session(session_id_i32).await
      .map_err(|e|
        format!("Failed to delete session from legacy repository: {}", e)
      )?;
...
impl AuthenticationSessionRepositoryAdapter {
  ...
  async fn store_ddd_session_metadata(
    &self,
    session: &AuthenticationSession
  ) -> Result<(), String> {
  ...
  async fn delete_ddd_session_metadata(
    &self,
    session_id: &SessionId
  ) -> Result<(), String> {
...
mod tests {
  ...
  impl SessionRepository for MockLegacySessionRepository {
    async fn create_session(
      &self,
      _session: crate::domain::shared_kernel::entities::session::Session
    ) -> Result<
      crate::domain::shared_kernel::entities::session::Session,
    ...
    > {
      Ok(crate::domain::shared_kernel::entities::session::Session {
        id: Some(1),
        user_id: 123,
        // Test session without legacy Firebase reference
        ip_address: None,
    ...
    async fn find_session_by_id(
      ...
      Option<crate::domain::shared_kernel::entities::session::Session>,
      Box<dyn std::error::Error + Send + Sync>
    > {
      Ok(None)
    }

    async fn find_sessions_byuser_id(
      &self,
      user_id: i32
    ) -> Result<
      Vec<crate::domain::shared_kernel::entities::session::Session>,
      Box<dyn std::error::Error + Send + Sync>
    > {
      Ok(vec![])
    }
  ...
  async fn test_repository_adapter_creation() {
    let mock_pool = Arc::new(
      crate::infrastructure::adapters::repositories
