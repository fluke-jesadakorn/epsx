...
impl TransactionRepositoryAdapter {
  pub fn new(db_pool: Arc<DbPool>, cache: Arc<dyn Cache>) -> Self {
    Self {
      db_pool,
      cache,
    }
  ...
  async fn get_cached_transaction(
    &self,
  ...
  async fn store_transaction_in_db(
    &self,
    record: &TransactionRecord
  ) -> Result<(), String> {
    info!(
  ...
  async fn load_transaction_from_db(
    &self,
    tx_hash: &TransactionHash
  ) -> Result<Option<TransactionRecord>, String> {
...
impl TransactionRepositoryPort for TransactionRepositoryAdapter {
  ...
  async fn find_pending_confirmations(
    &self
    ...
    impl<'de> Visitor<'de> for TransactionRecordVisitor {
        ...
        where V: MapAccess<'de>
      {
        ...
        while let Some(key) = map.next_key()? {
          match key {
            "payment_id" => {
              ...
              payment_id = Some(
                PaymentId::from_string(&value).map_err(|e|
                  de::Error::custom(format!("Invalid payment ID: {}", e))
                )?
              );
            }
            "tx_hash" => {
              ...
              tx_hash = Some(
                TransactionHash::new(
                  value,
                  crate::domain::payment::value_objects::payment_method::Network::Ethereum
                ).map_err(|e|
                  de::Error::custom(format!("Invalid transaction hash: {}", e))
                )?
              );
            }
            "network" => {
            ...
            "required_confirmations" => {
              if required_confirmations.is_some() {
                return Err(
                  de::Error::duplicate_field("required_confirmations")
                );
              }
        ...
        let payment_id = payment_id.ok_or_else(||
          de::Error::missing_field("payment_id")
        )?;
        let tx_hash = tx_hash.ok_or_else(||
          de::Error::missing_field("tx_hash")
        )?;
        let network = network.ok_or_else(||
          de::Error::missing_field("network")
        )?;
        let confirmations = confirmations.ok_or_else(||
          de::Error::missing_field("confirmations")
        )?;
        let required_confirmations = required_confirmations.ok_or_else(||
          de::Error::missing_field("required_confirmations")
        )?;
        let created_at = created_at.ok_or_else(||
          de::Error::missing_field("created_at")
        )?;
...
mod tests {
  ...
  async fn test_transaction_adapter_creation() {
    ...
    let adapter = TransactionRepositoryAdapter::new(mock_pool, mock_cache);
