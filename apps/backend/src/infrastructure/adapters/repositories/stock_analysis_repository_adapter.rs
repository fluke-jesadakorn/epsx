...
use crate::domain::market_analytics::aggregates::eps_ranking::{
    EPSRanking as DDDEPSRanking, RankingEntry, RankingPeriod, RankingType,
};
...
impl StockAnalysisRepositoryAdapter {
    ...
    fn convert_legacy_to_ddd_entry(
        &self,
    ...
    async fn build_ddd_ranking_from_legacy(
        &self,
        params: &EPSRankingParams,
    ) -> Result<DDDEPSRanking, String> {
        debug!(
            "Building DDD ranking from legacy data with params: {:?}",
            params
        );
            ...
            match self.convert_legacy_to_ddd_entry(legacy_ranking) {
                Ok(entry) => {
                    match ddd_ranking.add_entry(
                        ...
                        Err(e) => {
                            error!(
                                "Failed to add entry {} to DDD ranking: {}",
                                entry.symbol.as_str(),
                                e
                            );
                        }
        ...
        info!(
            "Built DDD ranking with {} entries from legacy data",
...
impl StockAnalysisRepositoryAdapter {
    ...
    pub async fn get_ddd_ranking_by_params(
        &self,
        params: &EPSRankingParams,
    ) -> Result<DDDEPSRanking, String> {
    ...
    pub async fn convert_legacy_rankings_to_ddd(
        &self,
        params: EPSRankingParams,
    ) -> Result<Vec<LegacyEPSRanking>, String> {
        debug!(
            "Converting legacy rankings to DDD format with params: {:?}",
            params
        );
        ...
        let rankings: Vec<LegacyEPSRanking> = entries
            .iter()
            .map(|(rank, entry)| self.convert_ddd_to_legacy_ranking(entry, *rank))
            .collect();

        info!(
            "Converted {} DDD entries back to legacy format",
            rankings.len()
        );
        Ok(rankings)
    }
