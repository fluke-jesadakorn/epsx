...
impl SessionRepositoryAdapter {
  ...
  fn map_db_to_session(db: SessionDb) -> Result<Session, AppError> {
    ...
    let wallet_address = WalletAddress::new(wallet_str).map_err(|e|
      AppError::validation_error(format!("Invalid wallet address: {}", e))
    )?;

    let params =
      crate::domain::wallet_management::aggregates::session::SessionLoadParams {
  ...
  fn session_to_new_db(session: &Session) -> NewSessionDb {
    NewSessionDb {
      id: uuid::Uuid
        ::parse_str(session.id().to_string().as_str())
        .unwrap_or_else(|_| uuid::Uuid::new_v4()),
      user_id: uuid::Uuid
        ::parse_str(session.user_id().to_string().as_str())
        .unwrap_or_else(|_| uuid::Uuid::new_v4()),
...
impl SessionRepositoryPort for SessionRepositoryAdapter {
  async fn find_by_id(
    ...
    let session_uuid = uuid::Uuid
      ::parse_str(id.to_string().as_str())
      .map_err(|e|
        AppError::validation_error(format!("Invalid session ID: {}", e))
      )?;
    ...
    let db_session = diesel
      ::sql_query(query)
      .bind::<diesel::sql_types::Uuid, _>(session_uuid)
      .get_result::<SessionDb>(&mut conn).await
      .optional()
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
  ...
  async fn find_by_wallet_id(
    ...
    let wallet_uuid = uuid::Uuid
      ::parse_str(wallet_address.to_string().as_str())
      .map_err(|e|
        AppError::validation_error(format!("Invalid wallet ID: {}", e))
      )?;
    ...
    let db_sessions = diesel
      ::sql_query(query)
      .bind::<diesel::sql_types::Uuid, _>(wallet_uuid)
      .load::<SessionDb>(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation("find_by_wallet_id")
  ...
  async fn find_active_by_wallet_id(
    &self,
    wallet_address: &UserId
  ) -> Result<Vec<Session>, AppError> {
    let mut conn = self.db_pool
  ...
  async fn delete(&self, sid: &SessionId) -> Result<(), AppError> {
    let mut conn = self.db_pool
      .get().await
      .map_err(|e|
        AppError::database_error(e.to_string())
    ...
    let session_uuid = uuid::Uuid
      ::parse_str(sid.to_string().as_str())
      .map_err(|e|
        AppError::validation_error(format!("Invalid session ID: {}", e))
      )?;

    diesel
      ::delete(sessions::table.filter(sessions::id.eq(session_uuid)))
      .execute(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
  ...
  async fn invalidate_all_for_wallet(
    &self,
    wallet_address: &UserId
  ) -> Result<u32, AppError> {
    let mut conn = self.db_pool
  ...
  async fn cleanup_expired(
    ...
    let rows_affected = diesel
      ::delete(sessions::table.filter(sessions::expires_at.lt(before)))
      .execute(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
  ...
  async fn find_by_criteria(
    ...
    let db_sessions: Vec<SessionDb> = if
      let Some(ref wallet) = criteria.wallet_address
    {
      diesel
      ...
      diesel::sql_query(&simple_query).load(&mut conn).await.unwrap_or_default()
    };
    ...
    let mut conn = self.db_pool
      ...
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation(
            format!("save_batch({} sessions)", session_list.len())
          )
      )?;
    ...
    for session in session_list {
      ...
      diesel
        ...
        .map_err(|e| {
          ...
          AppError::database_error(e.to_string()).with_component(
            "session_repository"
          )
        })?;
    }
  ...
  async fn get_session_statistics(
    ...
    let row = diesel
      ::sql_query(query)
      .get_result::<StatsRow>(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation("get_session_statistics")
...
impl SessionAnalyticsPort for SessionRepositoryAdapter {
  async fn get_detailed_statistics(&self) -> Result<(), AppError> {
    ...
    let stats = diesel
      ::sql_query(query)
      .get_result::<DetailedStatsRow>(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation("get_detailed_statistics")
  ...
  async fn get_activity_patterns(&self, days: u32) -> Result<(), AppError> {
    ...
    let patterns = diesel
      ::sql_query(query)
      .load::<ActivityRow>(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation("get_activity_patterns")
  ...
  async fn find_suspicious_sessions(&self) -> Result<(), AppError> {
    ...
    let suspicious = diesel
      ::sql_query(query)
      .load::<SuspiciousRow>(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation("find_suspicious_sessions")
  ...
  async fn get_sessions_by_ip(&self) -> Result<(), AppError> {
    ...
    let ip_groups = diesel
      ::sql_query(query)
      .load::<IpGroupRow>(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation("get_sessions_by_ip")
  ...
  async fn get_duration_distribution(&self) -> Result<(), AppError> {
    ...
    let distribution = diesel
      ::sql_query(query)
      .load::<DurationRow>(&mut conn).await
      .map_err(|e|
        AppError::database_error(e.to_string())
          .with_component("session_repository")
          .with_operation("get_duration_distribution")
