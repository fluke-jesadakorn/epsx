...
use crate::dom::entities::{TemporaryPermission, TemporaryPermissionStatus};
use crate::dom::values::UserId;
use crate::app::ports::repositories::{TemporaryPermissionRepo, TemporaryPermissionQuery, RepoError};
use crate::infra::errors::InfraError;

#[derive(Debug)]
pub struct PgTemporaryPermissionRepository {
    pool: PgPool,
}

impl PgTemporaryPermissionRepository {
...
#[async_trait]
impl TemporaryPermissionRepo for PgTemporaryPermissionRepository {
    ...
    async fn find_by_query(&self, query: &TemporaryPermissionQuery) -> Result<Vec<TemporaryPermission>, RepoError> {
        ...
        if let Some(granted_by) = &query.granted_by {
            query_builder = query_builder.bind(granted_by);
        }
        if let Some(limit) = query.limit {
