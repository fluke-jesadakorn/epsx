...
use super::web3_shared_types::{
    PermissionInfo, Web3PermissionResult
};
...
impl UnifiedWeb3PermissionService {
    ...
    pub async fn get_wallet_permissions(&self, wallet_address: &str) -> Web3PermissionResult<Vec<PermissionInfo>> {
        ...
        let permissions_result = sqlx::query!(
            "SELECT get_wallet_effective_permissions($1) as permissions",
            lowercase_wallet
        )
        .fetch_one(&self.db_pool)
        .await?;

        // Parse the JSONB result to get permission strings
        let permissions_json = permissions_result.permissions.unwrap_or_else(|| serde_json::Value::Array(vec![]));
        
        let permission_strings: Vec<String> = if let serde_json::Value::Array(permissions_array) = permissions_json {
            permissions_array
                .into_iter()
        ...
        let permission_details = self.get_permission_details(&lowercase_wallet).await?;
        ...
        let permissions: Vec<PermissionInfo> = permission_strings
            ...
            .map(|permission| {
                ...
                let detail = permission_details.iter()
        ...
        info!("✅ Found {} effective permissions for wallet: {}", permissions.len(), wallet_address);
        Ok(permissions)
    }
    ...
    pub async fn has_permission(&self, wallet_address: &str, permission: &str) -> Web3PermissionResult<bool> {
        ...
        let result = sqlx::query!(
            "SELECT wallet_has_permission($1, $2) as has_permission",
            lowercase_wallet,
            permission
        )
        .fetch_one(&self.db_pool)
        ...
        let has_permission = result.has_permission.unwrap_or(false);
        
        if has_permission {
            debug!("✅ Permission '{}' granted for wallet: {}", permission, wallet_address);
        } else {
            debug!("❌ Permission '{}' denied for wallet: {}", permission, wallet_address);
        }
    ...
    pub async fn assign_to_group(
        ...
        let membership_id = result.membership_id.unwrap_or_else(|| Uuid::new_v4());
        
        info!("✅ Successfully assigned wallet '{}' to group {} with membership ID: {}", 
              wallet_address, group_id, membership_id);
        Ok(membership_id)
    }
    ...
    pub async fn remove_from_group(
        ...
        if rows_affected > 0 {
            info!("✅ Successfully removed wallet '{}' from group: {}", wallet_address, group_id);
        } else {
    ...
    pub async fn verify_permissions_batch(
        ...
        let effective_permissions = self.get_wallet_permissions(wallet_address).await?;
        ...
        for permission in permissions {
            let has_permission = permission_set.contains(permission) || 
    ...
    pub async fn get_permission_stats(&self, wallet_address: &str) -> Web3PermissionResult<PermissionStats> {
        ...
        let _membership_stats = sqlx::query!(
            r#"
            SELECT 
    ...
    pub async fn get_group_memberships(&self, wallet_address: &str) -> Web3PermissionResult<Vec<GroupMembership>> {
        let lowercase_wallet = wallet_address.to_lowercase();
    ...
    async fn get_permission_details(&self, wallet_address: &str) -> Web3PermissionResult<Vec<PermissionInfo>> {
        let memberships = sqlx::query!(
            r#"
            SELECT 
        ...
        for membership in memberships {
            if let serde_json::Value::Array(permissions_array) = membership.permissions {
                for permission_value in permissions_array {
                    if let Some(permission_str) = permission_value.as_str() {
                        permission_details.push(PermissionInfo {
                            ...
                            permission_type: membership.permission_type.clone(),
                            is_active: true,
    ...
    fn check_wildcard_permission(&self, permission_set: &std::collections::HashSet<String>, requested_permission: &str) -> bool {
