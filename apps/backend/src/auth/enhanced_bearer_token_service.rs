...
impl EnhancedBearerTokenService {
  ...
  pub async fn generate_token(
    ...
    let user_permissions = self.permission_service.get_user_permissions(
      user_id,
      wallet_address
    ).await?;
    ...
    let (web3_permissions, nft_holdings, token_balances, dao_memberships) = if
      let Some(wallet_addr) = wallet_address
    {
    ...
    let rate_limit_tier = self.determine_rate_limit_tier(
      &access_level,
      &nft_holdings,
      &token_balances
    );
    ...
    let token = encode(&header, &claims, &self.encoding_key).map_err(|e|
      anyhow!("Failed to encode token: {}", e)
    )?;

    info!(
      "Generated enhanced Bearer token for user {} with {} permissions",
      user_id,
  ...
  pub async fn validate_token(&self, token: &str) -> Result<TokenValidation> {
    ...
    match decode::<EnhancedTokenClaims>(token, &self.decoding_key, &validation) {
      Ok(token_data) => {
        ...
        let rate_limit_remaining = self.calculate_rate_limit_remaining(
          &claims
        ).await?;
  ...
  pub async fn generate_api_key(
    ...
    let has_permission = self.check_api_key_permission(
      user_id,
      wallet_address
    ).await?;
    if !has_permission {
  ...
  > {
    ...
    let web3_permissions = self.get_web3_permission_claims(
      user_id,
      wallet_address
    ).await?;
  ...
  async fn get_web3_permission_claims(
    &self,
    user_id: &Uuid,
    wallet_address: &str
  ) -> Result<Vec<Web3PermissionClaim>> {
    ...
    let permissions = self.permission_service
      .get_user_permissions(user_id, Some(wallet_address)).await
      .unwrap_or_default();
      ...
      if perm.is_active {
        let claim = Web3PermissionClaim {
          ...
          source: match perm.source {
            super::unified_permission_service::PermissionSource::NftGated {
              ..
            } => {
              "nft".to_string()
            }
    ...
    debug!(
      "Found {} Web3 permission claims for wallet {}",
      claims.len(),
      wallet_address
    );
    Ok(claims)
  ...
  fn determine_rate_limit_tier(
    ...
    match access_level {
      ...
      AccessLevel::Premium =>
        RateLimitTier::Premium { requests_per_hour: 1000 },
      AccessLevel::Enterprise =>
        RateLimitTier::Enterprise { requests_per_hour: 10000 },
      AccessLevel::Admin => RateLimitTier::Unlimited,
    }
  }

  fn get_enterprise_features(
    &self,
    permissions: &[UnifiedPermission]
  ) -> Vec<String> {
  ...
  async fn should_refresh_permissions(
    &self,
    ...
    if needs_refresh {
      info!(
        "Permission refresh needed for user {} (token version: {}, current: {})",
        claims.sub,
  ...
  async fn get_user_permission_version(&self, user_id: &str) -> Result<u64> {
    ...
    Ok(row.try_get::<i64, _>("version").unwrap_or(0) as u64)
  }

  async fn calculate_rate_limit_remaining(
    &self,
    claims: &EnhancedTokenClaims
  ) -> Result<Option<u32>> {
  ...
  async fn get_hourly_request_count(&self, user_id: &Uuid) -> Result<u32> {
    ...
    Ok(row.try_get::<i64, _>("request_count").unwrap_or(0) as u32)
  }
  ...
  async fn get_user_data(&self, user_id: &Uuid) -> Result<UserData> {
    ...
    Ok(UserData {
      ...
      email: row
        .try_get("email")
        .unwrap_or_else(|_| "unknown@epsx.io".to_string()),
      name: None,
      permissions: None,
      audience: None,
      ttl_seconds: None,
  ...
  async fn check_api_key_permission(
    &self,
    user_id: &Uuid,
    wallet_address: Option<&str>
  ) -> Result<bool> {
    let permissions = self.permission_service.get_user_permissions(
      user_id,
      wallet_address
    ).await?;
    ...
    for permission in permissions {
      if
        permission.permission.contains("api_keys") ||
        permission.permission.starts_with("admin:")
      {
        return Ok(true);
      }
    }
  ...
  async fn generate_token_with_permissions(
    ...
    info!(
      "Generating token with specific permissions for user {}: {:?}",
      user_id,
      permissions
    );
    ...
    let access_level = if
      filtered_permissions.iter().any(|p| p.contains("admin"))
    {
      AccessLevel::Admin
    ...
    let claims = EnhancedTokenClaims {
      ...
      rate_limit_tier: self.determine_rate_limit_tier(
        &access_level_for_rate_limit,
        &nft_holdings_for_rate_limit,
        &token_balances_for_rate_limit
      ),
    ...
    let token = encode(&Header::default(), &claims, &self.encoding_key).map_err(
      |e| anyhow!("Failed to encode JWT: {}", e)
    )?;

    info!(
      "Generated scoped token for user {} with {} permissions",
      user_id,
      claims.permissions.len()
    );
    Ok(token)
  ...
  async fn store_api_key(
    ...
    sqlx
      ...
      .bind(
        uuid::Uuid
          ::parse_str(&api_key.id)
          .map_err(|e| anyhow!("Invalid API key ID: {}", e))?
      )
      .bind(user_id)
      .bind(&api_key.name)
      .bind(&api_key.key)
  ...
  async fn get_user_api_keys(&self, user_id: &Uuid) -> Result<Vec<ApiKey>> {
    ...
    for row in rows {
      ...
      let rate_limit: RateLimitTier = serde_json
        ::from_value(
          row.try_get("rate_limit").unwrap_or(serde_json::Value::Null)
        )
      ...
      api_keys.push(ApiKey {
        ...
        key: row.try_get("key_value").unwrap_or_default(),
        permissions,
        created_at: row.try_get("created_at").unwrap_or_else(|_| Utc::now()),
        expires_at: row.try_get("expires_at").ok(),
        last_used_at: row.try_get("last_used_at").ok(),
        usage_count: row.try_get::<i64, _>("usage_count").unwrap_or(0) as u64,
        rate_limit,
      });
    }
  ...
  async fn store_enterprise_team(&self, team: &EnterpriseTeam) -> Result<()> {
    ...
    sqlx
      ...
      .bind(
        uuid::Uuid
          ::parse_str(&team.id)
          .map_err(|e| anyhow!("Invalid team ID: {}", e))?
      )
      .bind(&team.name)
      .bind(&team.description)
      .bind(team.created_at)
      .bind(&team.plan_tier)
      .bind(team.monthly_quota as i64)
      .bind(team.current_usage as i64)
      .execute(&self.db_pool).await
    ...
    info!(
      "Successfully stored enterprise team {} with {} members",
      team.id,
      team.members.len()
    );
    Ok(())
...
mod tests {
  ...
  async fn setup_test_service() -> EnhancedBearerTokenService {
    ...
    let web3_permissions = Arc::new(
      super::super::Web3PermissionService::new(
        pool.clone(),
    ...
    let permission_service = Arc::new(
      UnifiedPermissionService::new(pool.clone(), web3_permissions)
    );
  ...
  async fn test_generate_token() {
    ...
    let token = service
      .generate_token(&user_id, None, None, Some(1)).await
      .unwrap();
