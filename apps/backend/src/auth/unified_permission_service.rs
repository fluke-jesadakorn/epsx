...
/// permissions across all authentication methods (Web3, manual, API keys).
...
use super::web3_permission_service::{
  Web3PermissionService,
  PermissionInfo as Web3PermissionInfo,
};
...
impl UnifiedPermissionService {
  ...
  pub async fn check_permission(
    ...
    if let Some(cached_result) = self.get_cached_result(&cache_key).await {
      debug!(
        "Permission check cache hit for user {} permission {}",
        check.user_id,
        check.permission
      );
    ...
    info!(
      "Permission check for user {} permission {}: {} (took {:.2}ms)",
      check.user_id,
      check.permission,
      result.allowed,
      check_duration
  ...
  pub async fn grant_permission(
  ...
  ) -> Result<UnifiedPermission> {
    info!(
      "Granting permission {} to user {} by {}",
      request.permission,
      request.user_id,
      request.granted_by
    );
    ...
    info!(
      "Successfully granted permission {} to user {}",
      request.permission,
      request.user_id
    );
  ...
  pub async fn revoke_permission(
    ...
    info!(
      "Revoking permission {} from user {} by {}",
      permission,
      user_id,
      revoked_by
    );
  ...
  pub async fn process_automatic_web3_permissions(
    &self,
    wallet_address: &str
  ) -> Result<Vec<String>> {
  ...
  async fn perform_permission_check(
    ...
    let user_permissions = self.get_user_permissions(
      &check.user_id,
      check.wallet_address.as_deref()
    ).await?;
    ...
    if !allowed {
      for permission in &user_permissions {
        if
          self.wildcard_permission_matches(
  ...
  async fn get_web3_permissions(
    &self,
    wallet_address: &str
  ) -> Result<Vec<UnifiedPermission>> {
    let web3_perms =
      self.web3_permissions.get_wallet_permissions(wallet_address).await?;
  ...
  fn convert_web3_permission(
    &self,
    web3_perm: Web3PermissionInfo
  ) -> UnifiedPermission {
    let verification_data = web3_perm.verification_data
      .as_ref()
    ...
    let source = match web3_perm.permission_type.as_str() {
      "nft" =>
        PermissionSource::NftGated {
          contract_address: verification_data
            .get("contract_address")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown")
            .to_string(),
          token_id: verification_data
            .get("token_id")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string()),
        },
      "token" =>
        PermissionSource::TokenGated {
          contract_address: verification_data
            .get("contract_address")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown")
            .to_string(),
          required_balance: verification_data
            .get("required_balance")
            .and_then(|v| v.as_str())
            .unwrap_or("0")
            .to_string(),
        },
      "dao" =>
        PermissionSource::DaoGoverned {
          dao_address: verification_data
            .get("dao_address")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown")
            .to_string(),
          proposal_id: verification_data
            .get("proposal_id")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown")
            .to_string(),
    ...
    let metadata = verification_data
      .iter()
  ...
  async fn get_database_permissions(
    ...
    // Should query for manual permissions and API key permissions

    // Placeholder implementation
    Ok(
      vec![UnifiedPermission {
  ...
  fn wildcard_permission_matches(&self, granted: &str, required: &str) -> bool {
    ...
    for (granted_part, required_part) in granted_parts
      .iter()
  ...
  async fn update_stats(&self, check_duration_ms: f64, cache_hit: bool) {
    ...
    stats.average_check_time_ms =
      (stats.average_check_time_ms + check_duration_ms) / 2.0;
...
mod tests {
  ...
  async fn test_wildcard_permission_matching() {
    ...
    assert!(
      service.wildcard_permission_matches("admin:*:*", "admin:users:view")
    );
    assert!(
      service.wildcard_permission_matches("epsx:*:view", "epsx:analytics:view")
    );
    assert!(
      !service.wildcard_permission_matches(
        "epsx:analytics:*",
        "admin:users:view"
      )
    );
