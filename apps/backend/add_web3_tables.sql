-- Add Web3 functionality to existing database
-- This script adds only the missing Web3 tables without modifying existing ones
...
-- Make firebase_uid nullable if not already
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'firebase_uid' AND is_nullable = 'NO'
    ) THEN
        ALTER TABLE users ALTER COLUMN firebase_uid DROP NOT NULL;
    END IF;
END $$;

-- Web3 authentication nonces table
...
CREATE INDEX IF NOT EXISTS idx_web3_nonces_wallet ON web3_auth_nonces(wallet_address);
CREATE INDEX IF NOT EXISTS idx_web3_nonces_nonce ON web3_auth_nonces(nonce);
CREATE INDEX IF NOT EXISTS idx_web3_nonces_expires ON web3_auth_nonces(expires_at);

-- Wallet permissions table - Core Web3 permission system
...
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_wallet ON wallet_permissions(wallet_address);
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_permission ON wallet_permissions USING GIN (permission);
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_type ON wallet_permissions(permission_type);
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_active ON wallet_permissions(wallet_address, is_active) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_expires ON wallet_permissions(expires_at) WHERE expires_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_nft ON wallet_permissions(nft_contract_address, nft_token_id) WHERE permission_type = 'nft_gated';
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_token ON wallet_permissions(token_contract_address) WHERE permission_type = 'token_gated';
CREATE INDEX IF NOT EXISTS idx_wallet_permissions_dao ON wallet_permissions(dao_contract_address, dao_proposal_id) WHERE permission_type = 'dao_granted';

-- NFT permission configurations
CREATE TABLE IF NOT EXISTS nft_permission_configs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_nft_configs_contract ON nft_permission_configs(contract_address, network);
CREATE INDEX IF NOT EXISTS idx_nft_configs_permission ON nft_permission_configs(permission);
CREATE INDEX IF NOT EXISTS idx_nft_configs_active ON nft_permission_configs(is_active) WHERE is_active = true;

-- Token permission configurations
CREATE TABLE IF NOT EXISTS token_permission_configs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    contract_address VARCHAR(42) NOT NULL,
    network VARCHAR(50) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_token_configs_contract ON token_permission_configs(contract_address, network);
CREATE INDEX IF NOT EXISTS idx_token_configs_permission ON token_permission_configs(permission);
CREATE INDEX IF NOT EXISTS idx_token_configs_active ON token_permission_configs(is_active) WHERE is_active = true;

-- DAO permission proposals and voting
CREATE TABLE IF NOT EXISTS dao_permission_proposals (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_dao_proposals_dao ON dao_permission_proposals(dao_contract_address, network);
CREATE INDEX IF NOT EXISTS idx_dao_proposals_target ON dao_permission_proposals(target_wallet_address);
CREATE INDEX IF NOT EXISTS idx_dao_proposals_status ON dao_permission_proposals(proposal_status);
CREATE INDEX IF NOT EXISTS idx_dao_proposals_voting_end ON dao_permission_proposals(voting_end);
...
-- Web3 permission verification cache
CREATE TABLE IF NOT EXISTS web3_permission_cache (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    wallet_address VARCHAR(42) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_web3_cache_wallet ON web3_permission_cache(wallet_address);
CREATE INDEX IF NOT EXISTS idx_web3_cache_expires ON web3_permission_cache(expires_at);
CREATE INDEX IF NOT EXISTS idx_web3_cache_contract ON web3_permission_cache(contract_address, network);

-- Migration helper table to track wallet linking
...
CREATE INDEX IF NOT EXISTS idx_wallet_migrations_user ON wallet_migrations(user_id);
CREATE INDEX IF NOT EXISTS idx_wallet_migrations_wallet ON wallet_migrations(wallet_address);
CREATE INDEX IF NOT EXISTS idx_wallet_migrations_status ON wallet_migrations(migration_status);
...
DROP TRIGGER IF EXISTS update_wallet_permissions_updated_at ON wallet_permissions;
CREATE TRIGGER update_wallet_permissions_updated_at BEFORE UPDATE ON wallet_permissions
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_nft_permission_configs_updated_at ON nft_permission_configs;
CREATE TRIGGER update_nft_permission_configs_updated_at BEFORE UPDATE ON nft_permission_configs
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_token_permission_configs_updated_at ON token_permission_configs;
CREATE TRIGGER update_token_permission_configs_updated_at BEFORE UPDATE ON token_permission_configs
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_dao_permission_proposals_updated_at ON dao_permission_proposals;
...
-- Function to get wallet permissions with real-time verification status
