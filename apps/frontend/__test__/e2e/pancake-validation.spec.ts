/**
 * Quick validation test for PancakeSwap theme integration
...
test.describe('🥞 PancakeSwap Theme Validation', () => {
  test('Validate PancakeSwap Theme Elements', async ({ page }) => {
    console.log('🚀 Starting PancakeSwap theme validation...');
    ...
    // Step 2: Verify PancakeSwap theme elements are present
    const pancakeTitle = page.locator('h2:has-text("Pancake Stack Login"), h1:has-text("Pancake Stack Login")');
    const pancakeEmoji = page.locator('text=🥞');
    const pancakeCard = page.locator('.pancake-card, [class*="pancake"]');

    await expect(pancakeTitle.or(pancakeEmoji).or(pancakeCard).first()).toBeVisible({ timeout: 10000 });
    console.log('✅ PancakeSwap theme elements detected');
    ...
    console.log('🎉 PancakeSwap theme validation completed successfully!');
  });

  test('Validate Error Handling with Pancake Theme', async ({ page }) => {
    ...
    const hasError = await page.locator('text=error, text=Invalid, text=failed, .error, .pancake-toast').first().isVisible().catch(() => false);
    if (hasError) {
  ...
  test('Validate Mobile Responsive Theme', async ({ page }) => {
    ...
    const pancakeCard = page.locator('.pancake-card, [class*="pancake"], form');
    const cardBounds = await pancakeCard.first().boundingBox();
...
console.log('🥞 PancakeSwap Theme Validation Suite Loaded');