/**
 * Complete E2E Authentication Flow Tests with PancakeSwap Themes
...
test.describe('🥞 PancakeSwap Authentication Complete Flow', () => {
  ...
  test('🥞 Complete OIDC Authorization Flow - Frontend Login', async () => {
    ...
    // Step 4: Verify PancakeSwap theme elements are present
    await expect(page.locator('h2:has-text("Pancake Stack Login")')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('.pancake-logo, [class*="pancake"]')).toBeVisible();
    await expect(page.locator('text=🥞')).toBeVisible();
    console.log('✅ PancakeSwap theme verified - Pancake Stack Login detected');
    ...
    const submitButton = page.locator('button[type="submit"], button:has-text("Stack Complete")');
  ...
  test('👨‍🍳 Complete OIDC Authorization Flow - Admin Login', async () => {
    ...
    // Step 4: Verify Chef's Kitchen Portal theme for admin
    const isAdminTheme = await page.locator('h2:has-text("Chef\'s Kitchen Portal"), h2:has-text("Admin Portal"), text=admin').first().isVisible({ timeout: 5000 }).catch(() => false);
    const isPancakeTheme = await page.locator('h2:has-text("Pancake Stack Login")').isVisible({ timeout: 5000 }).catch(() => false);
    
    if (isAdminTheme) {
      console.log('✅ Chef\'s Kitchen Portal theme detected for admin');
    } else if (isPancakeTheme) {
      console.log('✅ PancakeSwap theme detected (fallback for admin)');
    } else {
  ...
  test('🍯 User Registration Flow with PancakeSwap Theme', async () => {
    ...
    if (currentUrl.includes('/oauth/authorize')) {
      ...
      const isRegistrationTheme = await page.locator('h2:has-text("Join the Pancake Family"), h2:has-text("Register")').first().isVisible({ timeout: 5000 }).catch(() => false);
      if (isRegistrationTheme) {
        console.log('✅ "Join the Pancake Family" registration theme detected');
      }
  ...
  test('🔥 Error Handling - Invalid Credentials', async () => {
    ...
    // Step 4: Verify error display with PancakeSwap theme
    const errorAlert = page.locator('text=Oops! Burnt pancake detected!, text=Invalid email or password, .pancake-toast, .error');
    await expect(errorAlert.first()).toBeVisible({ timeout: 10000 });
    console.log('✅ Error handling with pancake theme verified');

    // Step 5: Verify error styling
    const burntPancakeIcon = page.locator('text=🔥');
    if (await burntPancakeIcon.isVisible()) {
      console.log('✅ Burnt pancake error icon detected');
    }
  });
  ...
  test('📱 Mobile Responsive Authentication Flow', async () => {
    ...
    // Step 2: Verify mobile-responsive pancake theme
    await expect(page.locator('h2:has-text("Pancake Stack Login")')).toBeVisible();
    
    // Check if pancake card is responsive
    const pancakeCard = page.locator('.pancake-card, [class*="pancake"]');
    const cardBounds = await pancakeCard.boundingBox();
    expect(cardBounds?.width).toBeLessThanOrEqual(375);
    console.log('✅ Mobile-responsive pancake theme verified');
...
console.log('🥞 PancakeSwap Authentication E2E Test Suite Loaded');