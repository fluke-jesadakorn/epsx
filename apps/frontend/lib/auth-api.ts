...
// Lightweight main frontend auth API client that calls the centralized backend
...
interface BulkPermissionResponse {
  ...
  results: { [key: string]: { has_permission: boolean; reason?: string } };
}
...
class MainAuthAPI {
  ...
  async checkBulkPermissions(permissions: string[]): Promise<BulkPermissionResponse> {
    return this.apiCall<BulkPermissionResponse>('/auth/check-permission', {
      method: 'POST',
      body: JSON.stringify({
        permissions,
        app_type: 'frontend',
  ...
  async validateBulkRoutes(routes: Array<{ route: string; method: string }>): Promise<{
    results: { [key: string]: { allowed: boolean; reason?: string; required_permissions: string[] } };
    user_permissions: string[];
  }> {
    return this.apiCall('/auth/validate-routes', {
      method: 'POST',
      body: JSON.stringify({
        routes,
        app_type: 'frontend',
...
export const mainAuthAPI = new MainAuthAPI();

// Utility functions for easy use in components
export const authUtils = {
  async isLoggedIn(): Promise<boolean> {
    try {
      await mainAuthAPI.getCurrentUser();
      return true;
    } catch {
      return false;
    }
  ...
  async hasPermission(feature: string): Promise<boolean> {
    try {
      const result = await mainAuthAPI.checkPermission(feature);
  ...
  async canAccessRoute(route: string): Promise<boolean> {
    try {
      const result = await mainAuthAPI.validateRoute(route);
