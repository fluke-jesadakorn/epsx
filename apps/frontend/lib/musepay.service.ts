...
const MUSEPAY_CONFIG = {
  ...
  notifyUrl: process.env.NEXT_PUBLIC_MUSEPAY_NOTIFY_URL || 'http://localhost:3000/api/v1/webhook/musepay'
};
...
export function generateCustomerRefId(userId: string, packageId: string): string {
  const timestamp = Date.now();
  return `USER:${userId}:PKG:${packageId}:REQ:${timestamp}`;
}

export function parseCustomerRefId(customerRefId: string): { userId: string; packageId: string; requestId: string } {
...
export class MusePayService {
  ...
  async createPayment(userId: string, packageId: string, userEmail?: string): Promise<{ paymentRequest: PaymentRequest; musePayResponse: MusePayResponse }> {
    ...
    if (!response.ok) {
      throw new Error(`MusePay API error: ${response.status} ${response.statusText}`);
    }
  ...
  async processWebhook(webhookPayload: WebhookPayload): Promise<boolean> {
    ...
    const extraInfoObj = webhookPayload.extra_info ? JSON.parse(webhookPayload.extra_info) : {};
    ...
    const transactionData = {
      ...
      finishTime: webhookPayload.finish_time ? new Date(parseInt(webhookPayload.finish_time)) : new Date(),
    ...
    if (status === 1 || status === '1') {
      ...
      await this.updateUserPaymentStatus(userId, paymentRequestData, transactionData);
    }
  ...
  private async updateUserPaymentStatus(
    ...
    if (!packageData) {
      console.error('Package not found for payment update:', paymentRequest.packageId);
      return;
    }
  ...
  async getPaymentRequest(customerRefId: string): Promise<PaymentRequest | null> {
    const paymentRequestQuery = query(
      collection(db, 'payment_requests'),
