...
mod account_fullstack;
mod auth_fullstack;
mod credits_fullstack;
mod developer_fullstack;
mod home_fullstack;
mod news_fullstack;
mod notifications_fullstack;
mod pay_orders;
mod payment_fullstack;
mod push_fullstack;
mod watchlist_fullstack;
...
pub async fn run() {
    ...
    epsx_bff::fullstack::verify_public_assets().expect("complete Dioxus frontend assets");
...
pub fn state_from_env() -> Result<AppState, String> {
    ...
    validate_internal_url(&content_url, cookie_environment, "CONTENT_SERVICE_URL")?;
...
pub fn build_app(state: AppState) -> Router {
    fullstack::application(state)
}

pub(crate) fn build_app_with_legacy_pages(state: AppState, legacy_purchases: bool) -> Router {
    // Legacy producers exist only for regression fixtures; no native app can enable them.
    #[cfg(not(test))]
    let _ = legacy_purchases;
    let browser_assets = epsx_bff::static_assets::service_worker_router(&browser_runtime_dir());
    #[cfg(test)]
    let browser_assets = if legacy_purchases {
        epsx_bff::static_assets::browser_runtime_router(&browser_runtime_dir())
    } else {
        browser_assets
    };
    let router = Router::new()
        ...
        // into the frontend.
        .route("/api/payments/pay-orders", get(pay_orders::api))
        .route("/api/payments/pay-orders/{id}", get(pay_orders::api))
        ...
        .route(
            ...
            post(payment_adapter::merchant_checkout),
        )
        .route(
        ...
        .route("/chat", post(chat_adapter::chat_create_form))
        .route("/chat/{id}", post(chat_adapter::chat_conversation_form))
    ...
    if legacy_purchases {
        ...
        return router
            .fallback(move |request: Request| {
                fallback_handler(State(legacy_state.clone()), request)
            })
            .merge(
                Router::new()
                    .route("/account/payments", get(pay_orders::page))
                    .route("/account/payments/{id}", get(pay_orders::page))
                    .layer(axum::middleware::from_fn(security_headers))
                    .with_state(state),
            );
    }
...
async fn serve_tailwind_css() -> impl IntoResponse {
    ...
    match tokio::fs::read(&path).await {
        Ok(bytes) => (
            ...
            .into_response(),
        Err(_) => (axum::http::StatusCode::NOT_FOUND, "tailwind.css not found").into_response(),
    }
}
...
mod routing_tests {
    use super::*;
    // Explicit old-renderer fixtures remain as parity references. Production's
    // public build_app always constructs the Dioxus Fullstack application.
    fn build_app(state: AppState) -> Router {
        super::build_app_with_legacy_pages(state, true)
    }
    ...
    #[tokio::test]
    async fn fullstack_cutover_has_no_legacy_fallback_and_preserves_aliases() {
        let app = crate::fullstack::application(test_state());
        for (path, location) in [
            ("/pricing?ref=test", "/plans?ref=test"),
            ("/manual?section=account", "/analytics"),
            ("/portfolio/other-owner", "/portfolio"),
        ] {
            let response = app
                .clone()
        ...
        for path in [
            "/does-not-exist",
            "/runtime/epsx_browser_runtime_bootstrap.js",
        ] {
            let response = app
                .clone()
                .oneshot(Request::builder().uri(path).body(Body::empty()).unwrap())
            ...
            .unwrap();
            assert!(body.contains("data-dioxus-hydrated"));
            assert!(!body.contains("src=\"/runtime/epsx_browser_runtime_bootstrap.js"));
        }
        let response = app
            .clone()
            .oneshot(
    ...
    async fn pricing_redirect_status_and_target_are_preserved() {
        ...
        let retired = request(Method::GET, "/manual?section=account").await;
        assert_eq!(retired.status(), StatusCode::TEMPORARY_REDIRECT);
        assert_eq!(retired.headers()[header::LOCATION], "/analytics");
    }
