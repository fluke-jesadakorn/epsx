...
import { PaymentLoading } from './PaymentLoadingState';
import PackageSelection from './PackageSelection';
import AssetSelection from './AssetSelection';
import PaymentForm from './PaymentForm';
import type { PaymentLoadingState, PaymentError, CurrencyType } from '@/app/constants/packages';
...
type PaymentStep = 'package' | 'asset' | 'details' | 'processing' | 'confirmation';
...
export default function PaymentFlow({ className }: PaymentFlowProps) {
  ...
  const [loadingState, setLoadingState] = useState<PaymentLoadingState>({ state: 'idle' });
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      if (transactionId) {
        ...
        throw { type: 'TRANSACTION_FAILED', reason: 'Transaction recording failed' };
      }
    } catch (error) {
      ...
      const paymentError: PaymentError = 
        error && typeof error === 'object' && 'type' in error
          ...
          : { type: 'TRANSACTION_FAILED', reason: 'Unknown error occurred' };
      
      setLoadingState({ state: 'error', error: paymentError });
    }
  };
  ...
  return (
    <PaymentErrorBoundary onReset={handleReset}>
      <PaymentLoading state={loadingState}>
        <div className={className}>
          <div className="max-w-3xl mx-auto">
            ...
            <div className="mt-8">
              ...
              {currentStep === 'confirmation' && loadingState.state === 'success' && (
                <div className="text-center py-12">
                  <h2 className="text-2xl font-bold text-green-600 mb-4">
                    Payment Successful!
                  </h2>
                  <p className="text-lg text-muted-foreground mb-8">
                    Thank you for your purchase. You can now access your package features.
                  </p>
                  <button
