...
export function UserLevelDisplay({ className }: UserLevelDisplayProps) {
  ...
  return (
    <Card
      className={`relative overflow-hidden transition-all duration-300 hover:shadow-xl ${levelBackgrounds[currentLevel]} ${className}`}
    >
      ...
      <CardHeader className="relative pb-2">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
            <div
              className={`relative p-2 sm:p-3 rounded-2xl bg-gradient-to-br ${levelGradients[currentLevel]} shadow-lg transform transition-transform hover:scale-105 flex-shrink-0`}
            >
              <div className="text-white">{levelIcons[currentLevel]}</div>
      ...
      <CardContent className="relative space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
            {LEVEL_BENEFITS[currentLevel].slice(0, 4).map((benefit, index) => (
              <div key={index} className="flex items-start gap-2 text-sm">
                <div
                  className={`w-2 h-2 rounded-full bg-gradient-to-r ${levelGradients[currentLevel]} animate-pulse mt-1 flex-shrink-0`}
                ></div>
        ...
        {nextLevelOptions.length > 0 &&
          userStatus?.paymentCount !== undefined && (
            <div>
