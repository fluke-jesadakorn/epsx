...
import { useState, useEffect, useCallback, useRef, useTransition, type KeyboardEvent } from 'react';
  ...
  return (
    <div className="h-[calc(100vh-12rem)] flex gap-4">
      ...
      <div className="w-[360px] flex-shrink-0 flex flex-col">
        {/* Filter Bar */}
        <div className="flex items-center gap-2 mb-3 p-2.5 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-xl">
          <SlidersHorizontal className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
          <select
            value={statusFilter}
            onChange={e => setStatusFilter(e.target.value)}
            className="flex-1 px-2.5 py-1.5 text-xs font-medium bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/8 rounded-lg text-foreground focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 transition-all cursor-pointer"
          >
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800/40 flex items-center justify-center mb-3 border border-slate-200 dark:border-white/5">
                ...
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl transition-all ${
                    isSel
                      ...
                      : 'bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-gray-100 dark:bg-slate-800/50 hover:border-slate-300 dark:hover:border-gray-200 dark:border-white/10'
                  }`}
                >
      ...
      <div className="flex-1 border border-slate-200 dark:border-white/5 rounded-2xl bg-white dark:bg-slate-900/30 overflow-hidden flex flex-col">
        {showNew ? (
          <ChatTopicSelector topics={topics} onSelect={handleCreate} />
        ) : selectedConv ? (
          <>
            {/* Header */}
            <div className="border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-900/60 p-4">
            ...
            {/* Reply */}
            <div className="border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/40 p-3">
              {canResolve && (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800/40 flex items-center justify-center mb-4 border border-slate-200 dark:border-white/5">
