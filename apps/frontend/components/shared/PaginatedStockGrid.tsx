    ...
    onPageChange: async (page, limit) => {
      ...
      try {
        ...
        if (useApi) {
          ...
          const { fetchPaginatedStockData } = await import(
            '@/app/actions/stockRankingPaginated'
          );
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 auto-rows-max w-full max-w-full">
        {stockData.data.map((stock, index) => {
          ...
          const isLocked =
            !currentPageAccessible && globalRank > maxAllowedLimit;

          return (
            <Card
              key={`${stock.symbol}-${index}`}
              className={`relative w-full max-w-full overflow-hidden transition-all duration-300 ${
                isLocked
