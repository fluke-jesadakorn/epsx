...
export function PermissionCard({
  ...
  const isExpiringSoon =
    permission.timeRemaining && permission.timeRemaining < 24 * 60 * 60 * 1000;
  ...
  return (
    <div className="space-y-3">
      {history.map((item) => {
        ...
        return (
          <div
            key={`${item.permission}-${item.timestamp}`}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-1">
                ...
                <Badge
                  variant="outline"
                  className={cn('text-xs', getActionColor())}
                >
                  {item.action}
                </Badge>
              </div>
...
export function TabNavigation({
  ...
  activeTab: string;
  timestampedPermissions: TimestampedPermission[];
        ...
        <Card>
          ...
          <CardContent>
            <PlatformDistributionChart
              distribution={analytics.platformDistribution}
            />
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {analytics.usageStats?.recentlyGranted?.map((item) => (
              <div
                key={`${item.permission}-${item.grantedAt}`}
                className="flex items-center justify-between"
              >
                <span className="text-sm font-medium">{item.permission}</span>
