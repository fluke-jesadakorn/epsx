...
function hasValidSession(request: NextRequest): boolean {
  ...
  if (parts.length !== 3) {
    console.log('Middleware: Invalid JWT format, parts:', parts.length);
    return false;
  }
  
  // Additional basic checks could be added here
  const isValid = token.length > 100; // Minimum reasonable token length
  console.log('Middleware: Session validation result:', isValid, 'Token length:', token.length);
  return isValid;
}
