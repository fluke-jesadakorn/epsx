...
| Identity ranking offset | `shared/rust/epsx-identity-service` + `shared/rust/epsx-ranking-store` | Backend-owned wallet plan/permission projection | A2.7 adds a pure resolver, A2.8 adds a static core-owned snapshot adapter, A2.9 removes the emit/SSE mounts from the production binary code path, A2.10 exports a hermetic fail-closed workload-authenticated RPC composition, and A2.11 extracts the adapter into a library-only store package. Store and composition remain unwired; no concrete credential verifier, analytics metadata, TLS, owner delegation, schema/query-plan activation or durable event replacement exists, the runtime still always returns free-plan offset, and checked-in 50052 configuration is stale. |
...
A2.9 removes `POST /v1/emit` and the ranking-offset SSE stream from the identity production binary code path and makes the historical handler/bus modules test-only. No authenticated authoritative publisher or durable outbox/revision/cursor/replay/repair transport replaces them. A2.10 exports a fail-closed authenticated query composition, and A2.11 extracts the static adapter into a library-only store, but production `main.rs` constructs neither, the analytics client sends no authorization metadata, and no concrete verifier, TLS, owner delegation or certified store activation exists. The live gRPC query is therefore still the unauthenticated always-Free shim rather than active plan assignments. Checked-in identity and analytics manifests continue to advertise the old 50052/SSE configuration; A2.9 through A2.11 do not render or deploy them, so external reachability remains unproved rather than declared closed.
...
The verifier reads only local Git objects and local files. It refuses database, Redis, chain/RPC, identity-network, live-market-data and production-looking environment variables, removes proxy variables, and never starts a service. A2.4 and A2.5 are replayed from only their contracts and explicit implementation paths at A2.6's immutable post-A2.5 target base; A2.7 is replayed from its immutable post-A2.7 commit because A2.8 moves the repository port into the shared contract crate; A2.8 is replayed from its immutable post-A2.8 commit because A2.9 changes identity main; A2.9 is replayed from immutable post-A2.9 commit `60ababc75a79d173b3b217df8e9b9155795a1117`; and A2.10 is replayed from immutable post-A2.10 commit `005a604542271050279a6190fc00eada00f32137` because A2.11 changes the lockfile and backend adapter. A2.6 and A2.11 are verified against the canonical worktree with alternate child roots disabled. This preserves completed historical digests while allowing downstream slices to add independently bounded evidence. The 40-anchor inventory retains A2.10's replacement of the redundant direct market gRPC-timeout anchor and replaces the unreferenced event-database default anchor with the A2.11 contract; operational event-database configuration remains a STOP elsewhere in the contract. Integrity passing proves only that the 14 source pins, 40 target anchors, composed A2.4 through A2.11 boundaries, domain separation and unchanged 24-blocker STOP contract are internally consistent.
