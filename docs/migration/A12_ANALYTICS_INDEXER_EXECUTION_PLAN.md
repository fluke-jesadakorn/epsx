...
5. **Make ranking entitlement authoritative.** Keep A2.6's fail-closed authenticated error boundary, then make identity resolve the minimum effective offset from one atomic active backend plan-assignment snapshot, including overlap, scheduled start/expiry, cancellation and downgrade. Publish a monotonic projection revision through a transactional outbox; authenticate RPC callers and publishers; persist consumer cursor; reconcile gaps. Explicit free-plan success must be distinguishable from authority failure.
...
The verifier reads only local Git objects and local files. It refuses database, Redis, chain/RPC, identity-network, live-market-data and production-looking environment variables, removes proxy variables, and never starts a service. A2.4 and A2.5 are replayed from only their contracts and explicit implementation paths at A2.6's immutable post-A2.5 target base; A2.6 is verified against the canonical worktree. This preserves the completed historical digests while allowing the downstream slice to change the shared router and handler. Integrity passing proves only that the 14 source pins, 38 target anchors (including the A2.4 authorization, A2.5 bounded-provider, and A2.6 fail-closed authority contracts), domain separation and 24-blocker STOP contract are internally consistent.
