...
| Identity ranking offset | `shared/rust/epsx-identity-service` | Backend-owned wallet plan/permission projection | A2.7 adds a pure entitlement resolver and A2.8 adds a static core-owned snapshot adapter, but the statement is unexecuted and unwired, the runtime query still always returns free-plan offset, and its unauthenticated emit plus ephemeral SSE path is not a durable entitlement projection. |
...
The verifier reads only local Git objects and local files. It refuses database, Redis, chain/RPC, identity-network, live-market-data and production-looking environment variables, removes proxy variables, and never starts a service. A2.4 and A2.5 are replayed from only their contracts and explicit implementation paths at A2.6's immutable post-A2.5 target base; A2.7 is replayed from its immutable post-A2.7 commit because A2.8 moves the repository port into the shared contract crate; A2.6 and A2.8 are verified against the canonical worktree with alternate child roots disabled. This preserves completed historical digests while allowing downstream slices to add independently bounded evidence. Integrity passing proves only that the 14 source pins, 40 target anchors (including the A2.4 authorization, A2.5 bounded-provider, A2.6 fail-closed authority, A2.7 pure entitlement-snapshot and A2.8 static core-adapter contracts), domain separation and 24-blocker STOP contract are internally consistent.
