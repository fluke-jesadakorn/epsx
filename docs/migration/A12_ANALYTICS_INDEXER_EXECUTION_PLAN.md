...
| Identity ranking offset | `shared/rust/epsx-identity-service` | Backend-owned wallet plan/permission projection | A2.7 adds a pure resolver, A2.8 adds a static core-owned snapshot adapter, and A2.9 removes the emit/SSE mounts from the production binary code path. The statement remains unexecuted and unwired, the runtime query still always returns free-plan offset, no authenticated durable event replacement exists, and checked-in 50052 configuration is stale. |
...
The verifier reads only local Git objects and local files. It refuses database, Redis, chain/RPC, identity-network, live-market-data and production-looking environment variables, removes proxy variables, and never starts a service. A2.4 and A2.5 are replayed from only their contracts and explicit implementation paths at A2.6's immutable post-A2.5 target base; A2.7 is replayed from its immutable post-A2.7 commit because A2.8 moves the repository port into the shared contract crate; A2.6, A2.8 and A2.9 are verified against the canonical worktree with alternate child roots disabled. This preserves completed historical digests while allowing downstream slices to add independently bounded evidence. Integrity passing proves only that the 14 source pins, 40 target anchors (including A2.4 authorization, A2.5 bounded provider, A2.6 fail-closed authority, A2.7 pure entitlement snapshot, A2.8 static core adapter and A2.9 production-binary event containment), domain separation and unchanged 24-blocker STOP contract are internally consistent.
