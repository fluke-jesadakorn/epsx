...
| Identity ranking offset | `shared/rust/epsx-identity-service` | Backend-owned wallet plan/permission projection | A2.7 adds a pure entitlement-snapshot resolver, but the runtime query still always returns free-plan offset and its unauthenticated emit plus ephemeral SSE path is not a durable entitlement projection. |
...
5. **Make ranking entitlement authoritative.** Keep A2.6's fail-closed authenticated error boundary and A2.7's pure resolver. Next add one core-owned atomic owner snapshot and reconcile it against source data before runtime activation. A2.7 preserves the source ranking query's active joined rows, exclusive expiry, Free Plan seed and overlap minimum; it does not treat `assigned_at` as scheduled start and does not apply subscription grace. Scheduling, grace, direct permissions, token claims and `PlanFeatures` require a separate explicit backend authority decision. Then publish a monotonic projection revision through a transactional outbox; authenticate RPC callers and publishers; persist consumer cursor; reconcile gaps. Explicit free-plan success must remain distinguishable from authority failure.
...
The verifier reads only local Git objects and local files. It refuses database, Redis, chain/RPC, identity-network, live-market-data and production-looking environment variables, removes proxy variables, and never starts a service. A2.4 and A2.5 are replayed from only their contracts and explicit implementation paths at A2.6's immutable post-A2.5 target base; A2.6 and A2.7 are verified against the canonical worktree with alternate child roots disabled. This preserves the completed historical digests while allowing downstream slices to add independently bounded evidence. Integrity passing proves only that the 14 source pins, 39 target anchors (including the A2.4 authorization, A2.5 bounded-provider, A2.6 fail-closed authority and A2.7 pure entitlement-snapshot contracts), domain separation and 24-blocker STOP contract are internally consistent.
