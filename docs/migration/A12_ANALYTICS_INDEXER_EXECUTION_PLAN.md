...
| Chain indexer | `services/indexer` | Canonical blocks, transactions, receipts and decoded logs | It currently writes placeholder block records, keeps the sync cursor in memory, has no reorg/finality/backfill model, and exposes direct unauthenticated routes. |
...
The service accepts any method for `/sync` and has no local token verification. Gateway policy does not protect direct service reachability. Status labels any lag under 100 as healthy without chain-specific finality or a degraded reason. Production manifests do not prove a separately deployed indexer workload.
...
3. **Close every direct-service boundary.** Health alone may be public. Market authenticated routes derive wallet from a verified token. Event admin reads keep exact audience/permission rules. Indexer sync is internal/admin-only at the service. Decide whether address history is deliberately public or owner/admin restricted.
