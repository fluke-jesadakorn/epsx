...
|---|---|---|---|---|
| Canonical backend | `payments`, `subscriptions`, `stock_ranking_assignments`, `payment_contexts`, `wallet_credits`, `credit_transactions`, `payment_audit_log` | `/api/payments` | Current runtime, migrator, and provisioning use `epsx_payments_{dev,staging,prod}`; unset `PAYMENTS_DATABASE_URL` still falls back to primary `DATABASE_URL` | Existing production-shaped writer; this audit does not declare it the future authority |
...
| Current pay candidate | `pay_intents`, `escrows`, `pay_links`, `pay_webhook_events` | `/api/v1/pay`, `/api/v1/admin/pay` | `epsx_payments_{dev,staging,prod}` | Owner/admin reads are principal scoped; owner/internal mutations are `404`; admin mutations are `401`/`403` unless authorized with manage permission, then handler-unavailable `404` |
...
Current environment provisioning, backend runtime/migrator, and pay deployment now agree on `epsx_payments_{dev,staging,prod}`. This resolves connection-name drift only. The historical `epsx_payment` schema remains a different model; renaming a database would not rename `payment_intents` to `pay_intents`. A3.14 adds exact bounded reconciliation for the backend's `payments.plans` projection, not an adoption ledger for the pay candidate's financial tables. The old `escrows` shape is still not proven compatible with the current candidate shape.
