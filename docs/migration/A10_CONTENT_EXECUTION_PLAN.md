...
| Frontend BFF | `target-frontend-routes`, `target-bff-ranking-absent`, `target-bff-plan-absent`, `target-bff-news-upstream`, `target-bff-news-not-found`, `target-bff-portfolio-absent` | News now uses strict live upstream adapters and preserves upstream not-found. Canned ranking, plan, and portfolio compatibility producers are removed; the UI fails closed while the content service still synthesizes an unknown-slug article and retains static ranking/content-plan sources. |
...
| UI | `target-news-ui-error-state`, `target-portfolio-ui-unavailable`, `target-analytics-ui-unavailable` | News exposes explicit empty/error/retry behavior without sample fallback; home market/plans/news previews, public plans, analytics, and authenticated portfolio now fail closed without sample business data, while complete state coverage across all lifecycle surfaces remains absent. |
...
| Public plans | DB-backed, list filters `is_public`, Redis TTL 900, tier sort; detail has a visibility gap. | Content JSON remains canned; the BFF producer, unused SSR loader, static UI catalog, and checkout controls are removed. | Subscription service owns records; both list/detail require `is_public && is_active`; filter/order/promotion/cache semantics are identical. |
| Ranking access | Backend permission service derives rank offset/cap; anonymous uses backend free offset. | The content service remains static; the BFF producer and sample analytics/home ranking UI are removed. | Only backend permission/plan authority derives access; downstream may render but never widen results. |
