...
- **4 partial:** `/news`, `/notifications`, `/notifications/manage`, and `/wallet-management`;
- **21 blocked**;
...
`/news` and `/notifications/manage` are now partial operational admin routes. News uses a strict SSR-only compatibility adapter for the pinned Rust backend's protected `GET /api/admin/news` read, forwards only a verified bearer and normalized `page`/`status`, caps the upstream body at 8 MiB, and projects away full content, author identity, cover media, and unused timestamps. Notification management uses a new backend-owned global `GET /api/v1/notification/admin/list` read: gateway and direct service both require the exact admin audience plus `admin:notifications:manage`, the BFF fixes `limit=20`, derives a bounded offset from one page value, caps the body at 256 KiB, and projects only bounded title/subject/channel/status/type/priority/timestamps. Recipient and user identity, template IDs, bodies, arbitrary data, delivery errors, read state, action URLs, stats, filters, and every mutation remain absent. Both pages distinguish ready, authoritative-empty, forbidden, unavailable, malformed, and recoverable out-of-range states; authenticated HTML is private/no-store and varies on cookie/authorization. `/payments` retains its bounded read-only payment-intents loader but stays blocked overall because its other source tabs, isolated service/database/browser proof, and all financial mutation authority remain open. Other operational pages dispatch without authoritative page data and continue to fail closed instead of presenting plausible sample state.
...
8. the accepted baseline remains 2 aligned / 4 partial / 21 blocked until evidence is deliberately updated.
