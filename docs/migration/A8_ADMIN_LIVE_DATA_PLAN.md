...
1. **B1 command and security — `/`, `/analytics`, `/audit-log`, `/settings`.** Dashboard now fails closed without fabricated KPIs, health, alerts, wallets, transactions, activity, uptime, or freshness; `/audit-log` fails closed without sample actors, IPs, timestamps, totals, filters, expansion, pagination, or export. Define canonical aggregate/read models and backend field authorization; remove remaining sample operational success; establish a dedicated backend audit permission; wire query-driven ranges, filters, pagination, and authorized server export; render dependency-specific ready/empty/forbidden/degraded/error/retry states; implement settings as typed versioned backend mutations.
