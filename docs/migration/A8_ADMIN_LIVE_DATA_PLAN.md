...
`/audit-log`, `/media`, `/news`, `/notifications/manage`, and `/wallet-management/wallets` are now partial operational admin routes. Audit uses a new extracted analytics-service read over only `infra_logs.unified_audit_log`: gateway and service independently require the exact admin audience plus `admin:audit:read`, startup verifies the existing schema, and rows use stable descending `(created_at,id)` keyset continuation inside a read-only repeatable-read transaction. Its BFF accepts only optional closed category/cursor values, caps bodies at 256 KiB, and stores only category/action/resource-type/effect/timestamp summaries; actor/target identity, network/device fields, before/after state, metadata, totals, details, and export are absent. Media uses a strict SSR-only compatibility adapter for the legacy Rust backend's protected `GET /api/admin/media/{news|public}?limit=100` read. The BFF verifies the admin session, forwards its bearer, follows no redirects, accepts only an empty query or exactly one `bucket=news|public` pair, caps the body, rejects error or malformed envelopes, and projects only key, size, and optional last-modified time; URLs and private chat/notification bucket keys do not cross into PageContext. Duplicate, empty, unknown, cursor, prefix, limit, chat, and notification inputs fail before I/O. The legacy backend currently repeats only its broad `admin:media:manage` compatibility guard, so direct exact-admin-audience enforcement and a dedicated read permission are explicitly not proven. News uses a strict SSR-only compatibility adapter for the pinned Rust backend's protected `GET /api/admin/news` read and projects away content and identity/media fields. Notification management uses backend-owned global `GET /api/v1/notification/admin/list` and projects only bounded delivery metadata. Wallet inventory consumes only `GET /api/admin/wallets/stats`; SQL counts stay `i64`, the backend repeats `admin:users:read`, and the BFF retains only total, active, inactive, and new-30-day counts after exact-envelope and arithmetic validation. It rejects every non-empty query and keeps rows, identities, balances, plans, permissions, activity, filters, details, and controls unavailable. These pages distinguish their applicable ready, authoritative-empty, forbidden, unavailable, and malformed outcomes; authenticated HTML is private/no-store and varies on cookie/authorization. `/payments` retains its bounded read-only payment-intents loader but stays blocked overall because its other source tabs, isolated service/database/browser proof, and all financial mutation authority remain open. Other operational pages continue to fail closed instead of presenting plausible sample state.
...
`apps/admin/src/ssr.rs` authenticates the request and normally dispatches with empty data params. The bounded `/payments?tab=payments` exception forwards only its closed intent query; `/news` forwards one normalized page/status to the protected legacy admin list; `/media` accepts only an empty query, defaulting to `news`, or exactly one `bucket=news|public` pair and calls the protected legacy inventory with a fixed limit of 100; `/notifications/manage` forwards one bounded page to the global redacted inventory; `/wallet-management/wallets` accepts only the absent query and calls the exact wallet-stats endpoint. The wallet adapter follows no redirect, caps the body at 256 KiB, strictly decodes the current admin envelope, requires valid non-negative `i64` counts with total equal to active plus inactive and new-30-days no greater than total, and projects only those four values. Zero is ready rather than empty; 403 is forbidden, every other non-200 status is unavailable, and malformed envelopes or relationships show no count. The media adapter rejects every duplicate, empty, unknown, cursor, prefix, limit, chat, or notification query before I/O, plus redirects, oversized bodies, HTTP-200 error envelopes, malformed success envelopes, URLs crossing its redacted projection, and private bucket requests. Only HTTP 200 decodes; 400 is malformed, 403 is forbidden, and every other status or transport failure is unavailable. `/audit-log` accepts only optional closed category and bounded canonical base64url continuation values, calls only the exact extracted audit endpoint without following redirects, binds returned rows to the requested category, and stores only a redacted typed keyset page; it never falls back to the sensitive legacy monolith route. The current unsigned continuation is reversible and carries the boundary timestamp, audit UUID, and category, so replacing it with an authenticated opaque token or explicitly accepting that exposure remains a production blocker. Dashboard, analytics, chat list/detail, news create/edit, notification create, developer portal, settings, wallet credits/access, wallet detail/disable, and wallet plan list/detail retain verified-session explicit-unavailable shells with no plausible operational records, credentials, balances, plan catalogs, forms, upload controls, or mutations. The wallet-list route shows only its four-count summary and a separate explicit inventory-unavailable notice.
...
Other surfaces currently gate read-only data behind broad manage permissions, notably chat, media, news, notifications, and settings. The media compatibility read deliberately retains the legacy `admin:media:manage` guard; this is not evidence of a dedicated read decision or direct monolith exact-admin-audience enforcement. The payment-intents tab now uses the backend-recognized `admin:payments:view` permission, while the Pay service repeats the exact admin-audience and permission check. The remaining execution batches must introduce canonical read permissions only where the backend recognizes them; the UI must not invent role expansion or policy.
...
3. **B3 support — `/chat`, `/chat/:id`, `/notifications`, `/notifications/manage`.** Keep `/notifications` partial until its three redirect proof gaps close. Admin chat list/detail fail closed without canned conversations, messages, counts, presence, filters, replies, assignment, or status actions; detail route references are bounded and explicitly unverified. `/notifications/manage` now consumes a strict read-only global notification summary with native pagination and explicit empty/forbidden/unavailable/malformed recovery, while stats, filters, identities, bodies, dialogs, and all lifecycle mutations remain absent. Prove the read against an isolated service/database and authenticated browser, introduce a reviewed operation-specific read split and source-compatible global filters/stats, then implement conflict-safe scheduling, acknowledgement/delete, delivery/reconnect, preferences, and recovery only through A11.
4. **B4 content and media — `/media`, `/news`, `/news/create`, `/news/:id/edit`.** `/media` now has a narrow read-only compatibility path to the legacy Rust backend for only `news` and `public`, with a fixed limit of 100 and explicit ready/empty/forbidden/unavailable/malformed states. Only key, size, and optional timestamp render; URL/preview, private buckets, storage totals, cursor/completeness claims, MIME/hash/ETag/integrity, filters, open/copy, upload, and delete remain absent. The legacy backend repeats its current `admin:media:manage` guard, but direct exact-admin-audience enforcement, a canonical `admin:media:read` decision, S3 continuation, and isolated storage/browser proof remain open. `/news` has a strict read-only compatibility path to the pinned Rust backend's protected admin list, with source-like status filtering, bounded pagination, escaped summary cards, and explicit ready/empty/forbidden/unavailable/malformed states; it never consumes the public marketing feed or exposes a content mutation. Create/edit remain fail-closed, authenticated edit references are bounded and explicitly unverified, and signed-out dispatcher and BFF shells return only to `/news` without disclosing the reference. Keep every media mutation blocked, move the list producer into canonical service ownership, split backend read/manage authority, add deterministic ordering and isolated storage/database/browser proof, then finish A10 detail/revision, upload, publish/cache, optimistic conflict, autosave/recovery, unsaved-change, and accessible editor/upload behavior.
...
| `/media` | strict legacy compatibility projection for only news/public key, size, and optional timestamp with ready/empty/forbidden/unavailable/malformed states; no URL, private bucket, completeness claim, or action | BFF admin session plus legacy `admin:media:manage` guard only; direct exact audience, dedicated read decision, storage/browser proof, and every mutation remain open | partial |
...
| `/notifications/manage` | strict read-only global delivery-summary projection with native pagination and ready/empty/forbidden/unavailable/malformed recovery; no identities, bodies, stats, filters, or mutations | gateway and direct service repeat exact admin audience plus current canonical `admin:notifications:manage`; isolated runtime/browser proof and future read split remain open | partial |
...
| `/wallet-management/wallets` | strict four-count `i64` status summary with ready/forbidden/unavailable/malformed states and zero as ready; rows, identities, balances, plans, permissions, activity, filters, details, and controls remain unavailable | exact-admin BFF session plus backend `admin:users:read`; direct monolith exact audience, canonical wallet-service ownership, isolated DB/browser proof, and repaired bounded deterministic list contract remain open | partial |
