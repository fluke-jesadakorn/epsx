...
The former dormant string-renderer badge no longer fetches `limit=1` or derives a global count from one list page; it is explicitly unavailable. That renderer is not the active navbar. The active SSR shell mounts the shared `epsx_templates::epsx_header()`, whose current header has no notification-badge integration at all, so a global shared-header badge remains **BLOCKED** until it consumes the separately reviewed unread-count route with authenticated browser/runtime evidence. This is not lifecycle completion: source-compatible pagination and filters, a wired global unread badge, live browser/runtime proof, mutations, preferences, push, SSE, and action-URL policy remain blocked. The admin UI also contains sample data and does not prove source-compatible stats, scheduling, or broadcast behavior.
