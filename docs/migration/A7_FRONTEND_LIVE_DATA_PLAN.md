...
| `/notifications` | `apps/frontend/app/notifications/page.tsx :: page: parseInt(params.page ?? '1')` | `pages/notifications.rs :: enum NotificationLoad` | explicit-outcome owner `GET /api/v1/notification/list`; required nullable keys and UTC timestamps, sample-free empty/error/retry, neutral fallbacks, static read-only rows; pagination/global count/lifecycle/browser proof absent | partial |
...
4. **B4 communication:** `/chat`, `/chat/:id`, `/chat/history`, `/notifications`. Notifications now has an authenticated, sample-free, explicit-outcome, read-only owner list and remains partial; lock its source-compatible envelope/pagination/global-count contract and prove browser behavior before adding backend-owned lifecycle/preference/push/SSE/action-URL features. The chat routes still need owner-scoped list/detail/create/send/resolve APIs, SSE reconnect, and keyboard/pagination/error behavior.
