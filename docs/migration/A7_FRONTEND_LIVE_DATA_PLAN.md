...
The most important distinction is loader presence versus a production-usable flow. `/analytics`, `/plans`, `/portfolio`, and dynamic payment have SSR fetch hooks whose `data_*` payload is not consumed by the page. Dashboard and developer usage still consume canned in-process values. The focused news slice now consumes strict dependency outcomes without frontend sample fallbacks, but it remains partial for the producer-contract and runtime-proof gaps above. Other pages deserialize live data but silently replace failures with samples, zeroes, or empty arrays. None of those remaining cases is live-data readiness.
