// capture.js — Playwright runner: visit a route, click every interactive
...
//   EPSX_AUTH_COOKIE     full Cookie header (e.g. "session=abc; csrf=xyz")
//   EPSX_AUTH_BYPASS_DEV set to "1" to add 0x...d3v1 dev-bypass cookie
//
// Exit codes:
...
const EXTRA_COOKIE = args.cookie || process.env.EPSX_AUTH_COOKIE || "";
const DEV_BYPASS = process.env.EPSX_AUTH_BYPASS_DEV === "1" ||
...
(async () => {
  ...
  // pinned 1208 cache. Playwright 1.60.0 ships with 1223, but 1208 is what
  // wave 22 verified works. The two are wire-compatible (both are recent
    ...
    try {
      ...
      const interactive = await page.evaluate((budget) => {
        ...
        for (const el of els.slice(0, budget)) {
          ...
          seen.add(el);
          // skip if not in viewport (we'll only click visible ones)
      ...
      for (const el of interactive) {
        try {
          ...
          // re-query fresh handle — interactives were captured at evaluate-time
      ...
      console.log(
        `  ok status=${resp ? resp.status() : "?"} nav=${urlBefore} -> ${urlAfter} interactions=${interactions.length}`,
      );
      totalOk += 1;
    } catch (e) {
