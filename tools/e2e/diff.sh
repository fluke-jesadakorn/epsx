#!/usr/bin/env bash
# diff.sh — diff prod vs dev baselines, emit per-slug diffs + a Markdown report.
#
# Usage:
#   bash tools/e2e/diff.sh                    # all 28
#   bash tools/e2e/diff.sh home,about,dashboard   # subset
#
# Outputs:
#   tools/e2e/diff/<slug>.json                structured per-slug diff
#   tools/e2e/diff/<slug>.diff.png            pixel-diff visualization
#   tools/e2e/diff/_summary.tsv               one row per slug
#   tools/e2e/report.md                       Markdown table + issue digest
set -u

HERE="$(cd "$(dirname "$0")" && pwd)"
SCRIPTS="$HERE/scripts"
PROD_DIR="$HERE/baselines/prod"
DEV_DIR="$HERE/baselines/dev"
DIFF_DIR="$HERE/diff"
LOG_DIR="$HERE/logs"
...
# Now aggregate the per-slug JSON files into a Markdown report
