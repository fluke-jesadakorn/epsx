...
for (const group of campaignGroups) {
  ...
  for (const scenario of scenarios) {
    for (const matrix of matrices) {
      test(
        `group ${group.id}: ${scenario.id} [${matrix.id}]`,
        async ({ browser, browserName }, testInfo) => {
          ...
          const checks = {
            ...
            sourceDom:
              new Set(captures.source.map(capture => capture.domSha256))
                .size === 1,
            sourceAccessibility:
              new Set(
                captures.source.map(capture => capture.accessibilitySha256)
              ).size === 1,
            targetScreenshot: screenshotEquivalence.target.equivalent,
            targetDom:
              new Set(captures.target.map(capture => capture.domSha256))
                .size === 1,
            targetAccessibility:
              new Set(
                captures.target.map(capture => capture.accessibilitySha256)
              ).size === 1,
          };
