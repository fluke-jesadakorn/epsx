...
ROOT=shared.ROOT
BUNDLE=Path(os.environ.get('DIOXUS_ADMIN_AUDIT_BUNDLE',str(ROOT/'target/dx/dx-admin/debug/web')))
SOURCE_BINARY=Path(os.environ.get('DIOXUS_ADMIN_AUDIT_BINARY',str(BUNDLE/'server')))
OUT=ROOT/'target/dioxus-migration/admin-recovery'
...
class Backend(adminfixture.Backend):
 def end_headers(self):
  if urlparse(self.path).path=="/api/auth/session/refresh":self.send_header("x-epsx-refresh-outcome","rotated")
  super().end_headers()
 def do_GET(self):
 ...
 def do_POST(self):
  ...
  if path in ['/api/auth/web3/verify','/api/auth/session/refresh']:
   if path.endswith('/refresh'):time.sleep(2)
   if path.endswith('/verify'):assert body['signature']=='0xfixture-signature' and body['client_id']=='epsx-admin'
   return self.reply({'success':True,'authenticated':True,'wallet_address':shared.WALLET,'permissions':[],'access_token':TOKEN,'refresh_token':'fixture-refresh','expires_in':600,'refresh_expires_in':3600,'user':{'wallet_address':shared.WALLET,'subject':shared.WALLET,'permissions':[],'auth_method':'web3_siwe'}})
...
def browser(*args):
 result=subprocess.run(['agent-browser','--session','epsx-dioxus-admin-recovery',*args],capture_output=True,text=True,timeout=35)
...
def main():
 ...
 with (OUT/'server.log').open('w') as log:
  binary=OUT/f'server-{os.getpid()}';shutil.copy2(SOURCE_BINARY,binary)
  ...
  try:
   ...
   browser('cookies','set','epsx.admin.refresh_token','fixture-refresh','--url',base,'--httpOnly','--sameSite','Lax')
   browser('open',base+'/plans')
   browser('eval','window.originalDocument=document')
   browser('wait','--text','Fixture day plan')
   ...
   assert len([path for path,body in CALLS if path=='/api/auth/session/refresh'])==1
   browser('wait','1000')
   assert len([path for path,body in CALLS if path=='/api/auth/session/refresh'])==1
   errors=browser('errors');assert 'panic' not in errors.lower() and 'error' not in errors.lower(),errors
   records.append('Refresh-cookie-only /plans recovers verified Admin session exactly once without document reload or Router panic')
   browser('find','role','link','click','--name','Edit plan');browser('wait','input[name="name"]')
   assert browser('eval','document===originalDocument')=='true'
   assert 'panic' not in browser('errors').lower()
   assert len([path for path,body in CALLS if path=='/api/auth/session/refresh'])==1
   records.append('Recovered Admin Router remains interactive after remount')
