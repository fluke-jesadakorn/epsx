...
OUT=ROOT/'target/dioxus-migration/admin-escrow'
...
class Backend(adminfixture.Backend):
 def do_GET(self):
  ...
  if path.startswith('/api/v1/admin/pay/escrows') or path.startswith('/api/v1/pay/escrows') or path.startswith('/api/v1/pay/intents/'):
   item={'id':PLAN,'description':'Fixture escrow dispute','amount':'5000000','token':'USDT','token_decimals':6,'payee':'0x'+'2'*40,'status':'disputed','available_actions':['resolve-release','resolve-refund']}
   return self.reply(item if path.endswith(PLAN) else {'items':[item]})
  if '/operations/' in path or '/contract-controls/' in path:
   return self.reply({'id':'op_fixture','status':'confirmed' if CONFIRMED else 'pending'})
 ...
 def do_POST(self):
  ...
  if path.startswith('/api/v1/'):
   if path.endswith('/confirm'):return self.reply({'id':'op_fixture','status':'pending'})
   tx={'from':shared.WALLET,'to':'0x'+'2'*40,'data':'0x','value':'0x0','chainId':'0x7a69'}
   return self.reply({'id':'op_fixture','operation':{'id':'op_fixture'},'status':'awaiting_signature','transaction_parameters':tx})
...
def browser(*args):
 result=subprocess.run(['agent-browser','--session','epsx-dioxus-admin-escrow',*args],capture_output=True,text=True,timeout=35)
...
def main():
 ...
 with (OUT/'server.log').open('w') as log:
  ...
  try:
   ...
   browser('open',base+f'/pay/escrows/{PLAN}');browser('wait','[data-dioxus-admin-escrows]')
   browser('wait','--text','Fixture escrow dispute')
   browser('eval',f'''window.originalDocument=document;window.wrongChain=true;window.sendCount=0;window.ethereum={{request:async({{method}})=>{{if(method==='eth_requestAccounts'||method==='eth_accounts')return ['{shared.WALLET}'];if(method==='eth_chainId')return window.wrongChain?'0x38':'0x7a69';if(method==='wallet_switchEthereumChain')throw Error('Fixture rejected network switch');if(method==='eth_sendTransaction'){{window.sendCount++;return '{HASH}';}}if(method==='eth_getTransactionReceipt')return null;throw Error(method);}}}};''')
   browser('find','role','button','click','--name','Release to recipient');browser('wait','--text','Fixture rejected network switch')
   assert browser('eval','sendCount')=='0'
   records.append('Wrong chain cannot submit Admin escrow resolution')
   browser('eval','window.wrongChain=false');browser('find','role','button','click','--name','Release to recipient')
   browser('wait','--text','Waiting for service confirmation')
   assert browser('eval','sendCount')=='1'
   browser('eval','Array.from(document.querySelectorAll("button")).find(b=>b.textContent==="Release to recipient").click()')
   assert browser('eval','sendCount')=='1'
   assert 'Transaction verified on chain' not in browser('get','text','main')
   records.append('Admin resolution stays pending and blocks duplicate wallet sends')
   CONFIRMED=True;browser('wait','--text','Transaction verified on chain')
   assert browser('eval','document===originalDocument')=='true'
   records.append('Only backend confirmation marks resolution complete')
   browser('find','role','link','click','--name','Merchant escrows');browser('wait','--url',base+'/pay/merchant-escrows')
   browser('select','select[aria-label="Environment"]','live');browser('wait','--url','**environment=live')
   assert browser('eval','document===originalDocument')=='true'
   browser('back');browser('wait','--url',base+'/pay/merchant-escrows')
   records.append('Merchant environment and Back preserve hydrated document')
   for width,height in [(1440,900),(390,844)]:
    browser('set','viewport',str(width),str(height));time.sleep(.4);browser('screenshot',str(OUT/f'escrows-{width}.png'))
    assert browser('eval','document.documentElement.scrollWidth<=innerWidth')=='true'
   records.append('Escrow desktop/mobile layout has no horizontal overflow')
