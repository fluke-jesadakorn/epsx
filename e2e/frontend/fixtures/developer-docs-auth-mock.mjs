...
const issuer = process.argv[4] ?? 'http://127.0.0.1:18081';
const kid = 'a7-developer-docs-ephemeral';

if (!keyPath || !['generate', 'serve', 'token'].includes(command)) {
  throw new Error(
    'usage: developer-docs-auth-mock.mjs <generate|serve|token> <private-key-path> [issuer]'
  );
}
...
function accessToken() {
  ...
  const wallet = '0x0000000000000000000000000000000000000a7d';
...
if (
  ...
  throw new Error('the A7 developer docs fixture requires a plain 127.0.0.1 HTTP origin');
}
const port = Number(parsedIssuer.port);
if (!Number.isInteger(port) || port < 1024 || port > 65535) {
  throw new Error('the A7 developer docs fixture requires an explicit unprivileged port');
}
