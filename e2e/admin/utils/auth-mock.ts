/**
 * Auth Mock Helper for Admin Frontend E2E Tests
 * Sets cookies and intercepts auth endpoints to simulate an authenticated admin session.
 */
import { type Page } from '@playwright/test';
import { MOCK_ADMIN, MOCK_TOKEN, ADMIN_PERMISSIONS } from '../fixtures/api-mocks';

const COOKIE_DOMAIN = 'localhost';

export async function mockAuth(page: Page, user = MOCK_ADMIN, token = MOCK_TOKEN) {
  const baseURL = page.context().pages()[0]?.url() ?? 'http://localhost:3001';
  ...
  await page.route('**/api/auth/users/permissions', route =>
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { permissions: ADMIN_PERMISSIONS }, success: true }) })
  );
