//! Canonical RS256/JWKS authentication boundary for extracted Rust services.
//!
//! Callers derive [`VerifiedPrincipal`] exclusively from a verified bearer
//! token. Inbound identity or role headers are never identity sources.
...
/// Authenticate a service request from its Authorization header only.
///
/// Middleware can insert the returned principal into request extensions after
/// this succeeds. Caller-supplied identity, wallet, role, and permission
/// headers are deliberately ignored and must never be treated as principals.
pub async fn authenticate_headers<V>(
    verifier: &V,
    headers: &HeaderMap,
) -> Result<VerifiedPrincipal, VerifyError>
where
    V: AccessTokenVerifier + ?Sized,
{
    verifier.verify(extract_bearer(headers)?).await
}
...
mod tests {
    ...
    #[tokio::test]
    async fn service_header_api_uses_only_one_strict_bearer() {
        let key = TestKey::generate();
        let verifier = verifier(&key);
        let token = key.sign(claims(), Algorithm::RS256);

        let mut headers = HeaderMap::new();
        headers.insert("x-user-id", "attacker".parse().unwrap());
        assert!(authenticate_headers(&verifier, &headers).await.is_err());

        headers.insert(
            header::AUTHORIZATION,
            format!("Bearer {token}").parse().unwrap(),
        );
        let principal = authenticate_headers(&verifier, &headers).await.unwrap();
        assert_eq!(principal.subject, "0xabc");
        assert_eq!(principal.wallet_address, "0xabc");
