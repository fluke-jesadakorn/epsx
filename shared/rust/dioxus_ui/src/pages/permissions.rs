//! /permissions — user permissions with tabs.
...
#[component]
fn PermCard(name: String, granted: bool, source: String, expires: String) -> Element {
    rsx! {
        div { class: if granted { "perm-card card card-glass border-success" } else { "perm-card card card-glass opacity-50" },
            div { class: "card-body",
                div { class: "flex items-center justify-between",
                    div { class: "font-bold", "{name}" }
                    if granted {
                        span { class: "badge badge-success", "Granted" }
                    } else {
                        span { class: "badge badge-info", "Not granted" }
                    }
                }
                p { class: "text-sm text-muted-foreground mt-2", "Source: {source}" }
                p { class: "text-sm text-muted-foreground", "Expires: {expires}" }
            }
        }
    }
}
