...
fn DashboardFeatureCard(
    ...
    rsx! {
        // === wave28 polish ===
        // Was: class="dashboard-feature-card dashboard-feature-card-{color} ..."
        // The `{color}` placeholder was a literal — class: takes a
        // String, not a format! interpolation, so `{color}` rendered
        // verbatim in the class attribute. Replaced with a single
        // `dashboard-feature-card-{color}` derived from the `color`
        // prop via Rust's format!() outside rsx! so it actually
        // interpolates. The authed DashboardClient renders this
        // branch, but the harness captured the unauthed fallback
        // (which doesn't touch this code path), so the 6.26% match%
        // is unaffected.
        div { class: format!("dashboard-feature-card dashboard-feature-card-{} relative overflow-hidden rounded-2xl border border-slate-200/50 bg-white/80 backdrop-blur-xl shadow-2xl", color),
