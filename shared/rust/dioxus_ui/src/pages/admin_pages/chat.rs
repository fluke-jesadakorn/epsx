...
pub fn render(ctx: &PageContext) -> (PageMeta, Element) {
    ...
    (meta, rsx! {
        AuthGate { user: ctx.user.clone(), feature: Some("support chat".to_string()),
            DashboardShell { current_path: ctx.path.clone(), page_title: "Support chat".to_string(), user: ctx.user.clone(),
...
fn RenderAdminConversation(ctx: PageContext) -> Element {
    ...
    rsx! {
        AuthGate { user: ctx.user.clone(), feature: Some("support conversations".to_string()),
            DashboardShell { current_path: ctx.path.clone(), page_title: "Conversation".to_string(), user: ctx.user.clone(),
                div { class: "container page-content",
