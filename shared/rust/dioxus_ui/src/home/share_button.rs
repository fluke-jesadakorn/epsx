...
pub fn ShareButton(
    ...
    let _ = on_click;
    // SSR delegates this typed action to the generated Rust/WASM runtime.
    ...
    let button_html = format!(
        r#"<button type="button" class="home-prod-share-btn w-full sm:w-auto min-w-[220px] h-14 text-lg font-bold bg-gradient-to-r from-orange-500 to-yellow-500 hover:from-orange-600 hover:to-yellow-600 text-white border-2 border-orange-400/50 rounded-2xl shadow-xl hover:shadow-orange-300/30 hover:scale-105 transition-all duration-300 group {}" data-share-text="" data-share-title="EPSX" data-epsx-action="share" aria-label="Share Platform"><span class="epsx-icon">{}</span><span>📤 Share Platform</span></button>"#,
        extra_class, icon,
    );
    rsx! {
