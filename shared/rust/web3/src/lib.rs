...
pub fn bsc_provider() -> Result<Box<dyn Provider + Send + Sync>> {
    let url: reqwest::Url = "https://bsc-dataseed1.binance.org"
        .parse()
        .map_err(|e: url::ParseError| Web3Error::Provider(e.to_string()))?;
    Ok(Box::new(ProviderBuilder::new().connect_http(url)))
}

pub fn bsc_testnet_provider() -> Result<Box<dyn Provider + Send + Sync>> {
    let url: reqwest::Url = "https://data-seed-prebsc-1-s1.binance.org:8545"
        .parse()
        .map_err(|e: url::ParseError| Web3Error::Provider(e.to_string()))?;
    Ok(Box::new(ProviderBuilder::new().connect_http(url)))
}
...
pub fn provider_for_chain(chain_id: ChainId) -> Result<Box<dyn Provider + Send + Sync>> {
    match chain_id.0 {
        ...
        _ => Err(Web3Error::Provider(format!(
            "Chain {} not supported",
            chain_id
        ))),
    }
...
pub fn usdt_address(chain_id: ChainId) -> Option<Address> {
    Token::USDT
        .address(chain_id)
        .and_then(|a| Address::from_str(&a.0).ok())
}
...
pub async fn fetch_eth_balance<P: Provider + ?Sized>(
    provider: &P,
    address: Address,
) -> Result<U256> {
    provider
...
pub async fn fetch_block<P: Provider + ?Sized>(
    provider: &P,
    number: u64,
) -> Result<Option<alloy_rpc_types::Block>> {
    provider
...
pub async fn fetch_tx_receipt<P: Provider + ?Sized>(
    provider: &P,
