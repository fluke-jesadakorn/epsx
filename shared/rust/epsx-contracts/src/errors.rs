// kernel extraction wave9 — moved verbatim from apps/backend/src/core/errors.rs
// Two changes vs. the source:
//   1. `From<crate::application::shared::error::ApplicationError>` impl was
//      removed (that bridge is application-layer concern; it now lives in
//      the `apps/backend/src/core/errors.rs` shim).
//   2. `From<crate::domain::shared_kernel::value_object::ValueObjectError>`
//      re-pointed at `crate::value_object::ValueObjectError` (the trait
//      moved into this crate in the same wave).
...
// NOTE: `From<crate::application::shared::error::ApplicationError>` is in
// the `apps/backend/src/core/errors.rs` shim — that bridge depends on an
// application-layer type that does not (and should not) live in this kernel.

// Value object error conversion (re-pointed at the kernel `ValueObjectError`).
...
impl From<Box<dyn std::error::Error>> for AppError {
    fn from(err: Box<dyn std::error::Error>) -> Self {
        AppError::new(ErrorKind::InternalError, format!("Internal error: {}", err))
    }
}
...
mod tests {
    ...
    /// Construct an AppError from a string and check that the kind round-trips.
    #[test]
    fn smoke_validation_error_round_trip() {
    ...
    /// Verify the `From<&str>` conversion path (one of the From impls the
    /// wave-9 audit needs to keep working) compiles and behaves correctly.
    #[test]
    fn smoke_from_str_conversion() {
        let err = AppError::from("network blip");
        assert_eq!(err.kind, ErrorKind::InternalError);
        assert!(err.message.contains("network blip"));
    }
}
